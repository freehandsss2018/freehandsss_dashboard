...
// Fixed UUIDs so the URL is stable
const WEBHOOK_ID   = 'fo-webhook-fhs-2026';
const WEBHOOK_PATH = 'financial-overview-fhs';
...
async function run() {
  ...
  if (!create.body || !create.body.id) {
    console.log('Failed:', JSON.stringify(create).substring(0, 500));
    return;
  }
  ...
  // Find actual webhook path from created workflow
  const wh = (create.body.nodes || []).find(n => n.type === 'n8n-nodes-base.webhook');
  const actualPath = wh ? wh.parameters.path : WEBHOOK_PATH;
  const actualWebhookId = wh ? wh.webhookId : 'unknown';
  console.log('Actual webhook path:', actualPath);
  console.log('Actual webhookId:', actualWebhookId);
  ...
  console.log('Testing webhook at /webhook/' + actualPath + ' ...');
  const test = await callApi('/webhook/' + actualPath, 'GET', null);
  console.log('Test status:', test.status);
  if (test.body && test.body.current) {
    console.log('SUCCESS! Revenue:', test.body.current.revenue, '| Profit:', test.body.current.profit);
  } else {
    console.log('Response:', JSON.stringify(test.body || test.raw).substring(0, 300));
  }

  console.log('\n=== SUMMARY ===');
  console.log('Workflow ID:', wfId);
  console.log('Webhook URL: https://yanhei.synology.me:8443/webhook/' + actualPath);
}
