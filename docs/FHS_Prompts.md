...
觸發：用戶提及「對帳」「Live 驗證」「訂單成本比對」「三端財務」「財務稽核」「Total_Cost 不對」「利潤差異」「成本差了」「驗證公式違規」「net_profit ≠ final_sale_price − total_cost」「某單利潤/成本啱唔啱」「財務規則／定義疑問（SKU tier、成本點計、快照可否重算）」（v1.16 補）
> 🔴 v1.16（2026-09-19，0600804 事故）：AI **毋須等用戶開口**，條件成立即主動派 `finance-auditor`（可背景跑）；向 Fat Mo 提財務問題前必須已派過；財務驗收只認其報告。Stop hook `stop-finance-auditor.js` 會攔截漏派（見 AGENTS.md v1.7.3「財務派工補充條款」）。
...
配套 Skills：`.fhs/ai/skills/finance-gatekeeper/SKILL.md`（§〇 強制派工閘 + 查詢路由 + 6 條死線；財務驗證/疑問必派 finance-auditor）、`.fhs/ai/skills/vendor/awesome-cc/read-only-postgres.md`（Supabase 就緒）
