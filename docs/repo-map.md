...
│   │   └── 0096_get_financial_charts_adjustment_and_monthly_window.sql ← cl-flow-fast 2026-09-23-1957：get_financial_charts() trend補adjustment_amount（原漏計）+ monthly tab窗口改曆月對齊（原日曆日滾動窗）；category_revenue/cost_breakdown逐字不變 ✅ 已部署
│   ├── rls/
...
│   │   │       ├── finance-auditor.md   ← v2.2.0 四端財務稽核員（v1.4.10 對齊、收款確收語義、V47.15、動態現況，2026-06-12 升級）
│   │   │       ├── tdd-guide.md         ← v1.1.0 TDD 測試驅動開發 subagent（Python + n8n 專用，2026-07-07 S152 十大框架條款吸收）【指令已退役，subagent 保留】
...
│   │   ├── fatmo-ops-quickcard.md        ← Fat Mo 人讀操作速查卡（S176 新增，D39 cl-flow A3-first 試點產出；harness 內建能力+FHS 指令核心集）
...
│   ├── canva_memory_validate.js        ← canva_auto/placement_memory.json schema v2 防退化校驗（require+CLI 兩用，D77，2026-09-13）
