-- 0088_sync_rpc_accessory_cost_restore.sql
-- 2026-08-16 (D64) — sync_order_to_mirror() accessory_cost 讀寫回歸修復
--
-- 【根因鏈】
-- 1. Migration 0080（2026-07-25）首次為 sync_order_to_mirror() 加入 accessory_cost
--    讀寫（orders + order_items 兩表），finance-auditor 曾獨立覆核 PASS。
-- 2. Migration 0081（2026-07-28，家庭/大寶/成人 V2 成本模型）用 CREATE OR REPLACE
--    FUNCTION 整個覆蓋同一 RPC，但其 base 版本源自更舊的 0075（跳過咗 0080），
--    令 accessory_cost 讀寫靜默消失——0081 本身無意改動配件成本邏輯，純粹係
--    「手抄漏一個欄位就會整全覆蓋打回舊版」（見 feedback_migration_repo_db_drift）。
-- 3. Migration 0087（2026-08-11，D63 續，修 deleted_at 冇 reset 嘅 bug）首版手抄時
--    已經用 pg_get_functiondef() 攞 live 定義，並且實測發現「live 版本 accessory_cost
--    0 次出現」，但當時誤判為「本來就冇呢個欄位」而非「已經回歸」，於是喺 0087 註解
--    寫低「刻意不順手補，維持單一改動原則」——呢個判斷令回歸被正式記錄成現狀，
--    一直冇人跟進到而家先發現。
--
-- 【現況實測】orders.accessory_cost / order_items.accessory_cost 喺 0081 後建立/更新
-- 嘅所有訂單恆為 0.00（含燈飾/羊毛氈等配件品項嘅訂單直接查 DB 可驗證）。
-- total_cost / net_profit 本身唔受影響——配件成本一直有計入品項/訂單總數，純粹係
-- 分類 rollup（accessory_cost 獨立顯示欄）出現缺口，非算錯錢。
--
-- 【生成方式（防漂移，沿用 0087 先例）】本檔由 live 定義（pg_get_functiondef）以
-- Python 程式化單一錨點插入 6 行生成（orders 表 3 處 + order_items 表 3 處：
-- INSERT 欄位清單、VALUES、ON CONFLICT UPDATE，邏輯與 0080 原始版本逐字一致，
-- 用 COALESCE(...,0) 保底避免 n8n 未傳值時炸 NULL），並經程式 diff 驗證「除呢 6 處
-- 改動外，其餘逐字不變」，避免 CREATE OR REPLACE 全量覆蓋再次打回未落 repo 嘅既有
-- 修復（0087 的 deleted_at=NULL 已於下方定義中保留）。
...
-- Smoke test：函式簽名存在 + 定義文本確實包含 accessory_cost（防止靜默漏補）
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  IF (
    SELECT pg_get_functiondef(oid) NOT ILIKE '%accessory_cost%'
    FROM pg_proc
    WHERE proname = 'sync_order_to_mirror' AND pronargs = 5
  ) THEN
    RAISE EXCEPTION 'sync_order_to_mirror() 定義仍未包含 accessory_cost，migration未生效';
  END IF;
END $$;
