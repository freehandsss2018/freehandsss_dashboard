...
// 文件健康快檢由 SessionStart hook 呼叫，無外部依賴。
// 可由規則檔啟用體積、索引、重複、斷鏈、複驗、週期與交接一致性檢查。
...
//
// Version: 0.1.0 | portable template
...
const REPO_ROOT = process.env['{{ENV_PREFIX}}_HEALTH_ROOT'] || path.resolve(__dirname, '../..');
const RULES_FILE = process.env['{{ENV_PREFIX}}_HEALTH_RULES'] || path.join(REPO_ROOT, '.fhs/tools/fhs-health-rules.json');
const CANONICAL_KEYS_FILE = path.join(REPO_ROOT, '.fhs/tools/canonical_keys.yml');
const REPORT_FILE = process.env['{{ENV_PREFIX}}_HEALTH_REPORT_OUT'] || path.join(REPO_ROOT, '.fhs/.health-report.json');
const ERROR_LOG = process.env['{{ENV_PREFIX}}_HEALTH_ERROR_LOG'] || path.join(REPO_ROOT, '.fhs/.health-check-error.log');
...
function checkCanonicalDrift() {
  ...
  try {
    ...
    for (const [keyName, def] of Object.entries(keys)) {
      ...
      // Structured 值只從顯式 reference_pattern 標記讀取，避免把其他文件的普通版本號誤判為參照。
      let refRe = truthRe;
    ...
    try {
      if (rule.scan_dirs) {
        ...
        for (const f of common) {
          issues.push(`重複: "${f}" 同時存在於外部記憶目錄與 ${rel(bDir)}`);
        }
      }
    } catch (err) {
...
// ── CHECK 5b: 未註冊桶 unregistered_file_checks───
...
// ── CHECK 5c: 複驗逾期 last_verified_checks──────
...
// ── CHECK 5d: 已完成項目仍留在進行中表格 ────────────────────────────────
// 規則指定表格與完成標記；已完成項目應移至歸檔區。
  ...
  for (const rule of rules.pending_table_leak_checks || []) {
    try {
      ...
      if (leaked.length > (rule.budget || 0)) {
        issues.push(`歸檔洩漏: ${rel(scanPath)} 進行中表格有 ${leaked.length} 項已完成但未歸檔（規定上限 ${rule.budget || 0} 項，${rule.source}）`);
      }
    } catch (err) {
...
// 記憶負擔歸零：定期檢查由健康規則配置觸發，毋須靠人手記住檢查週期，改由既有
...
// ── CHECK 7: 時限待辦漏帶 deadline_surfacing_checks──────────────
...
// ── CHECK 8: 便攜塊與持續待辦表不一致 ─────────────────────────────────
// 若便攜塊的待辦識別碼已在持續待辦表標為完成，提醒更新交接摘要。
...
function main() {
  ...
  if (issues.length > 0) {
    process.stdout.write(`⚠️  健康檢查：${issues.length} 項異常；詳情見報告。\n`);
    process.stdout.write(`   → 詳情見 .fhs/.health-report.json，跑 /health-check 看清理方案\n`);
  }
