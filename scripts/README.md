...
| `hooks/prompt-router.js` | `UserPromptSubmit` | 分析任務描述，建議最適 subagent / skill / model（建議模式，不強制）；v2.1.0（2026-09-19）財務路由改「🔴 必須派 finance-auditor」，並獨立於 first-match 疊加財務訊號提示 |
...
| `hooks/stop-finance-auditor.js` | `Stop` | 財務必派 finance-auditor 把關（2026-09-19，0600804 事故方案C-C6）：本輪有財務訊號（載 finance-gatekeeper／SQL 或 curl 觸及財務欄位／Read 財務權威文件／prompt 含財務字眼／回覆向 Fat Mo 提財務問題）而近 5 輪未派 `finance-auditor`，即回傳 `decision:block` 攔截收尾一次；`stop_hook_active` 放行防死鎖；非財務誤觸可寫【finance-auditor 豁免：理由】；fail-open；回歸測試 `hooks/test/run-finance-stop-fixtures.js`（20/20）|
| `hooks/fhs-health-check.js` | 由 `session-start-sop.sh` 末尾呼叫（非獨立 hook 掛載） | L1 文件健康快檢：零依賴，現偵測8類病——過肥/沉積孤兒/過時漂移/同名重複/歸檔斷鏈/未註冊桶/複驗逾期（2026-08-03 分桶重構新增5b/5c）+ 週期稽核到期（第6檢查，2026-07-05 S143：`/fhs-audit` 逾90天未跑才提醒）+ 時限待辦漏帶入便攜塊（第7檢查，2026-08-05：MASTER表帶未來日期嘅待跟進項若便攜塊動態段未反映即報警）+ P0.6歸檔洩漏（第8檢查，2026-08-26 D69續五：MASTER持續待辦表區間內殘留「✅完成」列未搬去歸檔區即報警，budget=0，防P0.6規則靜默停擺），規則資料在 `.fhs/tools/fhs-health-rules.json`，正常沉默、異常 ≤2 行警示，<2 秒；報告見 `.fhs/.health-report.json`；清理走 `/fhs-slim` 指令；fail-open（node不存在/腳本出錯絕不擋 session 啟動）；回歸測試見 `hooks/test/`（14組 fixtures，S142建10組+S143加cadence 2組+D69續五加P0.6洩漏偵測2組）|
