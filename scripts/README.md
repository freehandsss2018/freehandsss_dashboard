...
| `hooks/fhs-health-check.js` | 由 `session-start-sop.sh` 末尾呼叫（非獨立 hook 掛載） | L1 文件健康快檢：零依賴，現偵測7類病——過肥/沉積孤兒/過時漂移/同名重複/歸檔斷鏈/未註冊桶/複驗逾期（2026-08-03 分桶重構新增5b/5c）+ 週期稽核到期（第6檢查，2026-07-05 S143：`/fhs-audit` 逾90天未跑才提醒）+ 時限待辦漏帶入便攜塊（第7檢查，2026-08-05：MASTER表帶未來日期嘅待跟進項若便攜塊動態段未反映即報警，修補「最高優先3條」篩選器結構性漏帶時限項的漏洞），規則資料在 `.fhs/tools/fhs-health-rules.json`，正常沉默、異常 ≤2 行警示，<2 秒；報告見 `.fhs/.health-report.json`；清理走 `/fhs-slim` 指令；fail-open（node不存在/腳本出錯絕不擋 session 啟動）；回歸測試見 `hooks/test/`（12組 fixtures，S142建10組+S143加cadence 2組，新增第7檢查尚未補fixture）|
