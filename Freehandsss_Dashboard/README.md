...
- **2026-09-15（D79）**：訂單總覽批次色重做（方案C）——底色改跟訂單斑馬紋、顏色改跟批次（極淡底+色條+標籤），取代舊全格填色，解決混批/未入批單顏色分裂問題；順手修埋手機改批次冇即時變色嘅既有問題。fresh-context agent 用62張生產真單13項驗收全PASS。`V42.html` 已改，`current.html` 尚未同步，待 Fat Mo 授權部署。詳見 Changelog.md D79、decisions.md D79。
...
- **2026-09-24（訂單總覽鎖匙扣與頸鏈預設批次排序＋搜尋預設全部模式）**：
  1. 鎖匙扣與頸鏈預設批次排序：在類別視圖（鎖匙扣/頸鏈）預設排序下，未填/空批次（沒有空間）排在最頂部，其次依照最新批次降序排列（如 第36批 > 第35批 > 第34批...），同批次內則按訂單建立日期降序排列。
  2. 搜尋預設切換全部模式：點擊搜尋框輸入資料時，自動將分頁過濾切換為「全部」模式（不再侷限於「進行中」或「已完成」），清空搜尋文字時平滑復原先前的分頁選擇；搜尋狀態下亦支援手動點選分頁切換。
- **2026-09-23（訂單總覽篩選輸入框防走位＋底部功能Bar動態收納＋取模日曆手勢切換與防背景穿透滾動）**：
  1. 訂單總覽手機版點擊搜尋/輸入方框防走位：字級調升至 16px 消除 iOS Safari 強制 Auto-Zoom 與視窗平移；移除 translateY(-200px) 位移並在 scroll 監聽器加入焦點守衛，篩選列常駐置頂。
  2. 新/修訂單底部功能 Bar 動態表現對齊：解除 `navScope` 原先僅限總覽模式（`inReview`）的限制，全模式（<1130px）向下捲收起、向上捲浮現（Threads 式），切換模式即時復位展開。
  3. 取模日曆觸控增強與防背景穿透滾動：加入單指左右滑動切換月份手勢（左滑下月、右滑上月，附帶位移比對門檻防止誤觸）；開啟時鎖定 `body` overflow 與 touch-action，並在遮罩層攔截 touchmove 穿透，根治後方訂單總覽跟隨滑動問題。已同步升格 `Freehandsss_dashboard_current.html` 並部署 NAS。
