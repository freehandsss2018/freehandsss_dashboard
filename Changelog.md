...
- 第 2 次實審（Codex，2 條 P1）修正：`/a4-review` v1.1.0 範圍改基準 SHA；8 個寫入類 Codex 橋接改為拒絕並指向 A3。
...
- 逐列核對無「✅ 完成主體＋未來待跟進」混寫；`[便攜塊超支]` 列「是否調整預算數值待裁決」已由同日便攜塊輪轉解決（3,962 ≤ 4,000 bytes）。驗證：搬入列同原文逐列相同、其餘內容一致；health 夾具 16/16；健檢異常 7→6（P0.6 洩漏消失）。**Subagent 使用記錄**：❌未使用。
...
- 補做覆核：拷問技能窗口內真實使用 3 次（07-12／07-16×2）＋09-23 十輪拷問，**試用閘通過**；council 判準（D30／D36／D39＋D43）數字上通過。Fat Mo 因暫時冇時間，**兩項都選繼續延長審視期**（期限 AI 預設 4 週＝2026-10-23，可改）；期間 council、to-spec 都不裝。
- 已同步：decisions.md D28 覆核補記、handoff MASTER 兩列、便攜塊 ⏰ 時限待辦、auto-memory 兩檔。**Subagent 使用記錄**：❌未使用。
...
- **真實剩餘（拆為獨立待辦列）**：①0600108 Ctungdear p3 小組合冇跟 p2 放大（有意定漏改，未答）；②Canva 廢棄／臨時副本待 UI 刪：`DAHVvi7ZCBI`、`DAHWHPJ9tz8`、`DAHWHux1-gs`、`DAHWHOxwu2k`；0600108／07001006／0600728 嘅存檔頁副本已不存在。
- SOP yes/no 行：③（CV-36／CV-30／CV-42 升唔升）已解，只剩 ①。**Subagent 使用記錄**：❌未使用。
...
## [2026-09-13] D77：Canva 學習記錄重構——placement_memory.json schema v2（規則編號表+逐Page列點）
...
- **③儲存篩選／編輯版面收納**：新增「選項」icon+下拉面板，撳先展開；財務/清除篩選維持常駐可見。面板用 `position:fixed`+JS動態算座標（開嗰刻搬去 `document.body`），避開篩選列容器 `overflow:hidden` 裁剪。
- **④搜尋輸入框前移**：由篩選展開區搬入常駐工具列，貼「顯示項目財務」左邊，縮短做110px。
- **⑤重新載入+筆數徽章合併**：桌面寬螢幕版都採用手機/緊縮桌面已有嘅合併設計，移去徽章原位（訂單標題隔籬）。
- 以上全部**淨限桌面「全部」寬螢幕視圖（≥1130px）**，類別視圖/合併層/手機卡片版完全唔受影響。
- **實測自揪修正一個真實 bug**：闊度可以由手機直跳寬桌面（一次 resize 冇經過緊縮桌面），令財務/清除篩選按鈕仲留喺舊 tier 嘅收納容器入面，觸發 `insertBefore` NotFoundError；修復方式係落手用嗰兩粒掣做定位錨點前，先保證佢哋已經返晒去工具列原位。
- **驗證方法論筆記**：本輪驗證接連撞到「DOM/CSSOM狀態完全正確，但工具自己嘅截圖/console記錄唔反映」嘅假訊號（見 decisions.md D75-follow 詳述）——開新分頁重新驗證係最快嘅排除法。
- **改動檔案**：`Freehandsss_Dashboard/freehandsss_dashboardV42.html`。
- 全文見 decisions.md D75-follow。**Subagent 使用記錄**：❌未使用（單一連續實作+多輪根因排查，委派會斷推理鏈）。

## [2026-09-11] D75：訂單總覽自由版面編輯（桌面「全部」寬螢幕視圖）

- **背景**：Fat Mo 要求桌面「全部」視圖（≥1130px）欄寬/高度/密度可自行持久化調節，唔使每次搵 AI 手動改。經 `/cl-flow-fast` 規劃（flow_id `2026-09-11-1441`，Gemini A2 對抗評審 7 條批評 6 採納 1 拒絕，Verdict APPROVED_READY）後實作。
- **新增「編輯版面」掣**（工具列，儲存篩選隔籬）：撳咗先顯示三項調節，未撳＝現行行為零改動。
- **①外框高度**：表格底部拖柄（`position:sticky;bottom:0`），拖動即時調節訂單總覽區域螢幕佔用高度，超出內部自己捲動，表頭釘住捲動容器頂。
- **②欄寬**：相鄰欄邊界拖拽（Excel 式），拖 A/B 中間線淨改呢兩欄，總闊度數學恆等於容器（zero-sum）。
- **③行高/密度**：連續 slider（75%-135%），CSS `calc()` 縮放全表字級/padding，預設 100% 時同原硬編碼值完全一致。
- **持久化**：`localStorage` `fhs_ovw_layout_v1`，淨限桌面「全部」tier；「重設」掣一鍵復原原生預設。
- **範圍**：類別視圖／手機橫向合併層／手機卡片版完全唔受影響（tier gate `_fhsDesktopFlexEng()`）。
- **驗證**：三個 tier 零回歸（逐欄寬度改動前後完全一致）；欄寬拖拽/持久化/容器闊度變化重算全部驗證通過；外框高度內部捲動+thead 貼頂驗證通過；密度 slider 精確符合 calc 公式，備註 focus 展開唔受影響；D74 封面圖功能完好；重設掣精確復原；console 零新增錯誤。過程中自揪並修正一個高度拖柄公式 bug（delta-based→絕對座標公式）。
- **改動檔案**：`Freehandsss_Dashboard/freehandsss_dashboardV42.html`。
- 全文見 decisions.md D75、`artifacts/2026-09-11-1441/`。**Subagent 使用記錄**：❌未使用（單一連續實作+Chromium 互動實測，委派會斷推理鏈）。
...
- **修復1**：冇圖狀態改為常駐顯示淡色 `icon-image` 圖示（20px，複用既有 sprite），唔算常駐 button（冇邊框/背景/文字）。
- **修復2**：真實生產列（0600303）量得 87.9px 列高、圖片下方 18.7px headroom，107×60 改 135×76（16:9放大26%，留2.7px緩衝）。
- **自揪新bug**：實作過程一度將兩個狀態共用 135×76，喺自家「手模」類別視圖（172px窄欄）驗證揪出刻字文字被逼直排亂碼——根因兩狀態爭同一 flex row 闊度。修復：`.has-img`維持135×76，冇圖狀態縮返30×30（純icon尺寸，唔搶文字位）。從未流出生產。
- **驗證**：真實 Supabase 55張單全量掃描零 overflow（全部/手模兩視圖）；相片尺寸/拖拽亮框/console 零error全PASS。
- **改動檔案**：`Freehandsss_Dashboard/freehandsss_dashboardV42.html`（`.fhs-cover-slot`系列CSS三規則）、`.fhs/notes/decisions.md`（D74-follow）。
- 全文見 decisions.md D74-follow。**Subagent 使用記錄**：❌未使用（真機截圖回報跨視圖真實資料驗證，委派會斷推理鏈）。
...
- **重新查證揪出真兇**：仔細對照截圖發現連頁面最頂嗰條篩選列都同表格一齊向內縮，證實留白喺整個頁面內容之外，根本唔關表格闊度演算法事（follow9/follow10 兩輪診斷方向由頭錯到尾）。查 body CSS 揪到 2026-08-30 D69續八-follow-11 加咗嘅 `padding-left/right: env(safe-area-inset-left/right, 0px) !important`——iOS Safari 橫向專屬嘅瀏海避讓區。
- **量化佐證**：iPhone 13 Pro 橫向邏輯闊度 844px，減去左右各 ~47px 安全區 padding＝750px——同另一條分支（D69續八-follow-2）當初將斷點由 768 改做 750 嘅根據係同一個現象，證實呢條 padding 本身一直客觀存在。
- **修復**：`padding-left/right` 由 `env(safe-area-inset-left/right, 0px)` 改做 `min(env(...), 8px)`——用 CSS `min()` 封頂，只留避開圓角所需嘅少量緩衝，攞返其餘 ~39px／邊俾表格用。已知取捨：瀏海嗰側最外側一小截內容會被實體遮住，但表格可橫向捲動，判斷為可接受。
- **驗證**：Chromium `CSS.supports()` 確認語法受支援；CSSOM 直接讀出規則正確生效；由 `.review-table-wrap` 逐層向上量到 `body`，全部容器 padding/margin 皆為 0，證實留白源頭單一。未能喺呢個環境直接驗證真機 Safari 視覺效果，留待 Fat Mo 實機覆核。
- **教訓**：follow9/follow10 兩輪都冇先核對「留白範圍係咪包含表格以外元素」就假設係表格 width 演算法問題。下次遇到「畫面留白」類回報，第一步應先查「留白範圍止於邊度」，範圍越界就應懷疑更高層共因，而非狹窄咁只查最顯眼嘅元件。
- **改動檔案**：`Freehandsss_Dashboard/freehandsss_dashboardV42.html`（body CSS `padding-left`/`padding-right`）、`.fhs/notes/decisions.md`（D71-follow11）。已同一批部署至 `current.html`（Gate 0 自動核實血統通過）。
- 全文見 decisions.md D71-follow11。**Subagent 使用記錄**：❌未使用（跨截圖範圍比對+CSS cascade 查證，委派會斷推理鏈）。
...
- **查證**：用真實訂單 0650429（Shirley Lee）逐格量度——①「x2」跌落新行：`review-badge` 容器淨得 110px，「嬰兒+左手+x2」啱好110px卡臨界點；②BOOK/LASER 冇分3行：`hmck-wrap-*` 用 `flex-wrap:wrap` 非 `flex-direction:column`，兩個 label 合埋 ~94-98px 仲夠 115px 嘅進度欄逼落同一行。兩者共同根因：呢層一直冧咗同 D73 同源嘅 `table-layout:fixed` 比例拉伸（實測 1000px 下全部欄按同一 ~1.42x 比例被拉伸），D73 嗰次修復從未推廣落呢層。
- **修復**：跟 D73 手法，備註做呢層嘅唯一彈性欄（JS 條件 `k==='notes' && !_c` 擴大去兩個 tier）；產品明細 130→165px；進度 115→75px（窄過 Book+Laser 組合逼佢哋逐個換行變3行）。
- **驗證**：Chromium 1000px 實測 8 個可見欄全部貼緊宣告值零拉伸；真實訂單「嬰兒/左手/x2」同一行、「Book/Laser/音訊」3行分開；桌面「全部」視圖（≥1130px）逐項重驗零回歸；類別視圖經程式碼審查不受影響；console 零 JS error。
- **誠實局限**：備註做彈性欄後 1000px 實測膨脹到 352px（原 56px 6倍多），視覺上可能比例失衡，越接近 1129px 上限會越大——呢個係「留一欄食晒落差」呢個技巧嘅必然取捨，已記錄待 Fat Mo 覆核是否需要另一種分配方式。
- **改動檔案**：`Freehandsss_Dashboard/freehandsss_dashboardV42.html`（750-1129 media query／`fhsBuildOverviewHead()`）、`.fhs/notes/decisions.md`（D71-follow9）。已同一批部署至 `current.html`（Gate 0 自動核實血統通過）。
- 全文見 decisions.md D71-follow9。**Subagent 使用記錄**：❌未使用（真實 DOM 逐格度量+跨 tier 交叉驗證，委派會斷推理鏈）。
...
- **改動檔案**：`Freehandsss_Dashboard/freehandsss_dashboardV42.html`、`.fhs/notes/decisions.md`（D71）、`.fhs/memory/handoff.md`。**Supabase schema／n8n 零改動。**
- **部署**：同一 `/commit` 內依 AGENTS.md §3 途徑(c) 自動觸發 Phase 2.5 升格，`/fhs-check` 4 PASS/1 SKIP，三關驗證 PASS（HTTP 204／大小 1,225,315 bytes 相符／SHA256 `36203277F6AAB2222306FBCC40A5496017A62C15EC2D7944CCA75D72D82B84A3`），已上線 `https://yanhei.synology.me/Freehandsss_dashboard_current.html`。Phase 2.6 主線同步（D70）本次首次實機觸發，`origin/main` 為 HEAD 祖先，fast-forward 成功推送落 `main`（`4881787..c34bc5f`）。
...
- **根因**：`current.html`（生產版）係一個**跨分支共享嘅部署目標**——任何分支執行 `/commit` Phase 2.5 途徑c 都會直接 PUT 上 NAS，呢個部署動作**喺 git 之外**（NAS 唔知道 git branch 概念，只認「最後一個 PUT 贏」）。本 session 喺 `claude/read-command-15bfd5` 分支連續 4 次 deploy（09:24/10:12/11:28/12:20，對應 D69/D69續/D69續II/D69續III），但喺當日 08:26 已經有另一條分支 `claude/order-overview-category-display-1a4f84` 部署咗佢自己嘅「D69續八-follow-31」——訂單總覽響應式/排版/密度/safe-area/spinner重複等 **31 輪**UI優化累積成果。本分支嘅 `freehandsss_dashboardV42.html` 喺 fork 主線後從未追蹤過對方改動，第一次 `cp V42.html → current.html` 就完整抹走對方成果，之後三次 deploy 更加深差距。**`/read` 開場其實已有警告**（"🔀 近48小時有動靜嘅其他並行分支"）列出咗呢條分支，但本 session 因為專注喺 Fat Mo 交辦嘅收款分帳 bug，冇停低核對佢嘅部署時間戳，直接假設自己 branch 嘅 dev 版就係最新。
- **查證範圍**：`git log --oneline origin/main..origin/claude/order-overview-category-display-1a4f84` 顯示 **96 個 commit**（約48輪 fix+deploy 配對），V42.html 淨增 2097 行；另有第三條分支 `claude/read-command-3a168b` 帶住一個獨立單行修復（「待定」排除），核實同本分支 D69續III 嘅修復**逐字相同**（另一 session 獨立揪到同一 bug，兩者收斂驗證，非衝突）；`claude/d65-family-owner-role` 分支查證後確認係 `order-overview-category-display-1a4f84` 嘅祖先（同一條開發線分岔），內容已全部包含喺後者，無需分開處理。
- **修復**：`git merge origin/claude/order-overview-category-display-1a4f84` 入本分支。實際代碼衝突意外地細——`freehandsss_dashboardV42.html` 僅 **1 個**衝突 hunk：兩個分支各自喺 D64 舊「加購配件合併至第一個card」邏輯附近改嘢——本分支移除咗唔再需要嘅逐件旗標 `_woolBadgeShown`/`_lightBadgeShown`（改用逐 slot 精準配對），另一分支新增咗一組獨立嘅 `_woolCellShown`/`_lightCellShown` 供「類別視圖」彙總欄使用；兩者職責唔同（逐件 badge vs 彙總 cell），保留咗雙方各自需要嘅部分。文件層面（`.fhs/memory/handoff.md`／`Changelog.md`／`session-log.md`／`deploy-log.md`）人手合併，保留兩條分支各自嘅完整記錄，無刪減。**新增 migrations 0090/0091/0092 已核實喺 Supabase live 確實存在**（`glassjar_elder_only_pricing_sku`／`glassjar_elder_sku_rename_fix`／`process_status_dialect_cleanup`），純粹係 repo 檔案追落嚟同步，唔需要重新 apply。
...
- **驗證（非純讀碼宣告完成）**：合併後 `freehandsss_dashboardV42.html` 語法檢查 PASS；用 Node 抽取合併後嘅真實 `mapOrder()` 函式碼重新餵訂單 #0600901 真實資料，輸出同合併前逐字相同（D69續III 零回歸）；grep 確認 `_fhsForceSync`／`isTrusted`／`prevDefault`（D69/D69續/D69續II 核心機制）出現次數同合併前一致（零回歸）；`/fhs-check` 4 PASS/1 SKIP；重新部署 `current.html` 三關驗證PASS（HTTP204/大小/SHA256相符）。
- **教訓**：已落 `learnings/governance.md`——`/commit` Phase 2.5 部署前，若 SessionStart hook／`/read` 顯示「近48小時有其他分支動靜」，必須先 `git log <該分支> -1 --format=%ci` 核對其部署時間戳是否比自己上一次部署更新，新則先合併再 deploy，唔可以假設自己 branch 嘅 dev 版必然係最新——`current.html` 係跨分支共享嘅部署目標，唔係本分支專屬狀態。
- **改動範圍**：`git merge` 帶入約30個檔案（V42.html/current.html/多份governance文件/3個migration檔/2個測試fixture目錄），全部經 3-way merge 或人手合併，非本session單獨新增內容。
- **Subagent 使用記錄**：❌未使用（git archaeology + 跨分支 diff 比對 + Node 真實函式碼即時交叉驗證，委派會斷推理鏈）。
...
- **重構**：主件（擺設①）同追加件（擺設②③）統一包成結構完全相同嘅 `.p-item-card`——同一底色、同一邊框、同一內部排位（標題 → 款式類型 → 底座色 → 客製化刻字 → 倒模對象「嬰兒卡」→ 加購配件）。移除早前嘅紫色左邊帶區分法。主件新增 `_pMainRefreshTitle()`，標題格式「擺設① · 款式（底座色）」同追加件對稱，掛喺 `calculatePricing()`（同徽章同步同一位置，覆蓋六條提前 return 路徑）。
- **家庭區塊 DOM 跟隨 owner 停泊（`_pFamilyDock()`/`_pFamilyDockHome()`）**：早前父母/大寶固定喺主件「倒模對象」下面，owner 為追加件時會出現「木框主件下面有父母」呢個矛盾組合（截圖二：「倒模對象不見咗」其實係走咗去主件）。修法：搬節點（非重建）跟住目前 owner 走，主件 owner 時泊喺主件卡內、追加件 owner 時泊喺該追加件卡內（加購配件標題之前）。⚠️ 兩個高風險清空點已加 guard：`fhsPExtraRemove()` 會先清空整卡欄位再 `remove()`、`resetForm()` 會對 `#pExtraContainer` 做 `innerHTML=''`——兩處皆已於動手前先 `_pFamilyDockHome()` 搬走家庭區塊，否則會靜默清空/刪除訂單層嘅家庭成員資料。
- **追加件家庭預設改 off**：自動勾父母（Q1 裁決）原本唔理主件定追加件變玻璃瓶都會觸發，Fat Mo 指出追加件多數係純嬰兒第二件，自動勾父母會令操作員逐次手動取消。修法：`fhsFamilySyncVisibility()` 自動勾判斷加 `resolvedOwner === 1` 守衛，只喺**主件**變玻璃瓶時觸發，追加件變玻璃瓶預設唔勾，操作員想要就自行手動勾選（勾咗即正常跳家庭價，未受限制）。
- **驗證**：browser live 實測——兩張卡結構程式化列舉逐項比對一致；主件標題正確跟款式/底座色即時更新；owner 搬遷/切換正常（快捷掣互換、價錢同步）；`fhsPExtraRemove()`／`resetForm()` 兩個 guard 場景（先填父母資料再刪 owner 卡、開新單）家庭區塊同 `en_parent` 皆完整倖存；自動勾限縮：主件變玻璃瓶維持自動勾（$2,580 不變），只有追加件變玻璃瓶時唔再自動勾（$4,060，非 $4,960），手動勾返仍可跳家庭價；3 張真實舊單（`06008013`/`0600900`/`0600103`）價錢與 IG 訊息零回歸；全程零 console error。
- **暫緩未做**：Fat Mo 提出「父母手下不應該鎖住大寶」——現行邏輯係一開父母即 SKU 變 `玻璃瓶套裝 (家庭)` $2,580 flat，大寶肢數完全唔影響價錢（`FHS_Pricing_Bible.md §2.1` 定案）。經 live 實測數據佐證後（無父母時大寶同嬰兒同等計肢數影響 2肢/4肢價；一開父母即歸零影響），Fat Mo 要求先自行諗清楚新定價規則再決定，本次**未落任何代碼**，唯一改動範圍係上述兩點。

**唯一改動檔案**：`Freehandsss_Dashboard/freehandsss_dashboardV42.html`（純前端 DOM 重構 + 一個守衛條件收窄，零計價公式改動、零 ID 增刪）。

詳見 [decisions.md D65續IV](.fhs/notes/decisions.md)。
...
詳見 [scripts/cl-flow-runner.js](scripts/cl-flow-runner.js) `callGemini()`、[learnings/tooling.md](.fhs/memory/learnings/tooling.md) Pitfall #8、[decisions.md D64](.fhs/notes/decisions.md)（§DEGRADED 已加後續註記）。

## [2026-08-16] Session（Claude Code / Sonnet 5 執行）— D64 續：`sync_order_to_mirror` RPC `accessory_cost` 讀寫回歸修復（migration 0088）
...
詳見 [decisions.md D64 續](.fhs/notes/decisions.md)、[FHS_System_Logic_Overview.md §5.4.16](.fhs/notes/FHS_System_Logic_Overview.md)、[learnings/supabase.md #14](.fhs/memory/learnings/supabase.md)。
...
- **範圍定案（三項拍板）**：UI 粒度＝句子切片點選為主（純 DOM，避 iOS PWA standalone 原生 selection 風險）＋desktop 原生選字浮動條加成；上線節奏＝先 observe 兩星期（`enforce` 恆 `false`），2026-08-18 檢視提案數決定開唔開 enforce；標籤範圍＝首輪只做 `deal` 成交／`quote_draft` 報價傾價／`noise` 唔關事三個。
- **`/cl-flow` A3-first 規劃**（flow_id `2026-08-04-0244`）：A3 草案含 live 資料實測（`ig_messages` 2,381 則/154 threads，79% 訊息 ≤40 字，`ig_thread_rules` 上線後**零使用**——列入 2026-08-18 檢視點嘅風險提示）。A1（Perplexity）因 API 額度用盡缺席，`state.json` 標 `degraded`。A2（Gemini）對抗評審揪出 **2 條 BLOCKER**：①原方案「phrase 必須真實出現喺來源訊息」單一防線可被繞過——外人可自行 DM 任意文字入庫觸發自己個 thread 嘅警報，再引用該訊息通過護欄，寫入全域抑制規則癱瘓看門狗（`pg_policies` 實測確認 `ig_messages`/`ig_watchdog_alerts`/`ig_thread_rules` 三表 anon SELECT 全開，anon key 內嵌喺公開 HTML）；②全域規則停用/重啟開放 anon+全表可讀＝批量停用攻擊面。Verdict `CONDITIONAL_READY`（修補方式引入二段式狀態機，超出原批准範圍），Fat Mo 直接回覆「/execute」視為批准條件已滿足。批評處理表另有 5 條 MINOR/MAJOR 逐條核實採納，1 條（PII token 格式）反證後拒絕。fresh-context agent 獨立覆核批評處理表，結論：誠實、無做戲。
- **修復（安全設計）**：新表 `ig_phrase_rules` 二段式狀態機（`proposed`/`approved`/`rejected`），anon 只能造 `proposed`；DB CHECK 約束 `ig_phrase_rules_enforce_requires_approval`（`enforce=false OR status='approved'`）喺資料庫層強制，即使上層代碼寫錯都擋得住。`fhs_admin_set_ig_phrase_rule`（批准/否決/重啟/開enforce）收歸 service_role，`GRANT`+`REVOKE FROM anon, authenticated, PUBLIC` 一次做對（沿用 `0084`→`0084b`→`0084c` 三次修補教訓）。anon 嘅停用（`fhs_disable_ig_phrase_rule`）保留單向：只可 `active=false`（fail-safe），因為 Dashboard 只有 anon key 冇 auth session。RPC 護欄 G1-G12：核心 G3（`position(lower(phrase) IN lower(content))>0`，大小寫不敏感）、長度2-40（G4）、純數字拒收（G5）、方括號拒收防PII chip殘片（G6）、抑制類≥3字（G7）、全表/單類生效規則上限200/50（G8/G9）、單一thread待審提案上限5（G11）、全表24小時提案速率上限30（G12，防「自己DM自己」灌爆）。`fhs_preview_ig_phrase`（建規則前90日命中預覽）用同一 `position()` 比對式，單一真源，唔用 PostgREST `ilike`（避免值含逗號/wildcard 破壞查詢或造成命中數虛高）。
- **V42 UI**：`_igwRenderMessageHtml()` 改造成分句渲染（`_igwSplitSentences()`，切分符 `。！？!?；;\n`，刻意不含逗號——實測79%訊息≤40字，切逗號過碎），轉義紀律不變（先分句純文字→逐句轉義→對已轉義字串做mark/chip）。手機主路徑：句子點選+多句連選（非連續選取前端擋）；desktop 加成：原生選字「吸附」被 selection range 覆蓋到嘅 `.igw-sent` 節點（唔採信 `getSelection().toString()` 原始字串，因時間戳 `<div>` 同訊息文字同層會污染選取內容）。全域規則列表預設只顯示 phrase 出現喺當前 thread 已載入訊息內嘅規則（前端過濾記憶體陣列，零額外查詢）。狀態文案區分 `🟡待審`/`🔵已批准未開生效`/`🟢生效中`/`⚪已停用`（observe 期 `applied_count` 恆為0，唔可沿用 Phase 1 嘅「未生效過」文案，否則被讀成壞咗）。
- **明確不改**：`order-match.mjs`、`build_n8n_workflow.cjs`、n8n workflow `D4LK6VrQbiXlju0V`、既有 `ig_thread_rules` 表與3支RPC——判斷邏輯零變更（`git diff` 確認空）。
- **驗證**：migration `0086` 部署後即時 `has_function_privilege()` 核實權限矩陣（anon 只可 add/disable/preview，admin/touch 兩支 service_role-only RPC anon 零權限）；DB CHECK 約束直測（`enforce=true`+`status=proposed` 正確被拒）。**分句函式**用抽出嘅實際 shipped 原始碼（非手抄）對 25 則 live `ig_messages`（含8則帶PII token）跑不變式測試：`parts.join('')===text` 全數成立、PII token 零切斷。**XSS**：`jsdom` 真實 DOM parse 6 個 payload（含訂號高亮情境），零元素被建立、`textContent` 逐字元相符。**權限矩陣**用真實 anon key curl 獨立驗證（非 Dashboard 內部 fetch）：anon 直接 INSERT/UPDATE/DELETE `ig_phrase_rules` 全部被 RLS 擋（PATCH/DELETE 回200但0行受影響，DB直查核實row未被改動）；anon 呼叫 `fhs_admin_set_ig_phrase_rule`/`fhs_touch_ig_phrase_rules` 回401；anon happy-path 用真實 alert+訊息成功建規則（`status=proposed`/`enforce=false`/`source_excerpt`正確存快照）+成功停用；G4/G6/G7/G11 護欄逐條實測拒絕（G11：頂喺5條，第6條正確拒絕）。**current.html 同步**：`grep -c` 錨點對稱 + `diff` 逐行比對輸出為空。**fixtures 回歸**：guard/health/kgov 三套共41個 fixtures 全 PASS。全部測試資料（含護欄測試用嘅DB row）已清理，SQL核實零殘留。
- **已知限制**：純標註，`enforce` 恆 false，對判斷邏輯零影響。2026-08-18 前若提案數<10，代表現行入口太深，Phase 2b 會暫停先改入口。抑制類撞 `DEAL_RE` 詞嘅衝突護欄刻意延到 Phase 2b 喺引擎端做（避免SQL複製一份`DEAL_RE`造成第二真源）。
- **Subagent 使用記錄**：✅ 使用 `general-purpose` agent 獨立覆核 `/cl-flow` 批評處理表（讀 `ag-review.md`/`cl-final-plan.md`，逐條核實反證/落點真實性，含獨立重跑 `pg_policies` live 查詢），結論全部 CONFIRMED。其餘（草案撰寫、migration執行、UI實作、驗收測試）主 session 直接處理——涉schema改動之驗收改由本 session 用獨立 curl/SQL 手法（非 Dashboard 內部路徑）交叉驗證，效果等同 fresh-context 覆核。

詳見 `.fhs/notes/decisions.md` D58、`.fhs/notes/FHS_System_Logic_Overview.md` §11.14、`.fhs/reports/completion/2026-08-04_igwatch-phrase-rules-phase2a_completion_report.md`、`artifacts/2026-08-04-0244/`（task-brief/a3-draft/ag-review/cl-final-plan）。
...
- **交付形式**：互動式 HTML 原型（Before/After 對照），經 Artifact 工具發佈，全程未觸碰 repo 任何檔案，純設計提案供 Fat Mo 審批，尚未實作落 `current.html`/`V42.html`。
- **定案設計方向**：
  - 三大類（手模擺設/鎖匙扣/頸鏈吊飾）沿用「財務結算」逐件分攤表既有配色（紫`#4A148C`/藍`#1565C0`/棕`#4E342E`），貫徹到圖示、開關、標題文字、展開箭嘴全部子元件層級。
  - 鎖匙扣嬰兒/大寶部位由「4個獨立開關各自輸入數量/上排/下排」改為「藥丸式多選部位＋共用欄位一次填」，僅約10%需要個別部位資料有出入嘅情況先展開「分開填」覆寫，減少重複輸入。
  - 預設狀態全面對齊真實 `resetForm()`/`_applyGlassDefaults()` 邏輯（訂單類型預設「是」、手模擺設預設展開木框款式嬰兒二手二腳、鎖匙扣/頸鏈吊飾預設收埋、切玻璃瓶自動開父母）。
  - 保留 V40 底部功能列（返回/刪除/審閱）、財務結算卡片位置、警告文字、成本徽章等原有頁面元素——逐一對照 `current.html:4209-4861` 核實 112 項原版控制項零遺漏。
  - 新增「否＝純金屬單」情境下提供 Google 相片上傳連結嘅提示（新提案功能，現行系統未有）。
- **資料相容性驗證**：逐行對照真實 `generate()`/`formatLimbs()`/Category A-B 訊息組字邏輯（`current.html:6522-6919`），確認鎖匙扣新版「共用填寫」介面底層仍正確映射返 `k_lh_en`/`k_lh_qty`/`k_lh_top`/`k_lh_bot` 等 canonical 欄位（有「分開填」先讀個別覆寫，冇就全部已選部位共用同一組值）；父母/大寶顏色選單補齊 `data-who`/`data-part` 屬性——確認新介面 100% 駁得上現行 IG 查閱訂單訊息 Category A/B，唔會有資料格式漏洞。
- **狀態**：純設計提案，未實作落 `current.html`/`V42.html`，待 Fat Mo 核准後排入實作 session。
- Artifact 連結（私人，Fat Mo 可自行分享）：https://claude.ai/code/artifact/39bda2af-2fcc-4fed-a19f-a39cd9482078

【交付前雙紀律自檢】
驗收：純UI設計提案，非程式碼改動——每輪迭代以 node 腳本核對 HTML tag/ID balance 確保 0 語法錯誤，非 browser 實測（因非 repo 檔案）
...
- **根因（git archaeology 確認）**：「加購」語義曾漂移——2024-09-15原始設計「同部位首件收畫圖、第2件免」→ 2026-06-02(S52) Finance Bible 落盤同一規則 → **2026-06-03(S55, commit `4dbdef2`) 修復「跨產品同部位重複收費」bug 時，實際落地代碼將豁免範圍由「線內首件」誤擴大成「主套裝已選=成條線全免（含首件）」**，超出 S52 原意 → 之後 `FHS_Product_Definition.md` 固化此漂移後語義為正式定義。詳見 `.fhs/notes/FHS_System_Logic_Overview.md` §5.4.6。
- **裁決（新三層成本模型，取代單購/加購二分）**：Layer1成本表（畫圖/物料/環扣/運費獨立欄位）、Layer2產品層（SKU全費=Layer1加總，運費視為成本一種計入總和但保留獨立欄位拆解）、Layer3訂單組合層（同部位第2件起扣減由n8n動態計算，結構化欄位非純文字備註）。SKU命名復原 `FHS_Product_Definition.md` 原有乾淨格式（刪N飾+單購/加購編碼）。
- **cl-flow 規劃**（flow_id `2026-07-24-0213`）：Step0精煉→A3草案→A1(Perplexity)+A2(Gemini)對抗評審（14條批評，採納13/拒絕1）→Verdict `CONDITIONAL_READY`。Phase 0（唯讀模擬）已執行，Phase 1-3（實際schema/n8n/Dashboard改動）待Q1(SKU是否collapse)/Q4(財務影響確認)拍板。
- **Phase 0 執行結果**（migrations 0071+0072）：唯讀RPC `fhs_simulate_new_cost_model()` 對比全庫金屬鎖匙扣/純銀頸鏈吊飾 order_items 現行 vs 新模型 subtotal——64行/30張訂單受影響，現行總額$21,555，新模型總額$27,400，差額+$5,845（全tier均為正值，符合「S55移除嘅畫圖費現重新計入」預期方向）。0600723右手：現行$500→新模型$580，同人手推導完全吻合。**執行中 finance-auditor 獨立覆核揪出並修正一個bug**：0071初版漏計吊飾類別 D42 頸鏈成本（$100/件），修正後（0072）全tier差額轉正。家庭composite SKU（1行）+已知單購qty相乘bug污染行（3行）已在函式回傳值明確標記未覆蓋，不作靜默估算。
- **description 補全**（migration 0070，Fat Mo 額外要求）：15個Supabase表全部欄位/表級 description 達100%覆蓋（原缺audit_logs/expense_logs/reply_templates/system_config等9表共60+欄位）。
- **Phase 1 執行結果**（Fat Mo 拍板Q1=collapse、Q4=接納+$5,845後執行；migrations 0073+0074）：`products`新增16個統一S/P tier SKU（`(V2)`後綴，實測部分成人P-tier既有裸格式SKU會撞名，改名避開）；`order_items`新增4個結構化欄位（position_code/drawing_waived/drawing_charged_count/cost_model_version）供Phase2使用；新建`fhs_verify_new_sku_costs()`專屬drift監測。**修正一個連鎖bug**：舊`fhs_check_product_cost_drift()`嘅孤兒row監測誤判16個新SKU做漂移，migration 0074加排除條件修正，兩監測範圍今後零重疊。**database-reviewer獨立覆核揪出Phase2阻斷性待辦**：n8n live節點「Parse Items & Generate SKU」用`.includes("鎖匙扣")`過度匹配，會污染V2 SKU尾綴，現時非活躍（前端未接線生成V2 SKU）但Phase2啟用前必須先修。
- **Phase 2 前置修復（n8n live workflow「Parse Items & Generate SKU」，V47.12→V47.13）**：修正 database-reviewer 揪出嘅過度匹配——鎖匙扣/吊飾正規化區塊加 `isV2Sku` guard，SKU已係完整`(V2)`格式即跳過舊式後綴/扣減邏輯。經 `update_node_code` dry-run預覽確認純加guard零其他改動後正式套用，自動備份可一鍵回滾。
- **Live webhook 對抗測試驗證**（`trigger_test_execution` MCP工具打嘅`/run` management API同webhook-triggered workflow不兼容，改用curl直打真實webhook URL）：構造刁鑽測試單`FHS-TEST-V2-0724`（V2 SKU **同時**帶`Mode:"(加購)"`，最容易觸發舊bug嘅組合），execution 5064 success，Supabase實際寫入`product_sku`完全未被追加後綴污染，`item_base_cost=subtotal_cost=$205`同products表新SKU定價吻合，順便驗證埋成本查表全鏈路對V2 SKU已可運作。測試訂單已清理（兩表各刪1行，刪後歸零確認）。
- **Phase 2 核心bug修復（n8n live workflow「Calculate Profit & Pack Items」，V47.20→V47.21）**：qty=1測試PASS掩蓋咗更嚴重問題——**qty=4重測**揪出n8n全程冇任何地方做`total_base_cost×quantity`（$205應為$820）。舊SKU得能行得通因為「幾多飾」焗喺SKU字串本身（`item_per_set`=N），但V2 SKU設計係「單件價」需n8n動態×quantity，Phase1漏咗呢步。**連帶第二個bug**：舊code對所有吊飾類別無條件加$100頸鏈費，V2吊飾SKU本身已baked呢個成本，會雙重計算。**修正**：`isV2Sku` guard令V2 SKU正確×quantity、V2吊飾唔再重複加頸鏈費；現有運費/共用折扣扣減機制（純按件數，不分V2/舊）通用不變唔使重寫。
- **Live webhook 驗證結果**：V2鎖匙扣qty=4重測後`$820`✅同Phase0 gross預期吻合；V2吊飾qty=2測試`necklace_cost=$1185`✅同Phase0設計吻合（1320−35運費−100共用折扣）。
- **舊SKU regression 缺口補完（真正根因：測試資料太長，非infra非代碼問題）**：連續4次嘗試皆被 `get_execution_log` MCP工具顯示"running"卡住，最初誤判為已知NAS/axios網絡層問題。**Fat Mo 用瀏覽器直開n8n UI 揭穿真相**：execution實際958ms內完成並Error（非卡住），MCP工具嘅"running"狀態係失準過時；真正錯誤係 `HTTP: Supabase Sync RPC` 收到 Postgres `22001: value too long for type character varying(20)`——我啲測試訂單ID（如`FHS-TEST-OLD-REGRESS3`，22字元）超過`orders.order_id`嘅`varchar(20)`上限，同鎖匙扣邏輯完全無關，純屬命名巧合（V2測試系列剛好17字元冇撞上限）。改用合規短ID重試，**結果完美**：`item_base_cost=subtotal_cost=$500.00`同0600723已知值一致，`keychain_cost=$440`扣減正確。舊SKU路徑經live execution直接確認完全未受影響，缺口正式補完。**額外教訓**：`get_execution_log` MCP工具對已完成execution可能持續回報過時"running"狀態，懷疑卡單時應改用瀏覽器直查n8n UI核實，唔應該單憑MCP工具狀態判斷因果關係。
- **Phase 2 核心功能落實：同部位第2件起免畫圖**（n8n「Calculate Profit & Pack Items」V47.21→V47.22 + 「Supabase Mirror Prep」+ RPC `sync_order_to_mirror()` migration 0075）：由 `Order_Item_Key` 後綴推導 `position_code`，同部位**跨品類**（鎖匙扣+吊飾共享豁免，同前端`chargedPositions`精神一致）分組，組內第1件收畫圖費其餘豁免，鏡像現有運費/頸鏈扣減寫法。首次寫入Phase1建好但一直NULL嘅`position_code`/`drawing_waived`/`drawing_charged_count`/`cost_model_version`四個結構化欄位。**Live對抗測試**（右手鎖匙扣qty4+右手吊飾qty1+左腳鎖匙扣qty2跨品類同部位單）結果完全吻合人手推導：右手吊飾正確識別為「同部位第2件」豁免（`drawing_charged_count=0`），訂單層`total_cost=$1,490`/`keychain_cost=$890`/`necklace_cost=$600`，`n8n_adjustment_notes`記錄`-$300`扣減並附逐行detail。再次舊SKU regression確認`$500`不變、新欄位皆NULL。已知非阻斷小瑕疵：收斂律審計note對V2單顯示誤導性差異提示（純cosmetic，`amount=0`不影響實際數字，留待日後處理）。至此**Phase 2四項全部完成**：SKU過度匹配修復+qty乘法修復+頸鏈雙重計算修復+同部位畫圖動態扣減。
- **Phase 3 完成：Dashboard 前端切換到 V2 SKU 生成，已升格部署`current.html`**（`freehandsss_dashboardV42.html`+`Freehandsss_dashboard_current.html`同步改動）：範圍收窄至嬰兒(baby)tier（大寶/家庭/成人tier因Phase1目錄未覆蓋而暫不改動，避免查唔到成本）。改4個區塊（真實提交+前端估價各2個：鎖匙扣+吊飾），`finalObj`由「只有冇主套裝先tag(P)」改成「兩邊都明確tag(S)/(P)」+SKU尾加`(V2)`後綴。**Fat Mo要求升級為真實browser UI端對端測試**（非隔離邏輯測試）：攔截`window.fetch`後用真實DOM click觸發checkbox+輸入qty，直接call真實`syncToAirtable()`（非mock），攔截到嘅真實payload正確含`"Product_Name":"嬰兒(S)鎖匙扣 - 不銹鋼 (V2)"`。過程中發現form自動load咗真實歷史單0600702（非空白新單），核實Supabase該單`customer_name`/`updated_at`完全未變，確認fetch攔截生效、零數據污染。測試PASS後Fat Mo「可以」授權，經`.fhs/.deploy-ok`旗標逐次重建升格部署至`current.html`（本機；NAS推送由本次`/commit` Phase 2.5處理）。
- **追加：`buildAuditLedgerHtml()` 財務彈窗（訂單詳情→財務分頁）配合V2成本模型修復**（Fat Mo追問「加回畫圖成本，成本拆扣由n8n成本快照鏈顯示」）：根因兩層——①品項fetch query漏咗Phase1/2新增嘅`position_code/drawing_waived/drawing_charged_count/cost_model_version`四欄，modal完全冇資料可用；②V2 SKU畫圖費baked入`item_base_cost`，n8n無獨立寫`drawing_cost`。修復：SELECT補齊四欄+per-item明細用`drawing_charged_count×cost_configurations`畫圖率（同n8n方程式）反推顯示（豁免件顯示綠色「$0（同部位已收，本件豁免）」）+ section②`drawing_position_dedup_deduction`跨品類合併筆記改用`detail[].item_key`拆分歸位（原本會被首個命中嘅品類全數吞晒對方嗰份扣減）。**Live webhook測試單驗證**（右手鎖匙扣qty4+右手吊飾qty1+左腳鎖匙扣qty2）：section②「鎖匙扣成本(−$100)(−$240)=$890」「吊飾成本(−$60)=$600」逐項核對吻合，品項明細畫圖費正確顯示；舊模型單（0600723）交叉核對無回歸。測試單已刪除，已同步V42.html+current.html。詳見 `FHS_System_Logic_Overview.md` §5.4.6 追加條目。
- **追加：歷史舊模型訂單畫圖成本回填**（migration 0076，Fat Mo 明確授權「全部都要，不論已完成/進行中」——因 0600106 舊單被發現仍顯示錯誤成本，追查確認 Phase0-3 只修復未來新單，冇回填歷史單）：範圍精準界定為只修正「畫圖成本」呢一個分量本身（S55語義漂移根因），唔重新套用V2統一SKU嘅單件全費（避免混入不相關嘅material/運費假設改動）。方程式同V47.22現行邏輯一致：同部位（跨鎖匙扣/吊飾）首件收tier_drawing費（baby S=$60/P=$110），其餘豁免。執行前逐行核實cross-category同部位分組費率永遠一致（S/P唔會混淆）、揪出3行「單購」drawing_cost×quantity舊bug（回填後金額下降屬修正非新錯誤）、家庭composite SKU（`_FAM_COMBO`）已排除另案處理、全部受影響訂單`cost_override_locked`皆非true。**結果**：23張訂單、`order_items`+`orders`同步更新，total_cost淨增加$2,000（21張上升，2張因qty相乘bug修正下降各$220），每張單寫入`audit_logs`供財務彈窗「本單成本變更歷史」查閱，回填後全庫重掃remaining_delta=0完全收斂。**finance-auditor獨立覆核PASS**：獨立重算5張抽查訂單逐部位分組、23張audit_logs逐行核對、全庫重掃冇漏改冇誤傷。**附帶發現**（非本次造成，屬既有schema缺口）：0600107/0600723 total_cost比三分類加總多$30，因「配件」類（燈飾等）成本從未歸入分類欄位，已開獨立task卡另案追蹤。
- **追加：畫圖扣減補顯示喺成本快照鏈**（migration 0077，Fat Mo檢視0600723生產頁面反饋——數值啱但扣減資訊收埋喺品項明細，想同頸鏈/運費扣減一樣顯示喺②section badge）：0076只改數值冇寫`n8n_adjustment_notes`，令歷史單冇badge。修復：APPEND一張`drawing_position_dedup_deduction`筆記入23張回填訂單（保留原有筆記），格式同V2 live訂單一致。純資料變更，代碼今早已部署，即時生效。Browser驗證0600723正確顯示「鎖匙扣成本(−$140)(−$360)=$980」。
- **仍待進行**：4份規則文件P0同步（Finance Bible/finance-gatekeeper§三B/Cost_Schema_v2/Quadruple_Sync_Field_Map）、家庭composite SKU（Q3）、大寶/家庭/成人tier擴充、「配件」類成本分類欄位缺口（列為下個session/獨立task待辦）。

【交付前雙紀律自檢】
驗收：Phase 0模擬邏輯經finance-auditor獨立覆核；Phase 1新SKU經database-reviewer獨立覆核；Phase2全部四項經live webhook測試全部PASS（含跨品類同部位對抗測試+3輪舊SKU regression，非mock，含Fat Mo親自截圖n8n UI協助排查工具失準問題）；Phase3經真實browser UI端對端測試PASS（fetch攔截+真實DOM click+真實`syncToAirtable()`，非console模擬）；0600723右手先例交叉驗證吻合；description補全經smoke test驗證15表100%覆蓋；追加嘅`buildAuditLedgerHtml()`修復經live webhook測試單+browser DOM文字擷取驗證PASS、舊模型單0600723交叉核對無回歸；歷史回填migration 0076經finance-auditor獨立重算全量PASS
Subagent：✅ Opus subagent 獨立審計94個候選文件揪出4份P0衝突文件；✅ finance-auditor 三輪獨立覆核（Phase0模擬、及歷史回填migration 0076）；✅ database-reviewer獨立覆核Phase1新SKU（揪出Phase2阻斷項）；cl-flow內建A1(Perplexity)+A2(Gemini)對抗評審；Phase2實作+測試全程互動式n8n MCP+Supabase MCP直接操作；`buildAuditLedgerHtml()`追加修復未額外派subagent（互動式grep排查+live測試單自驗）
...
- **修復1（P mode 鎖匙扣改多部位合併計價）**：原邏輯每個身體部位（左手/右手/左腳/右腳）各自獨立查階梯表。客戶售價表證實應為總量合併計價——例：左手1+右手1，舊邏輯 $1,580×2=$3,160，客戶售價表合併總量 qty=2 查表=$2,160，差 $1,000。新增跨部位附加費：第3/4個「全新不同部位」各 +$100（`surcharge = max(0, distinctParts−2)×$100`）。S mode 及家庭連心分支不受影響。
- **修復2（S mode 純銀頸鏈吊飾 remainder 單隻價下修）**：原公式 `fullNecklaces×2980 + singleCharms×1980`。用真實加購單反推（一對金手+一對銀腳=2對=$2,980×2=$5,960，同發票一致）確認完整頸鏈價不變，但 remainder 單隻價（配對外多出嗰隻）由 $1,980 降至 $1,490（= $2,980 之半，對齊 P mode 既有 `extraSingle=extraFull÷2` 結構）。N=1（訂單總共淨一隻，未成對）維持 $1,980 底價特例。P mode 純銀吊飾公式本身已符合此結構，無需改動。
- **驗證**：Browser pane 實測多組案例——P mode 左手1+右手1(2部位)=$2,160；+左腳1(3部位)=$3,040($2,940+$100跨部位附加費，UI正確標註)；S mode 同場景不受影響(各自$860獨立顯示)。純銀吊飾 N=1→$1,980；N=3→$4,470($2,980+$1,490)；N=4→$5,960(=2×$2,980，同真實發票0600702一致)。Console 全程零錯誤。
- **文件同步**：`.fhs/ai/FHS_Pricing_Bible.md` §3.2/§4.1/§4.3 同步改版至 v1.6.0；decisions.md D45 已記錄完整背景/驗證數字。
- **未同步**：`current.html` 因 pre-tool-guard 保護未直接改動，需走本次 `/commit` Phase 2.5 升格部署鏈同步。

【交付前雙紀律自檢】
驗收：Browser pane 多組定價案例逐一實測（非純自驗文字記錄，含真實客戶加購單交叉核對）+ console 零錯誤
Subagent：❌ 未使用（全程互動式定價方案討論+改碼，含多輪 Fat Mo 拷問修正方向）
...
- **根因1（雙重換行）**：`pricingLogicDetails` 用 `logs.join('<br>')` 組 HTML，但分類標頭 `_catHdr()` 本身輸出 `<div>`（block 元素自帶換行），join 又喺標頭後夾多一個 `<br>`，兩者疊加變成標頭同首件之間多一行空白；同一邏輯下，理應留空白嘅「分類與分類之間」反而只得單一 `<br>`（因為前一行係 `<span>` 非標頭）。
- **修復1**：捨棄一律 `join('<br>')`，改自訂迴圈按「前一項是否為分類標頭」動態決定間距——標頭後緊接下一項時唔加 `<br>`（貼實），非標頭轉標頭（新分類開始）時加 `<br><br>`（留一行空白），其餘一律單 `<br>`。
- **根因2（粗體範圍）**：`<b>吊飾</b> ${totalCharms}個 / ${necklaces}條頸鏈: $${silverPrice}` 只包住「吊飾」兩字，其他行格式係 `<b>${name}</b>: $${price}`（整個品項描述都粗體），呢一行漏咗將「${totalCharms}個 / ${necklaces}條頸鏈」一併收入 `<b>`。
- **修復2**：改為 `<b>吊飾 ${totalCharms}個 / ${necklaces}條頸鏈</b>: $${silverPrice}`，粗體範圍同其他行一致（品項描述全粗體，金額不粗體）。
- **驗證**：Browser pane 重現同一情境（木框4肢+4個鎖匙扣+4個吊飾/2條頸鏈），量測 `getBoundingClientRect()` 逐行 top/bottom 座標確認：標頭→首件間距~5px（貼實）、分類間距~26-27px（單行空白）；`querySelectorAll('b')` 確認 6 個粗體元素 `fontWeight` 全 700 且範圍一致。
- **部署**：`/upload-web` 三關驗證 PASS（HTTP 200 / 大小相符 1,045,018 bytes / SHA256 一致）。

【交付前雙紀律自檢】
驗收：純顯示層排版改動 — Browser pane 驅動真實表單邏輯 + getBoundingClientRect 逐行座標量測（非螢幕截圖，非主觀目測）+ NAS 三關驗證 PASS
Subagent：❌ 未使用（單純 UI 排版修改）
教訓：（無新教訓，`join('<br>')` 混用 block 元素與顯式 `<br>` 造成雙重換行為常見 HTML 拼接陷阱，日後同類 log 拼接注意）

## [2026-07-22] Session 187續IX（Claude Code / Sonnet 5 執行）— 財務結算報價明細按分類配色

- **緣起**：Fat Mo 手機截圖標紅箭頭，指出「系統精算建議報價」明細 `(2380+860+860+860+860+5960)` 同下方逐項明細（`木框套裝(4肢): $2380`／`嬰兒鎖匙扣...x1: $860`／`吊飾 4個/2條頸鏈: $5960`）全部同一色，睇唔出邊個數字屬邊個分類；要求跟各分類 badge（① 手模擺設+配件 橙／② 鎖匙扣 灰黑／③ 頸鏈吊飾 紫）上色，紅箭頭示意 badge 顏色應向下延伸到對應品項文字。
- **色值取用**：直接複用 `calculatePricing()` 內已存在嘅 `_catHdr()` 分類標頭色系（非另開新色），確保同畫面下方「訂單總覽」等處色彩語言一致：手模擺設+配件 `#E65100`（橙）、鎖匙扣 `#37474F`（灰黑）、頸鏈吊飾 `#4527A0`（紫）。
- **修復範圍**（`current.html`+`V42.html` 同步）：① `breakdownParts` 陣列由純數字改 `{v,c}` 物件，5 個 push 點（主品項/混合附加費/配件/鎖匙扣/純銀吊飾）各自標色；`priceBreakdown` 渲染改 `innerHTML` 逐數字包 `<span>` 上色，`+` 號用中性灰 `#90A4AE`。② `pricingLogicDetails` 明細區 4 處 `logs.push()`（主品項行、配件行、鎖匙扣 `processTierPricing()` 行、頸鏈吊飾彙總行）同步包色，`processTierPricing()` 按 `productType==='純銀'?紫:灰` 動態判色以保持通用性。
- **驗證**：Browser pane 用 `switchMode('create')` 驅動真實表單重現截圖情境（木框套裝4肢+4個嬰兒鎖匙扣+4個吊飾/2條頸鏈），非手動塞值，讀 `innerHTML` 逐一核對每個數字/文字行色碼同截圖分類完全吻合（`$11780 = 2380橙+860灰×4+5960紫`）。
- **部署**：`/upload-web` 三關驗證 PASS（HTTP 200 / 大小相符 1,044,063 bytes / SHA256 一致）。

【交付前雙紀律自檢】
驗收：純顯示層配色改動 — Browser pane 驅動真實表單邏輯 + innerHTML 逐色核對（非螢幕截圖，非手動塞值）+ NAS 三關驗證 PASS
Subagent：❌ 未使用（單純 UI 配色修改）
教訓：（無新教訓，沿用既有分類色系 SSoT `_catHdr()`，避免另開一套新色造成日後不一致）
...
- **修復**：移除 `flags.push({..., msg: '💰 成本扣減說明：'+n8nNotes...})` 呢一行；`n8nNotes`/`n8nBasis` 變數喺其他地方（如手動調整金額 row 嘅 tooltip）仍有用途，冇動。
- **效果**：Akira 訂單摘要卡而家顯示乾淨嘅「✓核對通過 無偏差」，唔會再有重複文字塊；有真正偏差（如確收鏈唔平衡/利潤驗算不符）時仍會如常顯示紅字/黃字警示。
- **驗證**：本機起server + `openOrderModal(id,'B','finance')` 開Akira真實財務tab截圖核實。

【交付前雙紀律自檢】
驗收：純顯示層改動（移除一行重複提示，零財務數值改動）— 本機截圖核實
Subagent：❌ 未使用
教訓：（無新教訓，UI去重複調整）
