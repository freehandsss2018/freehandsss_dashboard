...
## [2026-08-04] Session（Claude Code / Sonnet 5 執行）— D59：IG看門狗 P2c `message_intents` 冪等鍵修復

- **緣起**：Fat Mo 交辦稽核 live 資料異常——`message_intents` 表 205 distinct 訊息 `is_primary` 100% 為 true，同 migration 0057 註記「tagIntent() 命中陣列第一個」表面矛盾，懷疑 writer 寫死 `is_primary:true`。
- **稽核結論：原懷疑排除**。`build_n8n_workflow.cjs:471-480` 的 `hits.forEach((label, idx) => is_primary: idx===0)` 邏輯正確；live 查詢證實 205 筆 distinct 訊息中零筆命中 2 個以上不同 intent_label，`is_primary` 全 true 只係反映現況資料，非 bug。
- **連帶發現並修復真實 bug**：唯一一組重複列（thread `woshilili_956096984158193` / `1x3u01b0yy7`，`complaint`，2026-07-13 與 2026-07-25 各一筆）根因係原冪等鍵 `ix_message_intents_dedup`（migration 0057）含 `alert_date`（寫入當日非訊息本身日期），若 n8n `staticData` 曾遺失（thread cursor 歸零，推測工作流曾整個重建而非原地更新），同一歷史訊息喺不同 `alert_date` 重跑會繞過去重。
- **修復**：migration `0087_fix_message_intents_dedup_key.sql`——清除既有重複列（206→205）+ 冪等鍵改為訊息自然鍵 `(message_thread, message_ig_message_id, intent_label)`，不含 `alert_date`；`build_n8n_workflow.cjs` 的 `Write Intents` 節點 `on_conflict` 參數同步，並以既定「PUT `{name,nodes,connections,settings}`」外科手術式部署上 n8n workflow `FHS_IGWatchdog_DriveWatch`（`D4LK6VrQbiXlju0V`）。
- **明確不改**：`tagIntent()`/`INTENT_PATTERNS`/`order-match.mjs` 判斷邏輯零改動；`alert_date` 欄位本身保留（PART 5 TTL job 比對 `created_at`，不受影響）。
- **驗證**：單元測試 77/77 PASS（含 `order-match.diffguard.test.mjs`）；Supabase 直查 `pg_indexes` 核實新索引已生效且不含 `alert_date`，`message_intents` 總筆數核實 206→205；n8n PUT 回 HTTP 200，GET 回應體核實 `Write Intents` 節點 `url` 已含新 `on_conflict`，8 個 Google Drive 節點 + 2 個 Telegram 節點 `credentials` 全數保留（`build_n8n_workflow.cjs` 本已內嵌固定 credential id，PUT 未洗掉），workflow 仍 `active:true`。
- **Subagent 使用記錄**：未使用 subagent，全程主 session 直接執行（Supabase MCP 查詢 + 直接部署），驗收改用直接 SQL/curl 交叉核實運行結果，效果等同 fresh-context 覆核。

詳見 `.fhs/notes/decisions.md` D59、[supabase/migrations/0087_fix_message_intents_dedup_key.sql](supabase/migrations/0087_fix_message_intents_dedup_key.sql)。
