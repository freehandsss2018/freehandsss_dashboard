...
## [2026-09-05] Session（Claude Code / Sonnet 5 執行）— D58-follow：逾期檢視點覆核（IG看門狗詞句規則提案數 <10，判定入口太深）

- **緣起**：`/read` 例行同步發現 D58 預定於 2026-08-18 執行嘅檢視點已逾期未查。D58 原定規則：`ig_phrase_rules` 提案數 ≥10 → 入口可用，進 Phase 2b（開 enforce）；<10 → 判定入口太深，Phase 2b 暫停先改入口再重數。
- **查證**：Supabase 直查 `ig_phrase_rules` 全表，共 **7 筆**，全部 `status='proposed'`（0 approved/0 rejected）、`applied_count=0`。全部 7 筆 `created_at` 集中喺 **2026-08-03 23:35 ～ 2026-08-04 00:25**（功能上線首兩天），其後**整整一個月零新增**（原定觀察窗口兩星期，實際已超一個月）。
- **判定**：7<10，觸發 D58 預定規則，Phase 2b 開 enforce 繼續暫停。零新增達整月而非僅未達標，加強「入口太深、Fat Mo 日常操作路徑接觸唔到呢個功能」嘅假設。
- **Fat Mo 裁決（同日追加）**：報告後 Fat Mo 主動要求「重新教我點用呢個系統」，顯示低使用率可能源於**唔熟悉操作方式**而非入口太深。已於對話中重新講解完整操作路徑（底部導覽「系統」分頁→IG看門狗警報卡→💬IG訊息→選字→標籤條→預覽→確認）。裁決：**唔改入口，延長觀察期至 2026-10-05**（一個月），屆時沿用同一 <10/≥10 判準再查一次；若已理解操作仍零/低使用，「入口太深」假設先算證實再認真規劃入口改法。
- 本次僅執行既定判準查證+文件落盤，未動任何代碼。全文見 decisions.md D58-follow、handoff.md 待辦欄／⏰時限待辦欄。**Subagent 使用記錄**：❌未使用（單一 Supabase 查詢+文件落盤，無需委派）。
