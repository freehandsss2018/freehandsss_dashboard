...
2. 讀取 `/.fhs/memory/handoff.md` **前 13 行**（`limit: 13`＝便攜塊；禁全檔 Read）— 同步上次 session 狀態與待辦；待辦明細 Grep MASTER 表後窗口讀
...
| n8n Workflow | → 見 `handoff.md` 便攜塊（單一真源，避免版本號重複維護漂移）|
| Airtable Base | app9GuLsW9frN4xaT |
| 數據源 | **Supabase (Primary Lead) + Airtable (Fallback Backup)** |
| database-reviewer / finance-auditor 版本 | → 見各檔 frontmatter `version:` 欄（單一真源）|
...
| `/ag-flow` ⚠️DEPRECATED（S134，改用 `/cl-flow`） | `.fhs/ai/commands/ag-flow.md` | `.claude/commands/ag-flow.md` | `.agents/workflows/ag-flow.md` |
...
| `/ag-plan` ⚠️DEPRECATED（S134，Antigravity 降為永久備援） | `.fhs/ai/commands/ag-plan.md` | — | `.agents/workflows/ag-plan.md` |
| `/ag-stitch-sync` ⚠️DEPRECATED（S134） | `.fhs/ai/commands/ag-stitch-sync.md` | — | `.agents/workflows/ag-stitch-sync.md` |
| `/ag-ui-import` ⚠️DEPRECATED（S134） | `.fhs/ai/commands/ag-ui-import.md` | — | `.agents/workflows/ag-ui-import.md` |
...
| 資源 | 位置 | 用途 | 誰用 |
|------|------|------|------|
| FHS Subagents（9個） | `.fhs/ai/subagents/freehandsss/` → `~/.claude/agents/freehandsss/` | ui-designer, frontend-developer, code-reviewer, database-reviewer, finance-auditor, tdd-guide, build-error-resolver, blender-3d-modeler, product-integration-validator | Claude Code 專用 |
