...
> 前二項（前端雙計、Audit Ledger badge）✅ 已修復並部署；第三項（n8n V47.20 記帳格式對齊）已通過 dry-run 驗證，待 Fat Mo `/execute` 確認後正式部署。見 `decisions.md` D42、`Changelog.md` S187 系列條目。
...
- **範圍**：只影響**新落單**及**未來人手 resync** 之單；現存 8 張含吊飾訂單／16 行品項（舊格式：每件 $465 對稱 + 單一 `+$200` note）**不強制 backfill**，Fat Mo 明確裁決維持原狀，需要時可個別 resync。
- **驗證**：Node harness 用 Akira 真實 4 吊飾+4鎖匙扣+1手模情境重算，`totalBaseCost=2605`／`necklaceCostTotal=1955`／`keychainCostTotal=440`／`handmodelCostTotal=210` 與現行完全一致；n8n `update_node_code` dry-run 通過（新舊碼長度差 +1,365 字元，全屬新增註解/邏輯，無語法錯誤）。
