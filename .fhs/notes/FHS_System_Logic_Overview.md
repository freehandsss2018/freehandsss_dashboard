...
### 5.4.1 加購鎖匙扣「N飾」數量漏算 — 雙層根因（Session 124 cl-flow 診斷，flow 2026-06-26-0922，待 /execute）

> ⚠️ live 坐實，**規劃完成待執行**；執行前見 `artifacts/2026-06-26-0922/cl-final-plan.md`。
...
- Phase 2（未排程）：`fhs_adjust_order_cost` RPC 寫 log_type=`order_cost_adjust`。
...
>
> ② 成本快照鏈 v2 架構（Session 103 修復；Session 124 加可展開明細）：
...
> - **per-item 可展開明細（S124，點2 降級版）**：每筆品項成本小計改用原生 `<details>`，展開只列**真實存在欄位**（`item_base_cost`、`quantity`、及 >0 的 `drawing/printing/chain/shipping_cost`）；四欄全空時顯示「明細未記錄（n8n 未寫入）」，**禁止前端用 cost_configurations 自行重算拆解**（違反成本單一真源）
...
> - **四欄禁用（訂單層主結構不讀）**：`drawing/printing/chain/shipping_cost`（91% 空，Task A 未完成）→ 訂單層成本不讀取，僅 per-item 展開內按需顯示 >0 者
>
> ⚠️ **已知資料 bug（S124 live 核實，待 Task A/獨立 cl-flow 修）**：加購鎖匙扣（如「嬰兒鎖匙扣-不銹鋼-2飾(加購)」）的 `order_items.subtotal_cost` 與訂單層 `keychain_cost` **未按 quantity 累加**——qty=2/3/4 多數仍只記首件全成本 $185（正解應為 首件185 + 加購件×(N−1)×$125）。根因在 n8n 成本計算，非前端；前端僅以 `fhsAudit_qtyWarn` 誠實揭露，不回寫。範例訂單：0600905 / 0600908（qty=2 記 185，應 ≈ 310）。
>
