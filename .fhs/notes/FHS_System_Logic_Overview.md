...
- **Phase 3（部分）：Dashboard 前端切換到 V2 SKU 生成（`freehandsss_dashboardV42.html`，dev版，未部署 `current.html`）**：
  ...
  - **尚未做**：4份規則文件P0同步（Finance Bible/finance-gatekeeper/Cost_Schema_v2/Quadruple_Sync_Field_Map）、家庭composite SKU（Q3）、大寶/家庭/成人tier嘅V2目錄擴充+對應Dashboard區塊改動。`buildAuditLedgerHtml()`財務彈窗顯示已於同日追加修復，見下方獨立條目。
  - **追加修復（同日，Fat Mo 追問「財務核對帳單介面配合返最新成本運算模式」）**：`buildAuditLedgerHtml()`（訂單詳情→財務分頁）原本完全睇唔到V2品項嘅畫圖成本，根因有兩層——(a) 品項層 fetch query（`buildAuditLedgerHtml` 呼叫前的 order_items SELECT，約line 11085-11088）漏咗 Phase1/Phase2 新增嘅 `position_code/drawing_waived/drawing_charged_count/cost_model_version` 四欄，令modal層完全冇資料可用；(b) 即使有資料，V2 SKU嘅畫圖費本身baked入`item_base_cost`，n8n無獨立寫`order_items.drawing_cost`（同舊模型唔同源）。修復：①SELECT補齊四欄；②per-item明細對`cost_model_version='v2_layered'`品項，用`drawing_charged_count × cost_configurations`畫圖原子費率（同n8n `getDrawingRateForV2Sku()`同一方程式，讀`window._fhsCostConfig`單一源）反推顯示，豁免件顯示綠色「$0（同部位已收，本件豁免）」而非消失；③section②「成本快照鏈」入面`drawing_position_dedup_deduction`筆記原本係鎖匙扣+吊飾合併單一筆，desc同時含兩個品類關鍵字會被keyword比對嘅首個`_costRow`全數吞晒，令另一品類睇唔到自己嗰份扣減——改用筆記`detail[].item_key`嘅`_K_/_M_`段拆返兩張獨立筆記各自歸位。**Live webhook測試單驗證**（右手鎖匙扣qty4+右手吊飾qty1+左腳鎖匙扣qty2，同Phase2用過嗰個跨品類同部位組合）：section②正確顯示「鎖匙扣成本 (−$100)(−$240) = $890」「吊飾成本 (−$60) = $600」，逐項金額同Supabase `n8n_adjustment_notes.detail`逐行核對完全吻合；品項明細正確顯示右手鎖匙扣「繪圖 $60」、左腳鎖匙扣「繪圖 $60」、右手吊飾「繪圖 $0（同部位已收，本件豁免）」。舊模型單（0600723）交叉核對顯示完全不變（無回歸）。測試單已刪除。改動已同步V42.html+current.html，**NAS部署待Fat Mo確認**。
...
- **歷史訂單回填（同日追加，migration 0076，Fat Mo 明確授權「全部都要，不論已完成/進行中」）**：Fat Mo 追問 0600106 舊單仍顯示錯誤成本，追查確認 S189 之前 Phase0-3 只修復咗「未來新單」（V2 SKU），冇回填歷史舊模型（`cost_model_version IS NULL`）訂單——同 0600723 最初被質疑嗰個問題本質一樣，一直未修正。範圍界定：只修正「畫圖成本」呢一個分量本身（S55 語義漂移根因），唔重新套用 V2 統一SKU嘅單件全費（避免混入不相關嘅material/運費假設改動）。方程式同 V47.22 現行邏輯一致：同部位（跨鎖匙扣/吊飾，由item_key尾綴_LH/RH/LF/RF判斷）首件收tier_drawing費（baby S=$60/P=$110），其餘豁免。**執行前逐行核實**：全部cross-category同部位分組費率（S/P tier）永遠一致（冇跨tier混淆風險）；揪出3行「單購」drawing_cost×quantity相乘舊bug（0600803_K_LH/07001007兩行），回填後金額下降屬修正非新錯誤；家庭composite SKU（`_FAM_COMBO`後綴）已排除，D41獨立方程式另案；全部受影響訂單`cost_override_locked`皆非true。**結果**：23張訂單、`order_items`+`orders`同步更新，總成本淨增加$2,000（21張上升$60-$240，2張因qty相乘bug修正下降各$220），每張單寫入`audit_logs`（`log_type='order_cost_adjust'`, `source='migration_0076'`）供`buildAuditLedgerHtml()`「本單成本變更歷史」查閱。回填後重新掃描全庫，同規則模式匹配嘅品項行 remaining_delta=0（完全收斂，冇漏網）。**驗收**：finance-auditor獨立覆核 **PASS**（獨立重算5張抽查訂單逐部位分組、23張audit_logs逐行核對、全庫重掃冇漏改冇誤傷）。**附帶發現**（非本次回填造成）：0600107/0600723 total_cost 比三分類加總多$30，原因係「配件」類（燈飾等）成本從未歸入`handmodel_cost`/`keychain_cost`/`necklace_cost`任何一欄，屬既有schema缺口，已開獨立task卡（`task_2d8d6b4f`）另案追蹤，不影響本次回填正確性。
- **同日再追加（migration 0077）**：Fat Mo檢視0600723生產頁面後指出——畫圖成本數值啱（$560=$500+$60），但扣減資訊淨係收埋喺品項明細展開先見到，佢想同「頸鏈共用折扣」/「運費共享扣減」一樣直接顯示喺②成本快照鏈嘅badge（同一類別嘅訂單層動態扣減）。根因：migration 0076只改咗數值本身，冇寫入`orders.n8n_adjustment_notes`（V2/live訂單嘅badge正正讀呢個欄位）。修復：為0076涉及嘅同一批23張訂單APPEND（非覆蓋，用`||`保留原有運費/頸鏈筆記）一張`drawing_position_dedup_deduction`筆記，格式/type/detail結構同V2 live訂單完全一致（`position_code`/`item_key`/`waived_units`/`drawing_rate`/`deduction`），令現有`_noteBadges()`+今日稍早`_K_`/`_M_`拆分邏輯原樣適用。**金額口徑注意**：呢個badge amount（如0600723=-$360）係Phase0模擬式「如果每件都收畫圖費、因共用部位而豁免咗幾多」語意，同0076 backfill delta（0600723=+$120，「實際補回幾多」）係兩個唔同概念嘅數字，刻意唔一致——前者係UI badge固定語意（同V2單一致），後者係migration審計記錄，兩者並存不衝突。**驗證**：browser直查0600723 dev preview，section②正確顯示「鎖匙扣成本 (−$140)(−$360)=$980」，純資料變更（Supabase `n8n_adjustment_notes`），代碼今早已部署落current.html，即時生效毋須重新部署。
- **同日四度追加**：Fat Mo反饋品項細明數額要「純成本計算，不涉及任何折扣」——原本額外顯示「單件基礎成本」（=item_base_cost）同下面printing/chain/drawing三行重複（已用真實資料核實printing_cost+chain_cost+drawing_cost=item_base_cost，三者已完整組成單件成本），令人誤以為要再加一次先等於成本小計；運費(shipping_cost)係前端假設「呢件係唯一件」嘅估算值非真實組成分量，且容易同訂單層運費共享折扣混淆。修復：移除「單件基礎成本」重複行+運費行，V2統一SKU（冇分開儲存printing/chain）改用`base2-draw2`反推「物料成本」，令breakdown可以純加總得返成本小計。Browser驗證0600723：繪圖$60+打印/物料$460+環扣/頸鏈$40=成本小計$560，純加總無折扣。已同步V42.html+current.html並NAS部署三關驗證PASS。
- **同日六度追加（Fat Mo 第三度糾正，核心規則確立）**：Fat Mo 指出品項細明必須顯示「全額」（quantity×畫圖費率，不理會同部位豁免），折扣只應該喺訂單層②快照鏈用badge顯示——同V2/live訂單現行架構完全一致（已用真實V2測試單核實：`item_base_cost` 恆為 quantity×單件全費，唔理該件實際有冇被同部位另一件豁免）。migration 0078 將23張backfill訂單38行 `order_items.drawing_cost/item_base_cost/subtotal_cost` 改返做全額，`orders` 表total_cost/keychain_cost/necklace_cost/net_profit完全不變（已驗證bit-for-bit一致——呢啲聚合欄位0076已經計啱，淨係item層代表方式錯咗）。同步簡化V2品項細明反推邏輯（`quantity×rate` 取代 `drawing_charged_count×rate`，移除v2Waived特殊顯示）。**核心規則正式確立**：品項層=全額（gross，唔夾雜任何位置共享折扣）；訂單層=淨額+badge解釋差額。此規則同時揭發 `FHS_Finance_Bible.md` §四【G2】worked example 一直錯誤將S55漂移後嘅演繹當官方範例、`FHS_Product_Cost_Schema_v2.md` §2.1 vs §5.2 自相矛盾（新舊材質成本並存）等權威文件過時/衝突問題，觸發全面文件審查（見下方待辦，Fat Mo已授權重寫）。
- **同日五度追加（Fat Mo 糾正）**：Fat Mo 指出「運輸是成本的一種」——上一輪修復誤把運費(shipping_cost)當「折扣」移除，違反S189最初裁決原則（「運費視為成本一種計入總和但保留獨立欄位拆解」，見cl-flow任務初始需求）。修復：加返運費行顯示，保留「單件基礎成本」重複行嘅移除（呢個係真正嘅顯示bug，兩者唔混淆）。運費暫時未計入item_base_cost/subtotal_cost屬現行訂單層共享折扣機制嘅既有架構限制，非本次顯示層範圍，成本小計繼續以n8n權威記錄值為準，運費行純供參考對照。Browser驗證0600723正確顯示，已部署current.html。

### 5.4.7 配件成本（羊毛氈/燈飾）獨立分類欄位（Session 189 追加，2026-07-25，cl-flow 2026-07-25-0148 ✅ 已修復）

**背景**：5.4.6 附帶發現（`task_2d8d6b4f`）——0600107/0600723 `total_cost` 比三分類（`handmodel_cost`+`keychain_cost`+`necklace_cost`）加總多 $30，原因係「配件」類（羊毛氈公仔/燈飾加購）成本從未歸入任何分類欄，Dashboard 財務彈窗②成本快照鏈完全睇唔到配件成本，同摘要總成本之間有無說明嘅落差（靜默缺口，非算錯錢——`total_cost`/`net_profit` 本身一直正確）。

**產品定義糾正**（本次 session Fat Mo 指正）：配件加購（羊毛氈/燈飾）並非依附所有立體擺設，而是**只依附立體擺設之下嘅玻璃瓶款式**（`pSubCat === '玻璃瓶款式'`）。已核實 `Freehandsss_dashboard_current.html` 第4265/6194/6202-6203行：`w_wool_en`/`l_light_en` checkbox 只喺 `isGlass` 為真時顯示啟用；木框款式冇此選項。`addon_product_sop.md`/`FHS_Product_Cost_Schema_v2.md` §7.1 原文「依附主產品（通常是立體擺設）」過於籠統，已同步修正。

**方案裁決**：評估「新增獨立 accessory_cost 欄位」(a) vs「歸入 handmodel_cost」(b) 兩方案，Fat Mo 選 (a)（獨立欄位，日後配件品類擴充更乾淨）。

**cl-flow 對抗評審**（flow_id `2026-07-25-0148`，CONDITIONAL_READY）揪出草案2個事實錯誤：① `extra.accessory_cost` fallback 永遠讀唔到值——根因係 `buildAuditLedgerHtml()` 呼叫前嘅 REST fetch `select=` 參數（第11081行）冇列 `accessory_cost`；② 草案誤稱新欄位 `NOT NULL DEFAULT 0` 「與現有 handmodel_cost 等欄位約束一致」——實測 migration 0001 顯示原三欄根本冇 NOT NULL，正確 precedent 應貼近 migration 0027（`NUMERIC(10,2) DEFAULT 0`，nullable）。兩者均已修正。

**實作四處改動**：
1. Migration 0079：`orders`/`order_items` 新增 `accessory_cost NUMERIC(10,2) DEFAULT 0`
2. Migration 0080：`sync_order_to_mirror` RPC 擴充支援 `accessory_cost` 讀寫（以 0075 全文為底）
3. n8n `Calculate Profit & Pack Items`（V47.23）新增 `accessoryCostTotal` 累加分支；`Supabase Mirror Prep`（V47.15）新增傳遞——經 live workflow MCP `update_node_code` 直接修改（dry-run 驗證後 `dryRun=false` 正式寫入，自動備份）
4. Dashboard `buildAuditLedgerHtml()`：第11081行 select 清單加 `accessory_cost`（fallback 根因修復）、`catSum` 公式納入 `accessoryCost`、新增「配件成本」`_costRow`

**歷史3張單 backfill**（動態 `SUM(item_base_cost)`，非硬編碼 $30，涵蓋 0696216）：0600107=$30、0600723=$30、0696216=$0（原成本已是$0，非bug）。`total_cost`/`net_profit` backfill 前後完全不變。

**驗證**：finance-auditor 獨立覆核 4 項全 PASS（3張單數值核對、品項層 accessory_cost=item_base_cost、收斂律 handmodel+keychain+necklace+accessory=total_cost 全部 diff $0、全庫掃描確認冇漏網品項）；另揪出 migration 0079/0080 一度只套用 DB 未寫落 repo 嘅漂移（撞正 `feedback_migration_repo_db_drift.md` 教訓），已即時補寫兩份 `.sql` 檔至 `supabase/migrations/`；RPC 路徑另用 synthetic 測試單（`TEST9999`，測試後即刪）驗證 `sync_order_to_mirror` 端對端正確寫入 `accessory_cost`，非純靠 backfill 直接 UPDATE 掩蓋 RPC 路徑未驗證嘅風險。
...
### 5.4.14 `sync_order_to_mirror` RPC `accessory_cost` 讀寫回歸（migration 0088，2026-08-16 ✅ 已修復）
...
### 11.14 Phase 2a：學習系統詞句級規則（cl-flow flow_id 2026-08-04-0244，2026-08-04）

**設計**：§11.13 嘅 `ig_thread_rules`（migration `0084`）只能表達「呢單判錯」——規則以 `thread` 為 key，只影響單一客人。判斷引擎嘅真正輸入係 `lib/order-match.mjs` 內硬編碼 regex（`DEAL_RE`/`QUOTE_RE`/`INTENT_PATTERNS`/`CAT_*_RE`），Fat Mo 無法喺唔改代碼、唔重部署 n8n 嘅情況下調整。Phase 2a 新建 `ig_phrase_rules` 表（migration `0086`）：Fat Mo 喺 thread overlay 點選訊息句子並標語意標籤（`deal` 成交/`quote_draft` 報價傾價/`noise` 唔關事），落地成**跨客人生效**嘅全域規則。本階段為 **observe 模式**：`enforce` 恆 `false`，唔改 `order-match.mjs`、唔改 `build_n8n_workflow.cjs`、唔部署 n8n，判斷邏輯零變更，純標註收語料，供 Phase 2b 開 enforce 前累積真實語料並量測命中率。2026-08-18 為檢視點：提案數 ≥10 視為入口可用進入 2b，<10 判定為入口問題（非需求問題）先改入口再重數。

**安全設計（A2/Gemini 對抗評審 BLOCKER #1/#2 修補）**：原方案假定「phrase 必須真實出現喺來源訊息」已足夠防線，同 §11.13 F3 護欄同一思路。實測發現 `ig_messages`/`ig_watchdog_alerts`/`ig_thread_rules` 三表 anon SELECT 全開（`pg_policies` 2026-08-04 實測確認），而 anon key 內嵌喺公開 Dashboard HTML——外人可自行 DM 任意文字入庫觸發自己個 thread 嘅警報，再引用該訊息通過護欄，寫入全域抑制規則癱瘓看門狗（Phase 1 嘅 thread 級規則因為只影響自己個 thread 冇呢個問題，Phase 2 全域規則性質完全唔同）。修法：**二段式狀態機**（`proposed`/`approved`/`rejected`），anon 只能造 `proposed`；DB CHECK 約束 `ig_phrase_rules_enforce_requires_approval`（`enforce=false OR status='approved'`）喺資料庫層強制 `enforce=true` 一定要 `status='approved'` 先過到，即使上層代碼寫錯都擋得住。`fhs_admin_set_ig_phrase_rule`（批准/否決/重啟/開 enforce）收歸 service_role，`GRANT`+`REVOKE FROM anon, authenticated, PUBLIC` 一次做對（沿用 §11.13 GRANT 矩陣教訓，唔重蹈 `0084`→`0084b`→`0084c` 三次修補嘅覆轍）。anon 嘅停用（`fhs_disable_ig_phrase_rule`）保留單向：只可 `active=false`（fail-safe），因為 Dashboard 只有 anon key 冇 auth session，全撤 anon 執行權會廢咗 Fat Mo 出事時嘅緊急停掣。

**RPC 護欄（G1-G12）**：`fhs_add_ig_phrase_rule` 核心係 G3（`position(lower(phrase) IN lower(content)) > 0`，大小寫不敏感，同 `phrase_norm` 比對契約一致）；另加長度 2-40（G4）、純數字拒收（G5）、含方括號拒收防 PII chip 殘片（G6）、抑制類≥3字（G7）、全表/單類生效規則上限 200/50（G8/G9）、單一 thread 待審提案上限 5（G11）、全表 24 小時提案速率上限 30（G12）——後兩者防止「自己 DM 自己」灌爆，係 Phase 1 F3 冇遇過嘅全域規則專屬風險。`fhs_preview_ig_phrase`（anon，建規則前影響預覽）用同一 `position()` 比對式（單一真源），刻意唔用 PostgREST `ilike`（值含 `,.()` 會破壞 query string；使用者揀嘅句子含 `%`/`_` 會被當 SQL LIKE wildcard 造成命中數虛高）。

**`phrase_norm` 跨層比對契約**：`ig_messages.content` 已係 `redactPii()` 輸出，而 `redactPii()` 第一步即 `toHalfWidth()`，故存庫內容已為半形，DB 端毋須實作 `toHalfWidth`。Phase 2b 引擎端對應寫法必須係 `toHalfWidth(redactPii(text)).toLowerCase().includes(phrase_norm)`——比對對象係 `redactPii()` 之後嘅文字，唔係原文，否則含 `ADDR_UNIT_RE` 改寫區段嘅規則永不命中。

**V42 UI**：`_igwRenderMessageHtml()` 改造成分句渲染（`_igwSplitSentences()`，切分符 `。！？!?；;\n`，刻意不含逗號——實測 79% 訊息 ≤40 字，切逗號會過碎），逐句包 `<span class="igw-sent">` 供點選；轉義紀律不變（先分句純文字操作→逐句轉義→對已轉義字串做 mark/chip）。手機主路徑：句子點選+多句連選（非連續選取前端擋）；desktop 加成：原生選字「吸附」被 selection range 覆蓋到嘅 `.igw-sent` 節點（唔採信 `getSelection().toString()` 原始字串，因為時間戳 `<div>` 同訊息文字同層會污染選取內容）。標籤條 `_igwRenderPhraseBar()` 前端鏡像檢查→`fhs_preview_ig_phrase` 影響預覽→`_igwConfirmInline()` 二次確認（D51 教訓一致做法）→寫入。全域規則列表預設只顯示 phrase 出現喺當前 thread 已載入訊息內嘅規則（前端過濾記憶體陣列，零額外查詢），狀態文案區分 `🟡待審`/`🔵已批准未開生效`/`🟢生效中`/`⚪已停用`（observe 期 `applied_count` 恆為 0，唔可沿用 §11.13 嘅「未生效過」文案，否則會被讀成壞咗）。

**驗證（執行時點）**：migration `0086` 部署後即時用 `has_function_privilege()` 核實權限矩陣（anon 只可 add/disable/preview，admin/touch 兩支 service_role-only RPC anon 零權限，service_role 可執行 admin）；DB 層 CHECK 約束實測（直接 `INSERT enforce=true status='proposed'` 正確被拒，零殘留）。分句函式對 25 則 live `ig_messages`（含 8 則帶 PII token）用抽出嘅實際 shipped 原始碼跑不變式測試：`parts.join('')===text` 全數成立、PII token（`[電話]`/`[IG帳號]`/`[門牌]`/`[付款尾碼]`）零切斷。XSS：`jsdom` 真實 DOM parse 6 個 payload（含訂號高亮情境），零元素被建立、`textContent` 逐字元相符原文。Browser 端對端實測（開 overlay→點句→建規則→DB 核實→停用→DB 核實）、`current.html` 同步、fixtures 回歸留待 cl-final-plan §6 驗證清單 V3/V10/V12 逐項完成，完成狀態見 `Changelog.md` 對應條目。

**已知限制**：Phase 2a 純標註，`ig_phrase_rules` 對判斷邏輯零影響（`enforce` 恆 false）。2026-08-18 前若提案數 <10，代表現行入口（總覽→igwatch→撳「💬 IG訊息」→開 overlay→選字）太深，Phase 2b 會暫停先改入口（候選：igwatch 卡片層加淺入口、每週 Telegram 推未標註提醒）。抑制類（`quote_draft`/`noise`）撞既有 `DEAL_RE` 詞嘅衝突護欄刻意延到 Phase 2b 喺引擎端做（避免喺 SQL 複製一份 `DEAL_RE` 造成第二真源），本階段冇呢條防線但因 `enforce` 恆 false 零實際影響。

詳見 `Changelog.md` 2026-08-04「IG看門狗學習系統 Phase 2a」條目、`artifacts/2026-08-04-0244/`（cl-final-plan.md §5 最終執行計劃）、`.fhs/notes/decisions.md` D58。