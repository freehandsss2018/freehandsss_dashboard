...
- **裁決（Fat Mo 2026-09-25，最終；同日曾兩度改選，以本條為準）**：
  - **S175 llm-council-skill：結案，唔裝。** 判準數字上通過（D30／D36／D39＋D43），但決策已由 `/cl-flow` 跨廠評審＋拷問覆蓋、冇「缺 council 而做錯」個案；5 顧問全係 Claude 分身，遜於跨廠交叉驗證；`.claude/skills/` 已 30 支。**重啟條件**：出現一單「`/cl-flow` A1/A2 不可用或全部同向、決策事後證實錯」個案。
  - **S170 拷問技能：試用閘通過，四支留低；`to-spec` 第二批繼續延長審視期**（Fat Mo 暫時冇時間），期限 AI 預設 4 週＝**2026-10-23**（列便攜塊 ⏰ 時限待辦，由 `fhs-health-check` 第7類機械檢查；可改期）。期間 to-spec 不裝。AI 傾向不吸收（FHS 已有 `/cl-flow` 規劃檔＋decisions.md 詳文層）。
