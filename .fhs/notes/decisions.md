...
**量度**（重播 `~/.claude/projects` 全部 143 session、2,050 輪；「無覆蓋」基數 1,635 輪＝現行 hook no-signal 且本輪冇 dispatch／依據標記／豁免標記）：
| 候選 | 多攔輪數 | 捉到實例？ |
|---|---|---|
| Edit/Write 寫入 `FIN_DOCS` 路徑 | 0 | 否 |
| 寫入內容含財務欄位名（方案 B） | 66（3.2%） | 否 |
| 回覆含財務字眼＋金額 | 40（2.0%） | 否 |
| 寫入內容含任何財務字眼 | 229（11.2%） | 是 |
| 回覆含任何財務字眼（＝訊號(e)拿走問句條件，方案 C） | 235（11.5%） | 是 |
**核心發現**：「捉到動機案例」同「低誤報」互斥——捉到實例嘅候選誤報 ≥11%，誤報 <4% 嘅候選全部捉唔到。

**系統層面限制（A2 批評#5，採納）**：實例本質係 `/commit` 輪轉述更早輪次已產生嘅結論（當時已被 hook 覆蓋）。Stop hook 無狀態，分辨唔到「轉述舊結論」同「作出新結論」——**唔係 hook 擴充可修補，方案 A/B/C 皆然**；D83 已否決語義判斷路線。

**D83 準則逐點適用（A2 批評#6，採納）**：準則＝「預防性擴張需已發生且重複嘅損害作依據」。①實證只有 1 宗，且屬轉述；②量度證明任何捉到它嘅方案 ≥11% 輪次摩擦；③豁免標記純文字，攔截量上升會令豁免通脹、hook 形同虛設（D83 AG 批評#3 已預告此模式）。本次「不擴充」決定同擴充方案接受同等 A2 評審（6 條批評：3 MAJOR＋3 MINOR，無 BLOCKER）。

**A2 評審處理**：採納 5、拒絕 1（#3 `FIN_DOCS` 寫入路徑係直接量度＝0 輪，非推論）。#1 採納結論但更正機制：即使唔排除暫存檔，該輪寫入內容都冇財務欄位名，方案 B 一樣捉唔到。

**執行內容（方案 A）**：`stop-finance-auditor.js` 只改頂部註解（v1.1.1，新增「已知不覆蓋」段，零邏輯；夾具 35/35 PASS）；本條；handoff 便攜塊 🟠[D83-follow] 改 ⚪ 已決定不修。

**重啟條件**：出現第 2 宗「錯結論經 Edit/Write 入庫且事後被 Fat Mo 發現」，或日後可度量豁免使用率。重議方案 B 前置：派 fresh-context agent 抽樣覆核 66 輪內幾多輪係真財務結論。

**量度侷限**：歷史輪次多數喺 hook 上線前，AI 冇機會用標記自救，「多攔輪數」係上限；豁免通脹無歷史數據可量。「不擴充」係基於證據不足以支持付費，唔係證明冇風險。

**Subagent 使用記錄**：❌ 治理設計評估＋transcript 重播量度，非財務數字判斷，豁免 `finance-auditor`；A2 評審由 Gemini（`gemini-2.5-flash`）執行。全文見 `artifacts/2026-09-24-0534/cl-final-plan.md`。
...
⚠️ **編號撞號說明**（2026-09-24 合併入 main 時重編：與主線 D79〔n8n secret 洩漏〕撞號，收款分帳雙模式→D86、批次色方案C→D87，同 D80 先例）：本檔緊接嘅兩條原暫編 D79（收款分帳雙模式 vs 訂單總覽批次色重做）係兩條獨立並行 worktree 分支各自編號、事後 merge 先發現撞號，非同一事件（同 D69 系列先例同類情況，見下方「分支合併事故」條目）。

[2026-09-15] (D86，原暫編 D79) 收款分帳逐件/簡化雙模式：自動填餘值一致化 + 總額不符確認 bar + 快捷掣顏色狀態化 — Fat Mo 截圖回報訂單 0600914 三個問題（自動填餘值缺提示/顏色唔取消/簡化模式$0 bug），`/cl-flow-fast`（flow `2026-09-15-0607`）規劃、Gemini A2 對抗評審 7 條批評 6 採納（BLOCKER：focusout setTimeout 競態改同步執行；MAJOR：多行 bar 互相覆蓋改佇列、「取消」原會誤改對面已收款箱改為只還原剛改嗰格、售價變動主動彈 bar 改靜默失效）。實作＋主session自測後另派 fresh-context agent 獨立覆核，12 項測試矩陣全 PASS，揪出 1 個簡化模式 `agg[cat]||''` 令合法 $0 顯示空白嘅小 bug 已一併修復。純前端 UI 修復，核心財務公式不變，未觸發 finance-gatekeeper 路由同步（同 D69 系列先例一致）。全文見 Changelog.md「2026-09-15 續」條目、`artifacts/2026-09-15-0607/cl-final-plan.md`（Phase 1.6(b) 無完成報告小改動，Changelog 為全文居所）。
...
**回饋 1 修復**：`.fhs-cover-slot`（冇圖狀態）原本完全透明零視覺痕跡，只喺 hover 或全域拖拽時先顯形——呢個係 D74 上一輪「兩樣都冇就留空」定案嘅副作用，過猶不及變成完全唔 discoverable。改為常駐顯示淡色 `icon-image` 圖示（20px，複用既有 sprite，唔新增 SVG symbol），平時已可見但唔算「常駐 button」（冇邊框、冇背景色、冇文字標籤，純圖示提示，同 Fat Mo 早前否決嘅 button 方案性質唔同）。

**回饋 2 修復**：真實生產列（訂單 0600303，單一品項）量得列高 87.9px，圖片 60px 高之下仲有 18.7px headroom。107×60 改 **135×76**（16:9，放大約 26%），留 2.7px 安全緩衝應付跨瀏覽器行高量度誤差。

**過程中自揪並修正嘅新 bug**：實作回饋1嗰陣，一開始將冇圖狀態嘅 icon 格仔都套用返 135×76（同有圖狀態共用尺寸），喺自家 browser 驗證「手模」類別視圖（刻字欄 `mwCat` 淨得 172px）時發現刻字文字被逼到得返 ~20px 位，觸發 `overflow-wrap:anywhere` 逐字元直排（視覺上似一串直排亂碼）。根因：冇圖狀態嘅 icon 格同刻字文字＋編輯掣係同一 flex row 嘅並列 sibling，兩者要爭同一份有限闊度；`flex:none` 令 icon 格唔會被壓縮，全部落差就轉嫁去文字身上。有圖狀態冇呢個問題，因為有圖時 wrap 淨係得 `coverSlotHtml` 一個 sibling，唔使同文字並列。修復：兩個狀態改用唔同尺寸——`.has-img` 維持 135×76（相片顯示尺寸，回應回饋2）；冇圖狀態縮返 30×30（icon 展示尺寸，唔再搶文字位）。此 bug 從未流出生產（喺 `/commit` 前嘅自家 Browser pane 驗證揪出並即時修正），但記錄低係因為呢個「兩個狀態貌似應該用同一尺寸」嘅直覺本身就係錯——填色/縮圖用嘅尺寸同「純提示 icon」用嘅尺寸係兩件唔同性質嘅嘢，唔應該共用同一個 CSS 數值。

**驗證**：真實 Supabase 55 張單全量掃描——「全部」視圖（330px 欄）同「手模」視圖（172px 欄）均 0 個 `.fhs-inline-eng-wrap` overflow（`scrollWidth > clientWidth`）；0600303 相片量得 135×76 正確；全域拖拽模擬（`DragEvent`）確認 30×30 icon 格觸發虛線框+米色底、唔會突然變大；console 零新增錯誤。

全文見 `Freehandsss_Dashboard/freehandsss_dashboardV42.html` `.fhs-cover-slot`/`.fhs-cover-slot.has-img`/`.fhs-cover-slot:not(.has-img)` CSS 三規則。**Subagent 使用記錄**：❌未使用（真機截圖回報跨視圖真實資料驗證，委派會斷推理鏈）。
...
**真正查證（用真實 DOM 度數，非估計）**：喺 1000px（落入 750-1129 範圍）用真實訂單 0650429（Shirley Lee）逐格量度：
1. **「x2」跌落新行**：`review-badge` flex container 實際淨得 110px（td 130px 宣告值扣曬 td/inner div padding），「嬰兒」(43px 連 margin)+「左手」(43px)+「x2」(24px)＝110px 啱好卡喺臨界點，任何 subpixel 都會逼「x2」跳去新行。
2. **BOOK/LASER 唔係乾淨 3 行**：`hmck-wrap-*` 嘅真實 markup 係 `display:flex;flex-wrap:wrap`（**非** `flex-direction:column`）——「Book」(~44px)+「Laser」(~50px)加埋先 ~94-98px，宣告 115px 嘅進度欄仲夠佢兩個逼喺同一行，要窄過呢個組合先會逐個換行變返 3 行。
3. **兩者背後嘅共同根因**：750-1129 呢層一直冧咗同 D73 一樣嘅 `table-layout:fixed` 比例拉伸——全部欄都有明確 `!important` width 時，瀏覽器按比例攤分「表格闊度－欄闊總和」嘅落差落每一欄。實測 1000px 下：宣告 115px 嘅進度被拉到 163px，Order_ID/Date/Customer/prod 全部跟同一 ~1.42x 比例拉伸——證實一直未修過，D73 嗰次修復淨止於 ≥1130px「全部」視圖，從未推廣落呢層。

**修復**：跟 D73 手法，喺 750-1129 media query 淨留「備註」一欄唔宣告 `!important` width（同 JS `fhsBuildOverviewHead()` 一齊拆，條件由 `k==='notes' && _fhsDesktopFlexEng(_c)` 擴大做 `k==='notes' && !_c`——兩個 tier 一齊生效），令佢做呢層嘅唯一彈性欄，其餘 8 個可見欄先至真正維持喺宣告值。連帶調整兩個數值：產品明細 130→165px（真實內容需要嘅下限+buffer）；進度 115→75px（刻意窄過「Book+Laser」組合所需嘅 ~94-98px，逼 flex-wrap 逐個換行）。

**驗證**：Chromium 1000px 實測——① Order_ID/Date/Customer/fin/prod/batch/stat 全部貼緊宣告值（88/68/58/92/165/52/75），零比例拉伸；②真實訂單 0650429 逐格量度：「嬰兒/左手/x2」三個 badge 同一行（top 值相同）；「Book/Laser/音訊」三行完全分開（top 值各不同）；③桌面「全部」視圖（≥1130px，follow7/8 成果）逐項重驗零回歸；④類別視圖經程式碼審查（`_c` truthy 時 CSS class／JS 條件皆唔命中）不受影響；⑤console 零 JS error（有數個 `ERR_NETWORK_IO_SUSPENDED` 網路層雜訊，同代碼改動無關）。

**誠實局限**：備註做咗呢層嘅唯一彈性欄後，喺 1000px 實測膨脹到 352px（原 56px 嘅 6 倍多）——視覺上可能顯得比例失衡（喺 750-1129px 範圍內越接近上限 1129px，備註會越大）。呢個係「淨留一欄食晒落差」呢個標準技巧嘅必然取捨：唔留彈性欄就會打回 8 欄一齊比例拉伸嘅原有 bug，冇兩全其美嘅 CSS-native 做法。已如實記錄，若 Fat Mo 覺得備註太誇張，可以再傾點樣加個上限（例如改派俾另一個唔咁顯眼嘅欄，或者接受表格唔撐滿容器闊度、右側留白代替）。

全文見 `freehandsss_dashboardV42.html` 750-1129 media query 區塊／`fhsBuildOverviewHead()`。**Subagent 使用記錄**：❌未使用（真實 DOM 逐格度量+跨 tier 交叉驗證，委派會斷推理鏈）。

---
[2026-09-06/07 續] (D71-follow8) 五點桌面「全部」視圖再覆核：日期收窄過度、產品明細復原、刻字仍太窄、進度收窄、餘位撥俾備註

**背景**：D71-follow7 部署後 Fat Mo 再截圖回報（附「手模」類別視圖screenshot作參考基準，經澄清確認實際改動對象仍係桌面「全部」寬螢幕視圖，非類別視圖本身）五點：①日期欄收窄過多，日期／限時警示應各佔一行、只准兩行；②產品明細過闊，要求復原 D73 之前嘅原始闊度；③刻字仍然太窄，要求 >320px；④進度欄要收窄，貼近類別視圖入面 checkbox 緊湊嘅觀感；⑤淨低騰出嚟嘅空間撥俾備註。

**設計**：彈性欄角色由「產品明細」（D71-follow7）交俾「備註」（notes）——產品明細復原返 D73 之前嘅原始固定闊度（220px，唔再彈性）。刻字／進度／日期三欄各自新增桌面「全部」視圖專屬覆寫值（`_FHS_DESKTOP_MW_OVERRIDE = {eng:330, stat:85, Date:90}`），**刻意唔改 `_FHS_TH_DEF` 嘅共用 `mw`**——類別視圖（手模/鑰匙扣/頸鏈）同 750-1129 緊縮桌面層（早已有獨立 `!important` 覆寫）完全唔受呢輪改動影響。日期欄額外將 `.ovw-date-line--stack`（日期／限時警示上下疊，原本只限手模類別視圖）推廣去桌面「全部」視圖，令收窄後嘅欄位淨需夠一行內容用。

**驗證**：Chromium 1400px 實測 11 欄——單號112／日期90(2行疊)／客人180／入帳成本利潤各65／刻字330（>320 達標）／產品明細220（復原）／批次60／進度85（收窄）／備註92（彈性欄，吸晒落差）；日期同備註 cell 逐項 `scrollWidth` vs `clientWidth` 核對零溢出；`toggleAuditMode()` 開關前後表頭不變；console 零 error。

**誠實局限**：備註（notes）而家係彈性欄，實際闊度隨螢幕闊度浮動——喺 1400px 測試環境量得 92px，**比 D71-follow7 之前嘅固定 110px 仲窄**（因為客人 180＋刻字 330＋產品明細 220 呢三個明確加大咗嘅闊度，喺呢個螢幕闊度總共食走咗比讓俾備註更多嘅空間）。如果 Fat Mo 實機睇落備註仲係太窄，可以進一步收窄客人／刻字其中一個嚟讓位——呢兩個數值（180／330）係基於量度／明確指示嘅「至少要幾多」下限，仲有得再拉扯分配。

全文見 `freehandsss_dashboardV42.html` `_FHS_DESKTOP_MW_OVERRIDE`／`fhsBuildOverviewHead()`／`.ovw-date-line--stack` 套用點。**Subagent 使用記錄**：❌未使用（跨欄位闊度分配權衡+Chromium 交叉驗證，委派會斷推理鏈）。

---
[2026-09-06 續] (D71-follow7) D73 覆核：客人欄現形過窄真 bug + 刻字裁減三分一

**背景**：D73 部署後 Fat Mo 覆核回報兩點：①客人欄壓縮太過，有客人名稱走位；②刻字佔位過多，要求減少三分一闊度。

**客人欄根因**：D73 令入帳/成本/利潤/單號等欄唔再受 `table-layout:fixed` 嘅比例落差拉伸，但客人欄嘅宣告值（`mw:80`）原來一直冇窄到單獨企得住——之前係靠嗰個「意外比例拉伸」將 80px 隱性谷大先冇出事。D73 拆除拉伸之後，客人欄嘅真實過窄現形。**用真實生產資料查證**（非估計）：`window.globalOrders`（60 張單）逐個用 Canvas 實際字體（13px/500）量度，最長客人名 `"Lovehedqie852 (Selina Lai)"` 需要 157px——同 2026-08-25「密度重排」舊 comment 早已預警嘅數字完全吻合（該 comment 講嘅係類別視圖 mwCat，但「全部」視圖嘅 mw 從未跟上呢個下限）。

**修復**：
- `Customer.mw`：80 → **180px**（貼緊 157px 實測值 + padding buffer），實測該真實訂單 TD `scrollWidth=179px`，`clientWidth=179px`，零溢出。
- 刻字（`eng`）：由「桌面全部視圖唯一唔宣告 width 嘅彈性欄」（D73 版本，會食晒 100% 落差）改為**明確上限 `_FHS_DESKTOP_ENG_MW = 260px`**（約 D73 版本量得 ~396px 減三分一）。
- 彈性欄角色改派俾**產品明細**（`prod`）——刻字已鎖定明確上限，產品明細本身多個 badge，額外闊度用得着，換佢做彈性欄冇壞處。實測 1400px：產品明細由固定 220px 變成彈性 255px。

**驗證**：Chromium 1400px 實測 11 欄全部貼緊宣告值（單號112／日期75／客人180／入帳成本利潤各65／刻字260／產品明細255／批次60／進度102／備註110，總和 1381px 同表格實際闊度吻合）；真實最長客人名 TD 零溢出；resize 事件 dispatch 後重繪冧唔到、欄闊維持一致；console 零 error。

全文見 `freehandsss_dashboardV42.html` `_FHS_TH_DEF.Customer`／`_FHS_DESKTOP_ENG_MW`／`fhsBuildOverviewHead()`。**Subagent 使用記錄**：❌未使用（真實生產資料 Canvas 量度+Chromium 交叉驗證，委派會斷推理鏈）。
...
**根因鏈**：
1. `order-overview` 分支早喺 2026-08-30 就修好咗 iPhone 橫向問題——commit `bffc231`「真機橫向手機模式最終修復，斷點由768改750」。iPhone 13 Pro 橫向名義 844px，扣除瀏海安全區後有效闊度落喺 750–768 之間，舊斷點 768 會誤判做手機而出卡片版。
2. 該分支 117 個 commit 從未 merge 入 `main`，而本分支由 `main` 開出，血統仍係斷點 768 嘅舊版。
3. `current.html` 係**跨分支共享嘅單一部署目標**，git 分支之間互相唔知對方部署過乜，PUT 就係完整覆寫。

**同類先例（本次為第三次）**：2026-09-03 `read-command-15bfd5` 連續 4 次部署覆寫另一分支 31 輪 UI 成果；同日 D69 系列兩條分支「雙方各自 deploy 覆寫咗對方生產版」（見上方 2026-09-03「分支合併事故」條目）。**D70 明文將此列為「刻意不處理」嘅未解課題**——本 Gate 即為補位。

**兩次誤診（記錄落嚟避免將來重蹈）**：症狀出現後，先後判斷為「iOS home-screen 快取」同「WebKit standalone webview 冷啟動時序」，兩個都係錯，仲白費 Fat Mo 時間去試關 Safari。**盲點成因**：用 Chromium 模擬 844×390 測到「正常」就當冇事——Chromium 唔會模擬 iOS 橫向安全區內縮，永遠測唔出 750–768 呢個區間；亦冇喺部署前檢查其他並行分支有冇更新過生產版。**教訓**：真機專屬症狀模擬唔到時，優先查「我部署嘅嘢係咪同真機睇緊嘅嘢同一份」，唔好一頭鑽落裝置行為推測。

**機制設計**：`scripts/upload-web.ps1` 新增 Gate 0，喺 PUT 之前執行：
1. 每次部署注入 `<meta name="fhs-deploy-src" content="<commit-sha>">`（同既有 `fhs-build` 時間戳並列）。
2. 下次部署前抓生產版，讀返個 commit，`git merge-base --is-ancestor <生產版commit> HEAD`：
   - **係祖先** → 我哋喺佢之上前進，放行。
   - **唔係祖先** → 對方有我冇嘅工作，**擋**，並列出 `git log HEAD..<sha>` 畀人睇會抹走乜。
   - **讀唔到標記／commit 不存在** → 無法核實，擋。
3. 逃生口 `-AllowClobber`，每次使用記入 `deploy-log.md` 稽核。

**安全依據**：同 D70 Phase 2.6 一樣，用 git 客觀祖先判斷，唔做任何內容猜測或自動合併——判斷錯只會擋多咗，唔會靜默放行。

**實測**：生產版當時無標記（Gate 0 上線前嘅舊部署）→ 正確攔截、exit 1、本機檔案零改動。

**已知限制**：`.ps1` 檔案**必須帶 UTF-8 BOM**，否則 PowerShell 5.1 會用系統 ANSI codepage 讀，中文全部亂碼並觸發連鎖 parse error（本次寫檔時中招一次，已修）。呢點同 `learnings/tooling.md` 記錄嘅 `Get-Content`/`Set-Content` encoding 陷阱同源，但適用對象係腳本檔案本身。

**善後**：生產版已復原成 `order-overview` 分支版本（三關驗證 PASS）；該分支 117 個 commit 已 merge 入本分支；D71 已喺合併後嘅正確血統上重做。**未解**：`main` 血統仍落後生產版（見 handoff MASTER 表 D72-follow）。

**Subagent 使用記錄**：❌未使用（跨分支 git 考古 + 部署管線改動 + 即時實測，委派會斷推理鏈）。
...
**驗證**：`node --check` 抽取全部 inline `<script>` 語法通過；本地 `npx serve` 起 Freehandsss_Dashboard 靜態伺服器，手機橫向寬度（900px）+ 真實 Supabase 快取資料（`window.globalOrders`，60 筆）實測——兩張與 Fat Mo 原始截圖相同嘅訂單（0600728 Dorothy $2,380/$210/$2,170、0650429 Shirley Lee $4,260/$770/$3,490）數值完全對應；`getComputedStyle` 直接量測三行 `.fhs-fin-amt` 均為 `12.5px`/`700` 完全一致；`toggleAuditMode()` 開關測試逐 SKU 明細正常顯示/隱藏；console 零 error。**[2026-09-06 D71-follow4 範圍收窄，Fat Mo 裁決]**：Fat Mo 覆核後指「優化做多了～財務欄的優化只適用於手機橫向模式介面，其他則保留不變」。原實作將合併套用落所有 ≥750px 嘅「全部」視圖（連闊螢幕桌面）——闊螢幕本身冇空間壓力，三欄分開掃描性反而更好。

**改為只喺 750–1129px 生效**（即對方 D69續八 定義嘅「緊縮桌面」層：iPhone 橫向 844/874、iPad 直向 834）：
- 新增 `_fhsFinMerged()` 判斷式；`fhsOverviewCols()` 按佢揀 `[fin]` 定 `[inc, cost, profit]`。
...
- **JS 邊界必須同 CSS `@media (min-width:750px) and (max-width:1129px)` 逐字一致**——兩處改一邊就要同步另一邊，否則會出現「JS 出咗 fin 欄但 CSS 冇對應寬度」嘅錯配。已實測 `window.matchMedia()` 同 `_fhsFinMerged()` 喺同一闊度判斷相同。
- 因為欄位組合係喺 render 嗰刻決定，另加 `resize`/`orientationchange` 監聽，**只喺合併狀態真係翻轉先重繪**（唔係每個 resize tick 都重繪成張表），沿用同檔 `fhsSyncCompactDesktopLayout` 嘅既有做法。

**實測三層**：1400px→原三欄；758px→合併（產品明細 142px／進度 126px）；390px→手機卡片版原樣。雙向拉窗口自動重繪、27 張單三個 id 齊全、邊界 JS/CSS 判斷一致、零橫向溢出、console 零 error。

**[2026-09-06 D71-follow3 最終結論，推翻下方 follow2]**：下方 follow2 嘅兩個診斷（iOS 快取、WebKit 時序）**全部錯**。真因係 D72 跨分支覆寫事故——`order-overview` 分支早已將斷點由 768 改 750 修好 iPhone 橫向問題，而本分支部署時用舊血統覆寫咗佢。已 merge 該分支（117 commit）並喺正確血統上重做 D71：跟對方 `_FHS_TH_DEF` 宣告式欄位架構，新增 `fin` 欄取代 `inc`/`cost`/`profit`，colspan 由 `fhsOverviewColCount()` 自動計算；緊縮桌面 750–1129px 區釋放 73px 分俾 prod(84→130)/stat(88→115)，欄闊總和維持 745px。實測 758px：表格版正常、產品明細實際 142px、進度 125px、三行字體 12.5px/700 完全一致、零溢出、audit toggle 135 項明細正常、console 零 error。全文見 D72 條目。

**[2026-09-06 D71-follow2 追記，回歸排查（❌ 結論已被 follow3 推翻，保留作誤診記錄）]**：Fat Mo 部署後回報「iPhone 13 Pro 橫向訂單總覽變咗直向卡片版」（橫向照片確認）。查證：①本地/生產檔逐 byte 核對零差異；②模擬 iPhone 13 Pro 實際橫向解析度（844×390）多次測試（本機 + 生產 URL）table 均正常顯示，console 零 error；③實測證實 CSS `@media(min-width:768px){#reviewAccordionContainer{display:none!important}}` 對顯示與否有絕對否決權，唔受任何 JS render 呼叫影響——即係話若真實裝置當刻 CSS 判定 <768px，JS 邏輯本身冇得反駁。**未能 100% 複現確診**，最可能解釋：iOS「加至主畫面」standalone webview 喺已經橫向嘅狀態下冷啟動時，layout viewport 初始化時序有已知 WebKit 落後問題，令 CSS/JS 讀到嘅闊度短暫（或持續至下次量度）唔同步真實方向；亦不排除裝置本身「顯示縮放」設定令有效邏輯闊度細過 844。**已落防禦性修復**（非邏輯改動，純追加）：仿照既有 `foResizeTimer`（財務圖表 resize 重繪）手法，新增 `_fhsReviewRecheck()`，`resize`/`orientationchange`/`load`後 500ms 各觸發一次重新呼叫現有 `renderReviewTable()`（唔改動判斷式本身，只係逼佢重新評估一次現狀闊度）。**誠實局限**：呢個修復針對「初始量度不同步、事後會自我修正」呢類情境；若真係裝置設定令闊度持續穩定細過 768（例如顯示縮放），呢個修復幫唔到手，需要 Fat Mo 核實 iPhone 設定 > 螢幕顯示與亮度 > 檢視 是否為「縮放」模式。全文見下方 D71-follow2 及 Changelog 2026-09-06 條目。

**[2026-09-06 追記，同一 `/commit` 內完成部署]**：依 AGENTS.md §3 途徑(c)，`/commit` 偵測本次 commit 確實改動 `freehandsss_dashboardV42.html` → Phase 2.5 自動觸發升格，`/fhs-check` 4 PASS/1 SKIP 前置通過，三關驗證 PASS（HTTP 204／大小 1,225,315 bytes 相符／SHA256 `36203277F6AAB2222306FBCC40A5496017A62C15EC2D7944CCA75D72D82B84A3` 相符），已上線 `https://yanhei.synology.me/Freehandsss_dashboard_current.html`（commit `c34bc5f`）。
...
**定案方向**：①三大類（手模擺設/鎖匙扣/頸鏈吊飾）沿用「財務結算」逐件分攤表既有配色（紫`#4A148C`/藍`#1565C0`/棕`#4E342E`），貫徹到圖示、開關、標題文字、展開箭嘴全部子元件層級 ②鎖匙扣嬰兒/大寶部位由「4個獨立開關各自輸入」改為「藥丸式多選部位＋共用數量/上排/下排一次填，約10%例外先展開個別覆寫」 ③預設狀態全面對齊真實 `resetForm()`/`_applyGlassDefaults()` 邏輯 ④保留 V40 底部功能列/財務結算卡片位置/警告文字/成本徽章，逐一對照 `current.html:4209-4861` 核實 112 項原版控制項零遺漏 ⑤新增「否＝純金屬單」提供 Google 相片上傳連結提示（新提案功能）。

**資料相容性驗證**：逐行對照真實 `generate()`/Category A-B 訊息組字邏輯（`current.html:6522-6919`），確認鎖匙扣新版「共用填寫」介面底層仍正確映射返 `k_lh_en`/`k_lh_qty`/`k_lh_top`/`k_lh_bot` 等 canonical 欄位，父母/大寶顏色選單補齊 `data-who`/`data-part` 屬性——確認新介面 100% 駁得上現行 IG 查閱訂單訊息 Category A/B。
...
1. **P mode 鎖匙扣**：原邏輯每個身體部位（左手/右手/左腳/右腳）各自獨立查階梯表（`processTierPricing()` 逐 item 計算）。客戶售價表「最少訂2個吊飾，小手、腳任選」證實應為**總量合併計價**。例：左手1+右手1，舊邏輯 $1,580×2=$3,160，客戶售價表用合併總量 qty=2 查表=$2,160，差 $1,000。Fat Mo 進一步確認「第3/4隻吊飾如是另一手/腳需+$100圖紙費」按**全新不同部位數**計（非數量位置）：`surcharge = max(0, distinctParts−2)×$100`。S mode 及家庭連心分支不受影響，維持原「各部位獨立」邏輯。
2. **S mode 純銀頸鏈吊飾**：原公式 `fullNecklaces×2980 + singleCharms×1980`。用真實加購單反推（一對金手+一對銀腳=2對=$2,980×2=$5,960，同發票一致）確認完整頸鏈價不變，但 Fat Mo 澄清 remainder 單隻價（配對外多出嗰隻）由 $1,980 降至 **$1,490**（= $2,980 之半，同 P mode 既有 `extraSingle=extraFull÷2` 結構對齊）。**N=1（訂單總共淨一隻，未成對）維持 $1,980 底價特例，不套用 remainder 公式**——因為「第三隻」措辭前提是已有一對存在。P mode 純銀吊飾公式本身已符合此結構，無需改動。

**執行**：`freehandsss_dashboardV42.html` `processTierPricing()`（P mode 分流，merge+surcharge 邏輯）及純銀吊飾計價區塊（`hasMainProduct` 分支 remainder 率、`_groups` 付款分期同步）。Pricing Bible §3.2/§4.1/§4.3 同步改版至 v1.6.0。

**驗證**（browser 實測，非文件推導）：
- P mode 左手1+右手1（2部位）→ $2,160；+左腳1（3部位）→ $3,040（$2,940+$100跨部位附加費，UI 正確標註）；S mode 同場景不受影響（左手/右手各自 $860 獨立顯示）。
- S mode 純銀吊飾 N=1→$1,980；N=3→$4,470（$2,980+$1,490）；N=4→$5,960（=2×$2,980，同真實發票 0600702 一致）。
- Console 全程零錯誤。

相關檔案：`Freehandsss_Dashboard/freehandsss_dashboardV42.html`（L7259起 processTierPricing、L7358起純銀吊飾計價）、`.fhs/ai/FHS_Pricing_Bible.md` §3.2/§4.1/§4.3。current.html 尚未同步，待既有部署流程升格。
...
**追記（2026-07-22，同日）— 推翻「只新單」決定，追加 7 張歷史單 backfill + n8n 正式部署**：Fat Mo 改變主意，要求連歷史單都要修改。已執行：(1) n8n `Calculate Profit & Pack Items` V47.20 `update_node_code` dry-run 通過（過程中揪出並修正一個草稿 bug——原稿誤把鎖匙扣嘅 `Chain_Cost` 顯示值一併清零，已收窄只影響吊飾類別）後 `dryRun=false` 正式部署，自動備份於 `.fhs/notes/aireports/n8n-mcp-backups/2026-07-22/`。(2) 7 張真實歷史單（Dede/Kathleen/Akira/DebbieHo/Amen/Selina Lai/Lokyi_C；第 8 張 `test1001` 為 $0 成本壓測殘留 fixture 非真實訂單，已排除）直接 Supabase UPDATE backfill——`order_items` 吊飾行三個成本欄位各 +$100、`chain_cost` 統一對稱 100；`orders.n8n_adjustment_notes` 正數 `necklace_chain_cost` 換成負數 `necklace_chain_sharing_discount`（DebbieHo N=1 個案 floor(1/2)=0 直接移除該筆記）。UPDATE 前後逐單核對 `orders.necklace_cost`/`total_cost`/`net_profit`/`final_sale_price` 完全不變，證明純記帳格式重分配。**端對端 live 驗證受阻**：測試單 `test9002` 經真實 webhook 提交後卡喺 n8n `running`（execution 4906）未落地，重新探測 Airtable API 確認 `HTTP 429` 依然存在，同 S186 已知嘅 `test5001`（execution 4902，已卡逾 12 小時）屬同一未解決外部阻塞，非本次 n8n 改動引起（新舊 code 同樣會喺此步驟卡住）。Supabase 核實兩張測試單均未落地，無孤兒記錄，但屬未完成端對端驗證，待 Airtable 額度恢復後補測。全文見 `Changelog.md` S187續IV 條目。
...
原因：Fat Mo 指出現行代碼冇處理「父母加入倒模」場景，並提供歷史單號 **0600107** 作先例（`P_MAIN` = `玻璃瓶套裝 (4肢)`，`item_sale_price=2580`，同單 `K_FAM_COMBO` 品項 specification 記載「父母手 + 嬰兒左手 + 嬰兒左腳」，證實此為當時操作者手動輸入嘅真實定價，非系統錯誤）。查 Supabase 全庫僅此一張訂單嘅 P_MAIN 同時符合「含父母」條件，冇第二個先例可比對「2肢+父母」是否同價，按 Fat Mo 指示文字（無限定肢數條件）採 flat 覆蓋解讀。
執行：`Freehandsss_Dashboard/Freehandsss_dashboard_current.html`（生產）與 `freehandsss_dashboardV42.html`（開發基線，兩檔同步）之 `calculatePricing()` TEMP_P_MAIN 價格區塊改動：`hasAdultInSet`/`hasBabyInSet` 判定提前至價格計算之前；`isGlassJar` 旗標；價格公式改為 `hasAdultInSet ? 2580 : (name.includes("4肢") ? 1680 : 1380)`；混合附加費區塊加 `&& !isGlassJar`；suggested-price log 新增「(含父母，套用家庭定價 $2,580)」提示字樣。**未改動**：SKU 命名邏輯（`buildOrderItemsForPricing()` / 送單 payload 建構函式）——玻璃瓶品名維持 `玻璃瓶套裝 (4肢)` 格式，parent 只影響售價唔影響品名，對齊 0600107 先例；`products.total_base_cost`（成本 $210 flat）不受影響，因本次僅改**售價**（operator 端建議值），非成本。
`FHS_Pricing_Bible.md` 同步更新至 v1.3.0：§2.1 表格新增第三列、§2.2 附加費表加排除條件；§6 footnote 順道修正一句已過時嘅技術債描述（見下一則決策）。
驗證：改動前後分別對 `Freehandsss_dashboard_current.html` / `freehandsss_dashboardV42.html` 執行 grep -c 唯一性檢查（改前=1、改後=1），確認替換精準未波及其他分支。此變動僅影響前端 suggested-price 顯示（operator 建議值），唔涉及 n8n 成本重算或已確認訂單嘅 `final_sale_price` 快照（見 finance-gatekeeper 死線 1/2），故未觸發 live 訂單回填。

**追記（同日）— 防呆補強 + current.html 升格**：Fat Mo 提出「唔會有得父母冇嬰兒嘅情況出現」，指出 §0 品牌核心（禁止單獨成人）喺 P_MAIN 呢個入口冇被現有 `hasAdult && !hasBaby` 防呆覆蓋（因 P_MAIN 品名/comboNote 從不含「父母」字樣，全靠獨立嘅 `en_parent` checkbox 判定）。補強：(1) `calculatePricing()` 新增獨立防呆區塊——立體擺設已勾父母但嬰兒肢體全部「無」時紅字阻擋、報價歸零；(2) $2,580 formula 本身加 `hasBabyInSet` 雙重防守（`(hasAdultInSet && hasBabyInSet) ? 2580 : ...`），防止防呆被繞過時仍誤計家庭價；(3) suggested-price log 顯示條件同步補 `hasBabyInSet`。
...
附帶發現（未修）：chargedPositions Set 不追蹤 P_MAIN 肢（PartDesc 空字串），混合訂單前端顯示可能雙計繪圖費 — Task A 範疇。〔2026-07-19 更正：此問題已於同日稍後 Session 66（見上方條目）修復，`FHS_Pricing_Bible.md` §6 footnote 過時字眼同步更正，見本檔最上方 2026-07-19 條目〕
...
**2026-09-25 覆核補記（試用閘逾期 47 天後補做）**：
- **背景**：2026-08-09 09:00 排程任務 `fhs-2026-08-09-skill-trial-gate-review` 有跑（41 秒、標記 succeeded），但其寫入 handoff 的 Edit 冇落進 repo，結論從未落盤，MASTER 表兩列維持「待覆核」至今。教訓：排程／背景任務嘅「寫入結論」唔可以當已落盤，須事後核對 repo。
- **判準 1（D27，拷問技能 4 週內真實使用 ≥2 次）：通過。** 窗口內 3 次：2026-07-12 D29 取模排程方案書拷問；2026-07-16 D37 Audit Ledger 五輪拷問；2026-07-16 n8n 殭屍 workflow 清理六輪拷問（Changelog 同日條目）。窗口後 2026-09-23 衛生機制重整期一十輪拷問（Q1-Q10）。
- **判準 2（D28 v2，4 週內大架構／治理 D 條目 ≥2 單）：數字上通過**（2026-07-12 至 08-09：D30 AI 團隊名冊、D36 拷問掛鉤機械化、D39 `/cl-flow` A3-first 重組；另 D43 Supabase 唯一 SSoT）。判準只係「准入評估」，唔等於應安裝。
- **裁決（Fat Mo 2026-09-25，最終）**：**S170 拷問技能同 S175 council 兩項都繼續延長審視期**，理由「暫時未有時間認真處理」。期限未指定，AI 預設 4 週＝**2026-10-23** 再覆核（列便攜塊 ⏰ 時限待辦，由 `fhs-health-check` 第7類機械檢查；Fat Mo 可隨時改期）。期間：council 不裝、`to-spec` 不裝，四支拷問技能照常留用。（同日稍早 Fat Mo 曾選「council 結案唔裝」，其後明確改為兩項一併延長，以後者為準。）
- **AI 評估（供 2026-10-23 覆核參考，非裁決）**：council 傾向唔裝——①D30／D36／D39 等決策已由 `/cl-flow` 跨廠評審＋拷問覆蓋，冇「缺 council 而做錯」個案；②5 顧問全係 Claude 分身，遜於跨廠交叉驗證；③`.claude/skills/` 已 30 支，殭屍風險係紅線。to-spec 傾向唔吸收（FHS 已有 `/cl-flow` 規劃檔＋decisions.md 詳文層）。council 重啟條件：出現一單「`/cl-flow` A1/A2 不可用或全部同向、決策事後證實錯」個案。
...
### D62：2026-08-10 — n8n Supabase Mirror 憑證 401 根因查明 + Code node 安全修復

**緣起**：`/fhs-check` 健檢腳本自報全部 PASS（4 passed, 1 skipped），但 LIFECYCLE/STRESS/ACCEPTANCE 輸出文字內藏「testXXXX never appeared in Supabase」警告——腳本判準只認 exit code，Red Flag 被 PASS 訊息淹沒。連續兩輪同日 `/fhs-check` 結果一致，手動查 Supabase api logs 確認：n8n（axios 來源）`PATCH /orders`、`GET /products` 全部 401，同期健檢腳本自己（獨立 key）全部 200——鎖定 n8n 自身憑證失效。追查 `orders` 表非測試訂單 `MAX(updated_at)` 停留喺 2026-08-04 13:15 UTC，真實訂單同步已斷 5 日以上（同 D59-follow/D61 已知未解項為同一根因）。

**查證過程**：追蹤 n8n workflow `FHS_Core_OrderProcessor` 嘅 `HTTP: Supabase Sync RPC`/`Mirror Delete to Supabase` 兩個 httpRequest node，發現唔用 n8n 標準 Credential 機制，而係讀上游 `Supabase Mirror Prep`（Code node）注入嘅 `$json.supabaseKey`；其原始碼：`process.env.SUPABASE_SERVICE_KEY || '<寫死secret>'`。查 n8n Docker Compose 環境變數，確認 `SUPABASE_SERVICE_KEY` 從未設定，故一直行緊寫死 fallback。

**Fat Mo 關鍵質疑「我一直都無改動過任何 key，之前一直都好好哋」**——順住呢個質疑重新查證而非直接建議 rotate：`git log --all -S"<secret前綴>"` 揪出該 secret 由 **2026-05-16 首次出現**，經 12+ 次 commit（n8n workflow JSON 備份檔案）持續明文寫落 repo；`gh repo view --json visibility` 確認 repo 係 **PUBLIC**；`git show origin/main:<檔案> | grep -c` 確認**查證當下**（2026-08-10）條 key 仍以明文存在於公開 GitHub `origin/main`。時序吻合 GitHub↔Supabase secret scanning 自動撤銷合作機制——非人為改動，係 Supabase 偵測公開洩漏後自動撤銷。

**裁決（Fat Mo AskUserQuestion 選定）**：於 `Supabase Mirror Prep` Code node 去除寫死 fallback，避免新 key 重複洩漏循環（而非只換新 key 落同一寫死模式）。

**執行**（`/execute` 授權，`.fhs/reports/planning/` 無 cl-flow Verdict——本任務屬直接事故診斷非規劃流程，改以 AskUserQuestion 明確選項 + dry-run 預覽作對等強度授權，已於執行時對 Fat Mo 透明申報此偏離）：
1. `mcp__n8n-mcp-server__update_node_code`（dry-run 預覽 → `dryRun=false` 正式寫入，自動備份 `.fhs/notes/aireports/n8n-mcp-backups/2026-08-10/6Ljih0hSKr9RpYNm/Supabase_Mirror_Prep.json`）：移除寫死 fallback，改為 `const SUPABASE_KEY = process.env.SUPABASE_SERVICE_KEY;` + `if (supabaseActive && !SUPABASE_KEY) throw new Error(...)`，其餘 6705 bytes 業務邏輯逐字不變。
2. 自動備份檔本身含已曝光 secret 明文一處，就地 Edit 遮蔽（`sb_secret_[REDACTED-...]`），未提交入 repo，避免同一洩漏模式再現一次。

**驗證**：`get_node` 覆核 live workflow 內容已更新（無 fallback 字串，guard 邏輯已生效）。真正端對端驗證（真實訂單能否落地）待 Fat Mo 完成環境變數設定後另行覆核，非本次 scope（本次僅授權 Code node 修復，帳戶層憑證輪替屬 Fat Mo 專屬操作）。

**待辦（Fat Mo 帳戶層操作，AI 依規不可代入密鑰）**：Supabase Dashboard → Project Settings → API 攞新 `service_role`/secret key → n8n Docker Compose `n8n:` service 嘅 `environment:` 新增 `SUPABASE_SERVICE_KEY=<新key>` → 重啟 `n8n-freehandsss` 容器。修復前，真實訂單建立會喺 Supabase mirror 步驟顯示明確執行 error（設計如此，取代之前嘅靜默 401，非新 bug）。

**教訓落盤**：`.fhs/memory/lessons/2026-08-10_supabase-secret-key-public-repo-leak-auto-revocation.md`（首次引入時間查證法、n8n secret fallback 反模式、health check 判準脫鉤三項教訓）。

詳見 Changelog.md 2026-08-10 條目（D62）、`FHS_System_Logic_Overview.md` §5.4.10、`.fhs/notes/session-log.md` 2026-08-09/2026-08-10 條目。

### D62 續：全 workflow 收斂 + 兩個平台層根因（2026-08-10 同日，Fat Mo 追加授權「你能自行處理嗎」）

**追加發現一：合共 3 個節點中招，非 1 個。** 驗證首次修復時發現 `GET /products` 仍 401，全 workflow 掃描同一 secret 揪出：`Supabase Mirror Prep`（已修）、`Smart Cache Strategist`（Code node，條件式 fallback）、`Mirror Delete to Supabase`（httpRequest node，**secret 直接寫死喺 `headerParameters` 完全冇環境變數判斷**，唔會隨設定環境變數自動痊癒）。三者現已全部修復，**live workflow 硬編碼 secret 出現次數 = 0**（API 全份 JSON 掃描實證，30 節點完好、workflow 仍 active）。`Mirror Delete to Supabase` 因係 httpRequest node（`update_node_code` 只食 Code node），改用 workflow PUT API（依 learnings/n8n.md #3 剝走 `settings.binaryMode`），全份 workflow 事前備份至 `FULL_WORKFLOW_pre_delete_node_fix.json`。

**追加發現二（平台層根因 A）：n8n Code node 預設封鎖所有環境變數存取。** 實測診斷輸出：`typeof $env=object` 但 `$env.SUPABASE_SERVICE_KEY` throw `access to env vars denied`；`typeof process=undefined`（沙盒內根本冇 `process`）。即係話 Fat Mo 喺 Docker 加嘅 `SUPABASE_SERVICE_KEY` 極可能設定正確，但 n8n 層面禁止讀取，需另加 `N8N_BLOCK_ENV_ACCESS_IN_NODE=false` 解封。**呢個反過來解釋咗整單嘢嘅源頭**：當年開發者唔係疏忽先寫死 secret，而係試過讀環境變數失敗（原始碼嗰個防禦性 `try{process.env.X}catch{null}` 就係實驗殘跡）就退回硬編碼，種下 3 個月公開洩漏嘅根。

**追加發現三（平台層根因 B）：容器重啟觸發 n8n 無聲升級，令 `require('axios')` 由可用變崩潰。** Fat Mo 重啟後（`image: n8nio/n8n:latest`），`Smart Cache Strategist` 整個 execution 死喺該節點，錯誤 `InternalTaskRunnerDisconnectAnalyzer.toDisconnectError` + `N8N_RUNNERS_MAX_OLD_SPACE_SIZE`，同 D55（2026-08-02）記錄症狀逐字吻合。時序鐵證：同一節點 15:24 仲成功發出 `GET /products`（Supabase log 有紀錄），15:52 重啟後零 request 直接死。**修法**：棄用 `require('axios')`，改 n8n 原生 `this.helpers.httpRequest()`（task runner 下經 RPC 代理回主進程），實測修復後節點正常執行。注意此崩潰屬**進程級**，節點內 try/catch 捉唔到，故「有 Airtable fallback」嘅設計實際上完全冇生效。

**驗收（execution log 實證，非靜態覆核）**：透過 n8n REST API `GET /api/v1/executions/<id>?includeData=true` 逐節點讀取狀態。修復前 exec 5949：`[ERROR] Smart Cache Strategist`（task runner 崩潰）。修復後 exec 5951/5953：Smart Cache `[ok]`、執行推進至 `Supabase Mirror Prep` 並吐出我方植入嘅診斷訊息（見追加發現二）。證明 ①axios 崩潰已解 ②環境變數封鎖為當前唯一剩餘阻斷點。

**Fat Mo 加咗 `N8N_BLOCK_ENV_ACCESS_IN_NODE=false` 後嘅波折（另一個同本次修復無關嘅獨立故障）**：重啟容器時撞到 `gluetun-vpn`（VPN 網路容器，n8n 用 `network_mode: service:gluetun` 寄生喺佢個網絡）`/dev/net/tun` 裝置揾唔到，NAS 重開機後自然解決；但重開機後 Container Manager 逐個容器獨立啟動 `n8n-freehandsss` 持續報「容器必須至少加入一個網路」（stale network reference，疑似重開機後 gluetun 容器 ID 變咗，n8n 舊容器定義仍指向舊 ID）。解法：project 層面用「建立」（非個別容器「啟動」）重新讀 compose YAML 建立容器，兩個容器同步正常啟動。

**最終驗收（2026-08-11 10:07 UTC，端對端實測）**：探測單 `testprobe01` 完整 CREATE→DELETE 週期。`get_execution_log` 顯示 CREATE 執行鏈 `Supabase Mirror Prep`→`Supabase Active Switch`→`HTTP: Supabase Sync RPC` 全部 `[ok]`；DELETE 執行 `Mirror Delete to Supabase` 全部 `[ok]`，整個 execution `status: success`。直查 Supabase `orders` 表確認 `testprobe01` 嘅 `updated_at`/`deleted_at` 時間戳同執行時間吻合——**真實訂單同步已完全恢復**，事故正式收尾。

**發現一個獨立、不相關嘅小問題（收尾同日已修，見 D62續II）**：CREATE 執行鏈末端 `Send Profit Report`（Telegram 通知節點）報 `Bad Request: can't parse entities`（Markdown 特殊字元解析錯誤），與 Supabase 憑證/同步完全無關，純粹通知發送失敗、不影響訂單資料本身已正確寫入。

### D62 續II：Telegram Markdown 解析錯誤修復 + n8n 版本鎖定（2026-08-11，Fat Mo 授權「1及2項均一起處理它」）

**問題一：Telegram entity 解析錯誤**。根因：`Pack Telegram Data` 組裝 `Full_Message` 時直接內插 `Customer_Name`/`Order_ID`/`Product_Name`/`Update_Note` 等動態文字，未轉義 Markdown 特殊符號；探測用客人名 `Credential_Probe` 含單一底線 `_`，Telegram 解讀為未配對斜體 entity 開始符，拒收成個訊息。

**修復嘗試一（失敗，記錄防重踩）**：先試喺 `Send Profit Report`/`Auditor Alert`/`Notify Telegram (Delete)`/`Send Test Summary` 四個 telegram sendMessage node 嘅 `additionalFields` 加 `parseMode: 'none'`，經 workflow PUT API 套用。**實測完全無效**——同一錯誤原封不動重現，證實呢個值喺目前 n8n 版本唔會被識別為「停用解析」（未進一步查證正確欄位值，改用更可靠嘅源頭修法）。

**修復嘗試二（成功）**：`Pack Telegram Data` 新增 `escMd()` 函式，喺源頭對 4 個動態欄位逐個 `_ * \` [` 符號前加反斜線轉義，經 `update_node_code` dry-run 預覽後正式寫入。**驗收**：探測單完整 CREATE→DELETE 週期，兩個 execution 均 `status: success`（之前 CREATE 恆定卡喺呢一步）。

**問題二：n8n 版本鎖定**。當前 Docker `n8n:` service 用 `image: n8nio/n8n:latest`，正正係 D62 續嗰輪「容器重啟＝無聲升級」故障嘅成因。因 AI 冇 NAS 檔案系統直接寫入權限，交俾 Fat Mo 手動執行：`docker-compose.yml` 嘅 `image` 改為 `n8nio/n8n:2.7.5`（即今日全部修復已驗證穩定嘅版本）→ project 層面「建立」重新部署。過程一度 YAML 貼錯（誤將 AI 提供嘅解說註解文字一併貼入檔案，產生重複 `n8n:`/`image:` key），已由 AI 指出具體錯誤行 + 提供精確修正後解決。

**最終驗收（2026-08-11 10:27 UTC，鎖定版本後）**：探測單 CREATE→DELETE 兩個 execution 均 `success`；Supabase `orders` 表 `testprobe01` 時間戳吻合。**D62 事故連同兩項收尾追加問題（Telegram 解析錯誤、版本鎖定）全部完成，狀態：已解決。**

### D63：學習經驗 + 四項技術性防線落地（2026-08-11，Fat Mo 要求「學習今次經驗並優化，審視 /fhs-check 是否需要更新」）

**要求本質**：Fat Mo 明確要求技術性防護，非單靠寫教訓文件俾人記得。逐一審視 D62 lesson「防再犯」四點原則，發現部分只講咗方向、未落地，本輪補齊：

1. **`Maintenance_Tools/run_all.py` 新增 DEGRADED 偵測層**：`DEGRADED_MARKERS` 掃描子腳本 stdout（"never appeared in Supabase" 等 3 個字串），命中即使 exit code=0 都標記 🟡 DEGRADED，同 🔴 FAIL 一齊阻擋整體 PASS——直接堵死 D62 事故「輸出有警告但總結全部通過」嘅確切成因。

2. **`Maintenance_Tools/FHS_Full_System_Test.py` 真正邏輯 bug 修正**：揪出確切根因——`if created and not wait_for_order_state(...)` 呢個 gating，CREATE 驗證失敗時會短路跳過 DELETE 驗證，仲印假嘅「Cleanup verified」+ exit 0。改為 CREATE 失敗即 `SystemExit(1)`。**注意**：`FHS_System_StressTester.py`/`FHS_Comprehensive_Test.py` 有相同 pattern，但涉及故意測試 failsafe 嘅案例（`created=False` 可能係預期行為），屬產品語義問題，AI 冇拍板——只喺 `run_all.py` 聚合層加 DEGRADED 兜底，冇動呢兩個腳本嘅個案邏輯，留待 Fat Mo 定義預期行為後再處理。

3. **`n8n-mcp-server/src/tools/update-node-code.js` 加密鑰遮蔽（真正洩漏源頭）**：`backupNode()` 用 `fs.writeFileSync` 寫節點原始碼落 repo，完全繞過 `pre-tool-guard.js`（MCP server 係獨立子進程，唔經 Claude Code 工具層 hook）。新增 `redactSecrets()`（同 pre-tool-guard.js Rule 2 同一套 pattern）喺寫檔前掃描遮蔽。**限制**：MCP server 進程全程運行緊，patch 要下次連接才生效，本 session 內未能實測驗證。

4. **`scripts/hooks/pre-tool-guard.js`**：Rule 2 頂部加互相參照註解，指向 `update-node-code.js` 嘅 `SECRET_PATTERNS`，防止兩處 pattern 清單日後只改一邊、產生防護落差。

**驗收**：`run_all.py`/`FHS_Full_System_Test.py` 兩者 `python -c "import ast; ast.parse(...)"` 語法檢查 PASS；`update-node-code.js` `node --check` 語法檢查 PASS + 獨立單元測試確認 `redactSecrets()` 正確遮蔽樣本密鑰、不影響其餘文字。完整 `python run_all.py` 端對端跑一次覆核新邏輯喺真實環境下行為正常（見下方驗收結果）。

**教訓（元層級）**：寫低教訓同真正落地防線係兩件事——D62 lesson 原本「防再犯」清單已預見呢 4 個修法方向，但停留喺文字建議層面。Fat Mo 主動追問先逼出呢個落差，未來完成事故收尾後應主動自問「呢啲建議有冇真正變成代碼」，唔應該等追問先做。

### D63 續：新防線即時捕獲一個獨立既有 RPC bug（migration 0087，2026-08-11）

**新防線第一次執行就見效**。加咗 DEGRADED 偵測 + 修正短路邏輯後首次跑 `run_all.py`，LIFECYCLE **首次真正 FAIL**（此前一直假 PASS），暴露一個同 D62 完全無關、更早存在嘅 RPC 邏輯 bug：

**Bug**：`sync_order_to_mirror` 嘅 `ON CONFLICT (order_id) DO UPDATE SET` 清單有 17 個欄位，**唯獨冇 `deleted_at`**。任何 order_id 一旦被軟刪除過，之後重用同一 ID 建立新訂單時 `deleted_at` 保持舊值 → 新訂單永遠卡喺「已刪除」，前端所有 `deleted_at=is.null` 過濾嘅查詢（訂單總覽/財務/交付提醒）都睇唔到。實證：`test9999003` `updated_at=2026-08-11` 但 `deleted_at=2026-08-03`。

**影響範圍實測**：全庫 58 筆訂單、6 筆有 `deleted_at`，其中 `updated_at` 明顯晚於 `deleted_at`（即撞到 bug）**僅 1 筆且係測試單本身，真實客戶訂單零受影響**——正常業務極少重用已刪除 order_id，只有固定 ID 反覆跑嘅測試腳本會命中。

**修復**：migration `0087_sync_order_to_mirror_reset_deleted_at.sql`，UPDATE SET 加 `deleted_at = NULL`。語義正確性：走到此 RPC 即代表 n8n 收到 create/edit（delete 分支走獨立 `Mirror Delete to Supabase` node 唔經此 RPC），故無條件清 `deleted_at` 成立。

**過程自我修正（誠實記錄）**：首版 migration 手抄時**擅自加咗 `accessory_cost` 欄位**（誤以為 live 版本已含），實測 `pg_get_functiondef` 確認 live 版本 `accessory_cost` 出現次數 = 0 後即刻作廢重做。改為**程式化生成**：由 live 定義以單一錨點插入 `deleted_at` 一行，並程式驗證「除該行外逐字不變」，杜絕手抄漂移（呼應 `feedback_migration_repo_db_drift` 教訓——`CREATE OR REPLACE` 係全量覆蓋，手抄漏一個欄位就會靜默打回舊版）。

**驗收**：`apply_migration` 成功 → 重跑 `FHS_Full_System_Test.py`（用返有殘留 `deleted_at` 嘅 `test9999003`）由 FAIL 變 **PASS**，「did not appear in Supabase」訊息消失，端對端證明修復生效。另修 `FHS_Full_System_Test.py` 缺少 UTF-8 stdout 包裝（新加嘅中文 FATAL 訊息喺 Windows CP950 下顯示成亂碼）。

**元教訓**：呢個係「加防線嘅即時回報」最好例證——防線本身唔止防未來，仲會即刻照出過去一直存在但被假 PASS 蓋住嘅問題。修 `/fhs-check` 判準 ≠ 淨係修個報告，係修返「睇唔睇得到真相」嘅能力。

### D63 續II：追查剩餘 DEGRADED，再揪出兩件事（2026-08-11）

修完 0087 後 LIFECYCLE 轉 PASS，但 STRESS/ACCEPTANCE 仍 DEGRADED。**唔可以就咁收貨——長期恆定 DEGRADED 等同恆定 PASS，一樣會造成警報疲勞（正正係今次事故嘅失效模式）**，故逐個案追查：

**① 落地延遲排除（實測，非估算）**：寫計時探針量度 create→落地耗時 = **3.6 秒**，遠低於腳本 20s 輪詢窗口，排除「逾時窗口太短」假設。同時發現 18:36 嗰次 STRESS 報 `test1001` never appeared 係 **0087 修復前嘅遺留現象**（`deleted_at` 永遠清唔走 → `not is_deleted` 永假 → 判定未落地），修復後 `test1001`/`test1004`/`test1005` 均正常落地。

**② `test2001`/`test2002`（ACCEPTANCE）真因：測試夾具過時，非生產 bug**。n8n execution log 顯示 `HTTP: Supabase Sync RPC` 報 `insert or update on table "order_items" violates foreign key constraint "order_items_product_sku_fkey"`。`FHS_Comprehensive_Test.py` 用緊 `金屬鎖匙扣 (不鏽鋼)` 呢個舊格式品名——注意仲係「不**鏽**鋼」，同現行目錄嘅「不**銹**鋼」係唔同字——該 SKU 早已不存在於 `products` 表，FK 拒絕寫入。**即係話 ACCEPTANCE 測試一直靜靜地失敗緊，被舊版腳本吞咗**。屬測試資料維護問題，需 Fat Mo 決定係更新夾具品名定係保留作 FK 防線測試，AI 未自行改動。

**③ 自我修正：我自己 D63 改嘅 `Smart Cache Strategist` 有手尾**。當時（`$env` 封鎖問題未確診前）保留咗 `process.env` 讀 key，但之後已證實沙盒內 `typeof process === 'undefined'`，令 `SUPABASE_KEY` 恆為 null、每次都靜默 fallback 落 Airtable（execution 顯示 `supabaseFetched=false`）——功能上唔算壞（Airtable fallback 係原設計），但完全冇行到預期嘅 Supabase 路徑，白費咗 D62 成輪修復。改為優先 `$env`、保留 `process.env` 作次要路徑。**驗收**：修復後 execution 顯示 `supabaseFetched=true, costKeys=2`，確認真正行 Supabase 查詢，順帶減少 Airtable API 用量。

**教訓**：「唔好收貨於一個恆定 DEGRADED 嘅健檢」本身就係今次最重要嘅紀律——如果我當時見 LIFECYCLE 轉 PASS 就收工，就會漏咗 ② 同 ③ 兩件事，而 ③ 更加係我自己嘅改動留低嘅缺陷。任何新加嘅警報，第一次觸發時必須逐個案追到底，確認「呢個警報係咪指向真問題」，否則佢好快會變成背景噪音。

詳見 `FHS_System_Logic_Overview.md` §5.4.11、`.fhs/notes/session-log.md` 2026-08-11 條目。

### D63 續III：STRESS/ACCEPTANCE 剩餘 DEGRADED 收尾——實測定案 expect_landing 語義（2026-08-11）

Fat Mo 授權「你幫我判斷，如果冇用或過時就刪除」，回應先前提出「TC-02~05／Test B 嘅預期落地行為屬產品語義、AI 唔應該拍板」呢個懸而未決事項。**判斷結論：唔刪任何測試案例**——TC-02~05 同 Test B 全部測緊仍然有效嘅防護行為（空品項/未知SKU/型別污染/缺主資料/假SKU），並非過時；唯一真正過時嘅係 ACCEPTANCE Test A 硬寫嘅品名 `金屬鎖匙扣 (不鏽鋼)`，全目錄搜尋確認 0 命中（已停產或命名規則已改），觸發 `order_items_product_sku_fkey` 外鍵違反，令 Test A 一直靜靜失敗被舊腳本吞咗。

**方法論：唔憑猜測定案，逐個案直打 webhook 實測真實行為**（`probe_edge_cases.py`，跑完自動清理）：

| 案例 | 實測結果 |
|---|---|
| TC-02 Empty Items | 冇落地（failsafe 正確拒收） |
| TC-03 Unknown SKU | 冇落地（未知 SKU 正確拒收，非靜默生成 $0 訂單） |
| TC-04 Polluted Data Types | **有落地**（字串型數字容忍轉型成功，`total_cost=-20.0` 符合鎖匙扣運費扣減機制） |
| TC-05 Missing Main Info | **有落地**（缺 Customer_Name/Deposit 正確 fallback） |
| Test-B Unknown SKU | 冇落地（同 TC-03 一致） |

**執行**：
1. `FHS_Comprehensive_Test.py`：修正 Test A 品名為現行目錄確認存在嘅 `嬰兒鎖匙扣 - 不銹鋼`；新增 `EXPECT_LANDING` dict，cleanup 邏輯改為比對「實測結果 vs 預期」，只有唔符先印 `[WARN]`（會觸發 `run_all.py` 嘅 DEGRADED 偵測），符合預期印 `[EXPECTED LAND]`/`[EXPECTED REJECT]`（唔會觸發）。
2. `FHS_System_StressTester.py`：同款手法，5 個案例全部有咗實測定案嘅 `EXPECT_LANDING` 值。

**驗收**：`python -c "import ast; ast.parse(...)"` 兩檔語法 PASS；跑完整 `run_all.py` 確認最終 Health Report 狀態（見執行記錄）。

**設計原則（留俾未來擴充）**：`EXPECT_LANDING` 用 `.get(order_id)` 取值、`None` 時維持原有「純資訊性 SKIP CLEANUP」行為——日後新增測試案例如果未及時定義預期值，唔會誤觸發 DEGRADED，只會保持沉默直至有人補上定義，避免「未分類 = 當作異常」嘅誤判。
...
### D67：`save_structured_order_items` RPC DELETE+INSERT 漏 14 個成本/V2 欄位（migration 0089）+ 前端連帶修復（2026-08-19）

**緣起**：Fat Mo 要求修 `task_4a9acd82`（`loadMode2Items()` select 漏 `accessory_cost`）同 `task_0c9d1c51`（hook regex 漏同一欄位）。查證 `loadMode2Items()` 期間發現，呢兩個 chip 描述嘅其實只係冰山一角——真正寫入路徑 `save_structured_order_items` RPC（Mode 2「儲存明細」掣觸發）用 `DELETE FROM order_items` 再 `INSERT` 重寫成張單品項，但 INSERT 欄位清單只得 14 個（`item_key`/`product_sku`/`item_category`/`quantity`/`engraving_text`/`specification`/`item_base_cost`/`subtotal_cost`/`handmodel_cost`/`keychain_cost`/`necklace_cost`/`batch_number`/`process_status`），漏埋其後（D46/D64/D65）陸續加入嘅 14 個欄位：`accessory_cost`、`drawing_cost`、`printing_cost`、`chain_cost`、`shipping_cost`、`item_sale_price`、`position_code`、`drawing_waived`、`drawing_charged_count`、`cost_model_version`、`family_member_config`、`reference_image_url`、`ai_suggestion`、`precomplete_status`。任何一次經 Mode 2 儲存（哪怕淨係改刻字/數量），呢 14 個欄位會被靜默清零/NULL。

**風險等級核實**：查全庫 133 個品項，`accessory_cost`>0 嘅 3 筆同 V2 欄位缺失嘅 0 筆完全冇重疊——呢個 RPC 從未喺任何有相關資料嘅訂單度真正執行過，屬**未爆地雷**而非已發生嘅意外，唔需要 backfill。

**Fat Mo 裁決**：全部一次過修（RPC + 前端 + hook），唔留手尾。

**修復一（核心）**：`migration 0089_save_structured_order_items_full_field_preserve.sql`——`v_prev_map` 由「淨存 `batch_number`/`process_status` 兩個」擴充做「存整行快照」，INSERT 時對 Mode 2 UI 本身唔會編輯嘅全部欄位一律 `COALESCE(新值, 快照值)`。沿用 0087/0088 防漂移先例，`pg_get_functiondef()` 攞 live 定義做 base。

**Smoke test 揪出一個真 bug（非純理論，非純讀碼宣告完成）**：`v_prev_map` 用 `to_jsonb(reference_image_url)` 儲存 array 欄位，若原值為 SQL NULL，`to_jsonb(NULL::text[])` 會存成 JSON `null`（scalar），令 `jsonb_array_elements_text()` 對其求值直接 `22023` error（"cannot extract elements from a scalar"），即使包喺 `COALESCE` 入面都會爆——`COALESCE` 唔係短路控制流，兩個分支都會被求值。修法：先用 `jsonb_typeof(...) = 'array'` 守衛，非 array 一律當 NULL。

**驗證（合成測試單，非真實客戶資料，已清理）**：①改刻字場景——`engraving_text` 正確改咗，其餘 9 個財務/V2 欄位（`accessory_cost`/`drawing_cost`/`drawing_waived`/`position_code`/`cost_model_version`/`family_member_config`/`ai_suggestion`/`precomplete_status`）全部原封不動保留；②array 型別 edge case——`reference_image_url` 有真實陣列值時再改一次數量，陣列同 `accessory_cost` 皆正確保留。

**修復二（前端，發現更早、更根本嘅既有 bug）**：`saveMode2Items()` 嘅 `prev` 來源係 `globalOrders`（`sbFetchItems()` select 唔含任何成本/V2 欄位，`prev.X` 恆為 `undefined`），舊碼 `prev.handmodel_cost != null ? prev.handmodel_cost : 0` 令**每一次** Mode 2 儲存都恆定向 `handmodel_cost`/`keychain_cost`/`necklace_cost` 三個欄位送明確 `0`。migration 0089 之前 RPC 本身冇 fallback 機制，呢個送 0 嘅行為一直照單全收（無 backfill 需要，理由同上——全庫零真實訂單經呢條路徑存過非零成本）。但若唔修呢處，migration 0089 嘅 `COALESCE` 保護對呢 3 個欄位（同新加嘅 `accessory_cost`）完全失效——`COALESCE` 見到明確 `0`（非 `NULL`）唔會 fallback。修法：`newItems` 建構全部 pass-through 欄位改送 `null`（非 `0`/`''`），令 RPC 端 `->>'field'` 正確讀到 SQL NULL 先觸發 fallback。同步喺 `loadMode2Items()` select list 加齊 14 個欄位（前端唔會用，但保持同 RPC 消費者定義一致）。

**修復三**：`task_0c9d1c51`——`pre-tool-guard.js` R11 同 `post-tool-kgov.js` `FINANCE_CONTENT_PATTERNS` 兩個 regex 加入 `accessory_cost`。
...
**運行驗證（同日 /fhs-check 觸發 18 個新 execution 完成）：16 個成功 execution 零 `supabaseKey`／零 `sb_secret_`，Mirror Prep 輸出鍵只剩 `rpcPayload`/`supabaseActive`，`HTTP: Supabase Sync RPC` 成功 6 次；2 個失敗 execution（7526/7534，`/fhs-check` 內建「Unknown SKU」預期拒絕測試，FK `order_items_product_sku_fkey` 409）仍存 `apikey` header＝路徑 B 實證未堵，已刪除；**每次 `/fhs-check` 都會產生約 2 個此類含 key 失敗 execution，credential 化前需事後清理**。先前「22 個失敗 execution 可能有單冇入 Supabase」推測大概率係同類測試單（記錄已刪，無法逐個證實）。
**新發現（另案，唔屬今次授權範圍，只記錄）**：另外 4 個 active workflow 定義內仍硬編碼**已撤銷舊 key**（指紋 `8fdf055d9e`，實測 401）：`FHS_Financial_Overview`（3 節點）、`FHS_Query_GlobalReview`（2 節點）、`FHS_IGWatchdog_DriveWatch`（2 節點）、`FHS_System_ErrorMonitor`（1 節點）。舊 key 已死故無 secret 風險，但前兩者 execution 持續 error（Fetch Orders/Items (Supabase) 401，共 70 個失敗記錄），功能實際已壞；87 個含舊 key 嘅 execution 保留未刪（無風險）。修法同本次（改 `$env`/credential），待 Fat Mo 決定。
**Subagent 使用記錄**：❌未使用（全程 curl + Python 指紋比對直接查證，避免 key 值進入任何 subagent 上下文）。

### D79 續：2026-09-20 — 另外 4 個 workflow 仍寫死已撤銷舊 key 修補（Fat Mo 授權「立即處理」）

**發現**：D79 掃全伺服器 execution 時揪出（fp `8fdf055d9e`＝D62 已撤銷舊 key，實測 401「Unregistered API key」）：D62 只收斂 `FHS_Core_OrderProcessor`，其餘 4 個 active workflow 漏咗——`FHS_Financial_Overview`（HTTP 節點 header ＋ 2 個 Code 節點 `process.env … || '舊key'` fallback）、`FHS_Query_GlobalReview`（2 個 Code 節點同款 fallback）、`FHS_IGWatchdog_DriveWatch`（`Write Mismatches`/`Write Intents` 兩個 HTTP 節點）、`FHS_System_ErrorMonitor`（`Log to Supabase`）。實況：FO/GlobalReview 由 2026-09-07 起（保存記錄範圍）持續 401，Dashboard 09-19 仍有呼叫；**IGWatchdog 兩個寫入節點設 `continueOnFail`+`alwaysOutputData`，每日 401 被吞成「success」**（`message_intents` 最後寫入 2026-08-03、`content_mismatch` 2026-07-25）；`error_logs` 表由始至終 0 行（ErrorMonitor 從未成功記錄過）。

**已執行（API PUT，四欄，逐個回讀驗證）**：12 處改動／8 個節點——HTTP 節點 header 改 `={{ $env.SUPABASE_SERVICE_KEY }}`／`={{ 'Bearer ' + $env.SUPABASE_SERVICE_KEY }}`（同 IGWatchdog 內已正常運作嘅 `Write Alerts`/`Write Messages` 兄弟節點同款）；Code 節點 key 讀取改 `$env` 優先、`process.env` 次之、都無則 throw（**移除舊 key fallback**）。回讀：只目標節點參數變、連線不變、`activeVersion` 快照亦已清（整份 JSON 零 `sb_secret_`）、仍 active。副作用已還原：PUT 剔走 `availableInMCP` 令 3 個 workflow 由 true 被重設 false，已連同 `availableInMCP:true`（剔走 `binaryMode`）再 PUT 還原，設定同改前逐項一致。改前備份（含已撤銷舊 key，故只放 scratchpad 唔入 repo）；repo 副本 `n8n/FHS_Financial_Overview_workflow.json` 改為 live 快照（原副本係舊版設計）；`scripts/deploy_native_supabase_mirror.js`／`update_n8n_supabase_mirror.js` 舊 key fallback／literal 已移除。

**運行證據**：FO webhook（exec 7560）HTTP 200、3 節點全 success；GlobalReview webhook（exec 7561）HTTP 200、`Fetch Orders (Supabase)` 經 `$env` 讀 key 拉到 62 行（＝COST_INTEGRITY 62 張生產訂單）；兩個 execution 全 JSON 零 `sb_secret_`。`error_logs` 欄位（`occurred_at`/`workflow_name`/`error_message`/`node_name`）同 ErrorMonitor 寫入內容吻合。**未能即時驗證**：IGWatchdog（排程 06:00 HKT，下次 2026-09-21）兩個寫入節點與 ErrorMonitor（須有 workflow 出錯先觸發）——已知同款 `$env` header 喺同 workflow 內 `Write Alerts`/`Write Messages` 正常運作。

**風險註記**：呢 4 個 workflow 而家用現行 key，失敗時同 OrderProcessor 路徑 B 一樣會將 `apikey` header 存入 execution data（以前存嘅係死 key）；同樣要靠 n8n credential 化先可根治，已併入 D79 待辦（②）。**IGWatchdog 空窗**：`message_intents`／`content_mismatch` 自 08-03／07-25 後冇新寫入，已知 08-10 key 撤銷後每日被吞，修復後只寫當日新資料，歷史空窗不補（原始 Drive 內容是否須回補，待 Fat Mo 決定）。

**finance-auditor 獨立覆核（同日，背景派工，唯讀）— 結論 PASS-with-notes，認證改動放行**：A) 改前備份 vs live 逐節點比對：9 節點不變、`connections`/`settings` 相同，只 `Financial Aggregator`（header 值）、`Fetch Orders/Items (Supabase)`（僅 `const SUPABASE_KEY` 一段）有差異，冇財務邏輯被改；順帶確認 `FO Webhook` 直連 `Financial Aggregator`，Fetch 兩個 Code 節點唔喺 webhook 路徑上。B) 用 anon key 獨立重算（orders 76／items 156，排除已取消同已軟刪除；日期口徑用 live RPC 嘅 `LEAST(confirmed_at, appointment_at)`）：`current`/`monthly`/`yearly` KPI（revenue／cost／profit／orders／margin／aov）、環比同比、分類單量／數量／成本全部吻合（例：yearly revenue 204,736、cost 35,020、profit 169,716、57 單）；直接 anon 呼叫 RPC 與 webhook 回應除 `last_sync` 外完全相同＝n8n 原樣透傳；`last_sync` 新鮮＝真成功執行。C) 冇 NaN／負利潤／orders=0，兩個 null（`current.marginChange`/`aovChange`）係上年同期無單之預期。
