...
**驗證**：`node --check` 抽取全部 inline `<script>` 語法通過；本地 `npx serve` 起 Freehandsss_Dashboard 靜態伺服器，手機橫向寬度（900px）+ 真實 Supabase 快取資料（`window.globalOrders`，60 筆）實測——兩張與 Fat Mo 原始截圖相同嘅訂單（0600728 Dorothy $2,380/$210/$2,170、0650429 Shirley Lee $4,260/$770/$3,490）數值完全對應；`getComputedStyle` 直接量測三行 `.fhs-fin-amt` 均為 `12.5px`/`700` 完全一致；`toggleAuditMode()` 開關測試逐 SKU 明細正常顯示/隱藏；console 零 error。**[2026-09-06 D71-follow3 最終結論，推翻下方 follow2]**：下方 follow2 嘅兩個診斷（iOS 快取、WebKit 時序）**全部錯**。真因係 D72 跨分支覆寫事故——`order-overview` 分支早已將斷點由 768 改 750 修好 iPhone 橫向問題，而本分支部署時用舊血統覆寫咗佢。已 merge 該分支（117 commit）並喺正確血統上重做 D71：跟對方 `_FHS_TH_DEF` 宣告式欄位架構，新增 `fin` 欄取代 `inc`/`cost`/`profit`，colspan 由 `fhsOverviewColCount()` 自動計算；緊縮桌面 750–1129px 區釋放 73px 分俾 prod(84→130)/stat(88→115)，欄闊總和維持 745px。實測 758px：表格版正常、產品明細實際 142px、進度 125px、三行字體 12.5px/700 完全一致、零溢出、audit toggle 135 項明細正常、console 零 error。全文見 D72 條目。
