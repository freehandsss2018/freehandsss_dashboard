...
- Phase 3：✅ Codex 通用模板 v0.1.0 已匯出至獨立 repo；manifest 39/39 非 SKIP 映射，另 30 份模板附加檔，共 69 檔（Phase 4 盲測補 SOP_NOW、Node SessionStart 及 .gitignore）。匯出後黑名單 0 hits（合成正例 PASS）；README 53/150 行、AGENTS skeleton 40/120 行。匯出後四套可攜 runner：guard 5/5、kgov 10/10、health 3/3、handoff gate 3/3；sample guard 規則檔與引擎一併匯出。新專案 scratch bootstrap、橋接生成和四套 runner 同樣 PASS。來源 commit 見模板 VERSION；Phase 4 待 fresh-context 盲測。
- Phase 4：✅ fresh-context 盲測 9/9 PASS-with-fixes；先揪出缺 SOP_NOW、Windows shell 依賴、離線 npm 強制安裝三項，Phase 3 修正後重測全過。完整命令與驗收邊界見 `.fhs/reports/completion/2026-09-26_s149-phase4-portability-dryrun_report.md`。
- Phase 5：✅ D97＋總完成報告已落盤；README、scripts README、repo-map、CLAUDE 路由、05 §7 模板 drift 健檢與 auto-memory 已同步。三項 guard 加固明列下次處理；回填律全域化只保留獨立提案，未自動生效。
