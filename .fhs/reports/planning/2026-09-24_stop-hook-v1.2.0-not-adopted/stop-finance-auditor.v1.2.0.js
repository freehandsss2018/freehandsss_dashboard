...
//
// Version: 1.2.0 | 2026-09-24（cl-flow-fast 2026-09-24-0651，Fat Mo 選方案 A + /execute 授權；
//                              補「純陳述句＋純 Edit/Write 寫財務文件」偵測缺口，D84）
// [前次] 1.1.0 | 2026-09-22（cl-flow-fast 2026-09-21-1536；修「lookback 5 輪內派過即無條件放行」缺口）
...
//
//   (f) v1.2.0 新增：Edit / Write / MultiEdit 寫入——純路徑＋字串存在性檢查，零語義判斷：
//       ① file_path 命中財務權威文件（FIN_DOCS，含 FHS_Cost_System_Overview）→ 直接算訊號；
//       ② file_path 為 .fhs/notes/decisions.md、.fhs/memory/handoff.md、根目錄 Changelog.md／CHANGELOG.md，
//          且「寫入內容」（Write.content／Edit.new_string／MultiEdit.edits[].new_string）含 FIN_COLUMNS → 算訊號。
//       原因：D83 重播揪出「純陳述句（無問號）＋純 Edit/Write 寫財務文件」嘅結論，(a)-(e) 全部捉唔到。
//   v1.2.0 設計決定（cl-flow-fast 2026-09-24-0651 A2 評審後）：
//     - 唔掃 old_string：handoff／decisions 嘅 Edit 慣常用含財務欄位嘅舊段落做錨點，掃 old 會近乎每次誤觸。
//     - 依據／豁免標記只認「回覆文字」（assistantText），寫入 new_string 內嘅標記唔算，
//       否則 AI 可喺被審查檔案自身塞豁免＝自我放行。
//     - 寫入內容一律經 writtenText() 型態防禦；financeSignal 內拋錯會被頂層 catch 靜默放行＝整輪偵測失效。
//   已知未覆蓋面（誠實記錄，D84）：①純刪除財務紀錄（new_string 無財務欄位）②Bash／Python／PowerShell 寫檔
//     ③subagent 側鏈（isSidechain）代寫 ④豁免／依據標記嘅真偽無法機驗（靠 AI 紀律＋Fat Mo 事後抽查）。
//
...
const FIN_DOCS = /(FHS_Finance_Bible|FHS_Pricing_Bible|FHS_Product_Cost_Schema|FHS_Cost_System_Overview|finance-gatekeeper[\\/]SKILL\.md)/i;
// v1.2.0：紀錄檔只限 .fhs/ 下嘅 decisions／handoff 同根目錄 Changelog（兩種大小寫都有追蹤）；`$` 錨定令 .tmp 等後綴唔命中
const LOG_DOCS = /([\\/]\.fhs[\\/](notes[\\/]decisions|memory[\\/]handoff)\.md|(^|[\\/])(Changelog|CHANGELOG)\.md)$/;
// 測試夾具／第三方套件內嘅巢狀 .fhs/ 同名檔唔係真紀錄檔（J7/J8 實測：health-fixtures 內有 .fhs/memory/handoff.md）
const NOT_REAL_LOG = /[\\/](node_modules|health-fixtures)[\\/]/;
...
// 只取「新寫入」內容；非字串／非陣列一律降級空字串，唔拋錯（A2 批評#2）
function writtenText(tu) {
  const input = tu.input || {};
  const str = v => (typeof v === 'string' ? v : '');
  if (tu.name === 'Write') return str(input.content);
  if (tu.name === 'Edit') return str(input.new_string);
  if (tu.name === 'MultiEdit') {
    return Array.isArray(input.edits) ? input.edits.map(e => str(e && e.new_string)).join('\n') : '';
  }
  return '';
}
...
function financeSignal(tu) {
  ...
  if (WRITE_TOOLS.test(tu.name || '')) {
    const fp = typeof input.file_path === 'string' ? input.file_path : '';
    if (FIN_DOCS.test(fp)) return '寫入財務權威文件';
    if (LOG_DOCS.test(fp) && !NOT_REAL_LOG.test(fp) && FIN_COLUMNS.test(writtenText(tu))) return '寫入紀錄檔含財務欄位';
  }
  return null;
}
