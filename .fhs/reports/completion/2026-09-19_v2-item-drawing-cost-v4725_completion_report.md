...
- **Dashboard 舊語義未改（2026-09-20 已評估，結論：暫不改）**：V42 `calculatePricing()` 畫圖成本估算仍沿用 S55（生產 HTML，另案）。V2 品項而家由 n8n 決定 `Drawing_Cost`，前端值被忽略，入帳／KPI 無影響。前端舊語義而家只餵兩處「估算」：①報價面板「系統成本」（`_totalCostNew`＝畫圖＋打印＋頸鏈＋環扣−運費扣減；3 張實單面板 $995／$650／$440 對 n8n 實際成本 $1,425／$1,040／$850，差 $430／$390／$410，其中舊語義造成 $120／$60／$120，其餘約 $290–$330 係面板本身唔計立體擺設 $210 等既有設計）；②n8n `Profit Auditor` anti-loss 守衛（只喺收款低於成本先報警；實單售價係成本約 4–5.6 倍，冇實際影響）。改動要動 `calculatePricing()`（生產 HTML）＋審查＋`/upload-web` 三關＋人手測，改完面板仍差約 $300，故不值得；若想面板成本可信，應另開項目令面板數字與 n8n 同一公式。
