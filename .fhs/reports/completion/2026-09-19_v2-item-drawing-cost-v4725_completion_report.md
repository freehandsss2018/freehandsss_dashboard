...
- **Dashboard 舊語義未改（Fat Mo 未裁決；2026-09-20 finance-auditor 核實後更正早前評估）**：V42 `calculatePricing()` 成本估算（`_totalCostNew`＝畫圖＋打印＋頸鏈＋環扣−運費扣減）仍沿用 S55 舊畫圖語義（生產 HTML，另案）。V2 品項而家由 n8n 決定 `Drawing_Cost`，入帳／KPI 無影響。
  - **性質**：Fat Mo 2026-06-03 確認該輸出＝「供操作者參考嘅預算估算，非確收數字」，n8n 擁有成本（`decisions.md:2598-2631`、AGENTS.md:86、Finance Bible §一）；2026-07-21 Fat Mo 將該數字 UI 隱藏（commit `aa12f5e`：`#drawingCost` `display:none`，成本／利潤顯示改由「核對訂單」負責，裁決原文「for quoting only … owned by the Audit Ledger」）。
  - **命名確實誤導過（2026-09-20 第二次 `finance-auditor` 覆核揪出，更正我早前講法）**：隱藏前嗰個徽章嘅文字係「**畫圖成本: $X**」（V42.html:5969），但寫入佢嘅值係 `_systemTotalCost`＝**全成本估算**（畫圖＋打印＋頸鏈＋環扣−運費扣減，V42.html:9989）——名實不符，由 2026-06-02（commit `aae874a`）起到 2026-07-21（`aa12f5e`）存在約 7 星期。我早前答「冇呢個介面標籤」係將「而家隱藏咗」講成「從來唔存在」，掩蓋咗真正嘅命名問題。同日 commit `7fea544` 亦刪走咗全系統唯一一句向操作員解釋兩套計算嘅 UI 文字（「成本估算已含打印/環扣/運費（後台記帳由 n8n 負責）」），今日 UI 零解釋。
  - **消費者**：只有 n8n `Profit Auditor` V45.8（只喺收款低於呢個估算先 Telegram 警報；62 張單中 29 張估算為 0 而跳過，冇一張收款低於 n8n 成本，最低售價／成本比 2.34；現存單 0 張符合觸發條件，D79 已刪 92 個 execution 故證明唔到「從未」；V45.8 由 AI 於 2026-03-27 實作，冇 Fat Mo 裁決紀錄）＋ `raw_form_state.__System_Total_Cost`（冇人讀，Field Map:275 標明「參考用，非寫入來源」）。⚠️ **但「前端成本估算＝純參考、零 DB 足跡」係錯嘅框架（2026-09-20 更正）**：訂單層 `System_Total_Cost` 的確只餵 Profit Auditor，但**品項層四分量**（`Drawing_Cost`／`Printing_Cost`／`Chain_Cost`／`Shipping_Cost`）由前端 `calculatePricing()` 計出、經 n8n 透傳，**真實寫入 `order_items` 對應欄位**（live：printing 58 行、shipping 58 行、chain 59 行、drawing 71 行非零）。其中 chain 自 V47.20、V2 drawing 自 V47.25 已由 n8n 收回自算，printing／shipping 仍係前端值。
  ...
  - **「前端同 n8n 分開計」有記錄，但批准情況要如實寫**（2026-09-21 兩輪 fresh-context 核對後更正：我先前寫「有裁決／權威翻轉」過頭，其後又矯枉過正定性 S60 為「AI session 決策」、06-03 為「唔係改制」，同樣過頭）：
    - 2026-03-22（`Changelog.md` V35.0，搜「前端報價優先」）前端 `System_Total_Cost` 曾成為利潤結算基準（Antigravity 改動）；03-24 因未授權整體重寫還原 19 節點穩定版；03-27 `Profit Auditor` 檔頭已寫「system price is reference only」。2026-05-16 Finance Bible 首版（commit `0fcc9ea`）已寫明「成本計算引擎＝n8n Layer 2」。
    - **2026-06-02 S53**：`calculatePricing()` 確立為唯一成本計算權威（cl-flow／完工報告記「已獲 Fat Mo 確認」，AI 撰寫、原話查唔到；同時將「n8n 信任前端成本」列 deferred）。
    - **2026-06-03 Fat Mo 確認**（`decisions.md` 該段標題為「AI 過失記錄」）：成本側歸 n8n、前端輸出＝參考預算估算；S57 據此撤回 B2「n8n 信任前端四分量」（批准者未註明）。同 S53 方向相反；係「糾正 AI 誤讀」定「改制」，repo 兩種講法都有、冇明文定案；Finance Bible（L1）自 05-16 冇改過。
    - **2026-06-05 S60**：品項四分量前端透傳，理由 (3)「n8n 拿不到部位級資料，無法重算 drawing 豁免邏輯（最高頻財務雷）」；其 (4)「n8n 信任前端成本分量」措辭同 S57 相反。**批准者查唔到**：cl-flow（`artifacts/2026-06-05-0136`）標明「需 Fat Mo 拍板」，repo 冇拍板記錄，之後交接稱已部署。
    - **2026-06-27 S125**：品項層四欄正式廢欄（保留欄位不 DROP、停止補寫投資；`FHS_System_Logic_Overview.md`「kgov 同步點」段，搜「正式廢欄」）；決策者未註明（handoff 列為已定決策，`decisions.md` 無條目）。D80（2026-09-19）為 V2 `drawing_cost` 補寫，與 S125「停止補寫」**方向相反／有張力**（D80 決策時未提 S125；V2 係 S125 之後新增路徑）；`printing／shipping` 仍係前端值。我早前寫「S125 只裁決 DB 四欄」係低估咗佢對提議「收歸 n8n」嘅牽連。
    - **「較易維護」查唔到任何記錄**（全 repo grep 易維護／方便維護／maintainab，唯一中文命中係財務總覽 KPI 方案比較，與此無關）；「離線」亦查唔到（`calculatePricing()` 反而要等 Supabase `cost_configurations` 載入，V42.html:9590-9594）。
    - **原未裁決（兩件；2026-09-21 Fat Mo 決定唔改，見 decisions D82）**：①前端成本估算（`System_Total_Cost`／賠本守衛）保留定退役；②品項四分量去向（S125 廢欄／D80 補寫／收歸 n8n）。AI 評估「暫不改」冇裁決支持亦冇裁決反對。端到端總覽見 `.fhs/notes/FHS_Cost_System_Overview.md`。
    - ⚠️ **S60 (3) 嘅技術前提今日已部分失效**：n8n 自 V47.22 起靠 `position_code` 拎到部位資料，V47.25 起 V2 品項畫圖費已由 n8n 自算、忽略前端值（§5.4.23）；但部位解析只涵蓋非家庭 V2 鎖匙扣／吊飾，唔含家庭組合同立體擺設主件。
