...
- **「嬰兒」與「大寶」的定義**（2026-08-22 Fat Mo 口述成文，此前三份權威文件皆從未記錄）：
  - **嬰兒**＝初生，或客人的**首個**孩子
  - **大寶**＝客人的**第二個**孩子，即嬰兒的哥哥／姐姐
  > 「大寶」是**相對於「嬰兒」而存在的稱謂**，故「只做大寶、沒有嬰兒」在定義上不可能發生——若訂單中沒有嬰兒，那個被倒模的孩子本身就會被定義為「嬰兒」。因此**有大寶的訂單必然同時有嬰兒**，「所有產品必須圍繞嬰兒展開」在任何合法組合下皆不被違反，大寶**不構成本節的例外**。
  > ⚠️ 此定義是 §2.1 定價分級的根基（大寶＝「＋$300 修正項」而非獨立產品線），亦是 2026-08-22 一次實作錯誤的根因——當時誤把新價綁在「零嬰兒＋有大寶」這個定義上不存在的組合，令新價永遠不會觸發。事故全文見 `FHS_System_Logic_Overview.md` §5.4.20。
...
| 玻璃瓶套裝 (4肢) | **嬰兒 4肢**，無大寶、無父母 | **$1,680** |
| 玻璃瓶套裝 (2肢) | **嬰兒 1–3肢**，無大寶、無父母 | **$1,380** |
| **玻璃瓶套裝 (4肢+大寶)** | **嬰兒 4肢** ＋**有大寶參與**（不論大寶幾多肢），無父母 | **$1,980**（2026-08-22 Fat Mo 定案；migration 0091）|
| **玻璃瓶套裝 (2肢+大寶)** | **嬰兒 1–3肢** ＋**有大寶參與**（不論大寶幾多肢），無父母 | **$1,680**（2026-08-22 Fat Mo 定案；migration 0091）|
| **玻璃瓶套裝 (家庭)** | **倒模對象含父母**（不論嬰兒/大寶肢數）| **$2,580** flat（2026-07-19 Fat Mo 定案，先例單號 0600107；migration 0060） |

**倒模對象組合窮舉**（2026-08-22 Fat Mo 逐格定案，玻璃瓶；✅＝可落單、🚫＝不可能）：

| 嬰兒 | 大寶 | 父母 | 判定 | 售價 |
|:---:|:---:|:---:|---|---|
| ✅ | — | — | 純嬰兒 | $1,380／$1,680 |
| ✅ | ✅ | — | **嬰兒＋大寶** | **$1,680／$1,980**（＋$300）|
| ✅ | — | ✅ | 家庭 | $2,580 flat |
| ✅ | ✅ | ✅ | 家庭 | $2,580 flat |
| — | ✅ | — | 🚫 **定義上不可能**（見 §0）| 由「未選嬰兒肢體」提醒接住，不阻擋 |
| — | ✅ | ✅ | 🚫 | 硬阻擋，拒絕報價 |
| — | — | ✅ | 🚫 違反 §0 禁止單人成人 | 硬阻擋，拒絕報價 |

> **兩個含父母的 🚫** 由既有防呆負責（`calculatePricing()`：`en_parent` 已勾但零嬰兒肢體 → 🚨 拒絕報價、價錢歸零），2026-07-19 起已在生產環境運作，2026-08-22 未改動。
> **「有大寶、零嬰兒」的 🚫** 不設硬阻擋：它是**資料輸入錯誤**（該孩子應輸入為嬰兒）而非違規組合，故沿用 D65 既有的橙色「未選任何嬰兒肢體」提醒讓操作員自行更正，不阻塞流程。
> **大寶為訂單層角色**（D65 起），只在操作員指定的 owner 件評估，故多件單中只有 owner 件會拿到「＋大寶」價階，其餘玻璃瓶件仍計純嬰兒 2肢/4肢 價。

判定依據（**2026-08-22 修正**，取代 2026-07-21 版）：`type = babyLimbCount >= 4 ? "4肢" : "2肢"`——即**只數嬰兒肢體**（4肢先算4肢，1–3肢一律2肢）。
⚠️ **本次推翻 2026-07-21 定案**：原定「大寶肢體同嬰兒肢體同等地位一齊計總數」（`(babyLimbCount + elderLimbCount) >= 4`）。自 2026-08-22 起，大寶改為以「**有冇參與**」做 ＋$300 修正項，**不再影響 2肢/4肢 分級**。實例差異：嬰兒1肢＋大寶1肢，舊制＝2肢 $1,380，新制＝2肢+大寶 $1,680；嬰兒4肢＋大寶4肢，舊制＝4肢 $1,680，新制＝4肢+大寶 $1,980。父母肢體則自始不影響 2肢/4肢 判定。玻璃瓶一旦 `hasParentGlass`（`en_parent` 已勾且至少一個嬰兒肢體≠「無」）即改用**獨立 SKU「玻璃瓶套裝 (家庭)」**、售價 $2,580 flat，不再依 4肢/2肢分級——**大寶本身不會觸發家庭價**，只有父母會。
**2026-08-22 新增「＋大寶」價階**：若該件**有大寶參與**（`hasElder`＝`elderLimbCount > 0 && !hasParentGlass`，`elderLimbCount` 只喺 owner 件且 `en_elder` 已勾先計入），改用獨立 SKU「玻璃瓶套裝 (N肢+大寶)」，2肢 $1,680／4肢 $1,980，即同 tier 純嬰兒價 ＋$300。實作位置 `_pDeriveSkuName()`；價錢真源 `_pPriceOfSku()` 中**大寶判斷必須排在通用 4肢/2肢 之前**——SKU 名「玻璃瓶套裝 (4肢+大寶)」本身含「4肢」字樣，次序調轉會令有大寶嘅單靜默收返純嬰兒價（少收 $300）。成本不變（$210 flat），故此 ＋$300 全數落在淨利潤，屬定價策略非成本差異（Fat Mo 2026-08-22 確認）。
