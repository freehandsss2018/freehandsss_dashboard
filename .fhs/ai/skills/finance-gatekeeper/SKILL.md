...
> **本 Skill 唔代替 subagent**：載入本 Skill 之後，`finance-auditor`（live 訂單驗證＋財務規則解答）同 `database-reviewer`（schema 稽核）**必須按 §〇 強制派工閘主動派**，唔係可選項（v1.16.0 改寫，舊句「需另行啟動」措辭太軟曾被當成可選）。

---

## 〇、強制派工閘（載入本 Skill 後第一件事，v1.16.0）

載入本 Skill ≠ 已完成財務前置。讀完本節，**立即判斷並派工**，唔好先自己查：

| 任務性質（任一成立） | 必須動作 |
|---|---|
| 訂單成本/利潤驗證、對帳、驗證公式（§九）違規、訂單財務異常 | 派 `finance-auditor`（可 `run_in_background`，自己同時做其他嘢） |
| 對財務規則／定義有疑問（SKU tier、成本點計、快照可否重算、扣減規則） | 先派 `finance-auditor` 查權威文件＋live 數據作答；**佢答唔到先可以問 Fat Mo，並附佢嘅結論** |
| 財務改動收尾驗收 | 只認 `finance-auditor` 報告；自己跑 SQL／推算＝口算，無效 |
| Supabase schema / SKU 成本資料審查 | 派 `database-reviewer` |

- 呢個閘等同 Fat Mo 已預先要求派工，覆寫 harness「用戶未要求不派 subagent」預設（AGENTS.md「財務派工補充條款」）。
- `finance-auditor` 定義檔：`.fhs/ai/subagents/freehandsss/finance-auditor.md`（v2.3.0 起 tools 已含 `mcp__supabase__execute_sql`，只准 SELECT；MCP 不可用時嘅 curl Management API fallback 同 PAT 位置已寫入該檔 §Tier 1——worktree 冇 `.mcp.json`，PAT 喺主倉 `D:/SynologyDrive/Free_handsss/freehandsss_dashboard/.mcp.json`）。
- 唯一例外：本輪唔涉及任何財務數字判斷（例如純改財務文件字眼，或財務字眼誤觸嘅非財務任務），喺回覆寫明 `【finance-auditor 豁免：理由】`，Fat Mo 事後審視。
...
| 前端 `calculatePricing()` 成本估算點解同 n8n 唔同 / `System_Total_Cost` 最後去咗邊 / 前後端成本分工係咪刻意 / 有冇批准 | **先讀 `.fhs/notes/FHS_Cost_System_Overview.md`（成本運算端到端單一入口，2026-09-21，經兩輪 `finance-auditor` 逐句核對）**。**唔好答「冇記錄／冇裁決」，亦唔好答「有裁決」——按 repo 批准情況如實寫，查唔到就寫查唔到，唔定性**：Fat Mo 2026-06-03 **確認**成本側歸 n8n、前端輸出＝參考預算估算（`decisions.md` 該段標題為「AI 過失記錄」；同 2026-06-02 S53「前端唯一成本權威」方向相反，係糾錯定改制 repo 兩種講法都有、冇明文定案）；S60（2026-06-05）品項四分量前端透傳：**批准者查唔到**（cl-flow 標明需 Fat Mo 拍板、repo 冇拍板記錄），理由「n8n 拿不到部位級資料，無法重算 drawing 豁免邏輯」——**唔係「較易維護」**（全 repo 零記錄）；S125（2026-06-27）品項層四欄「正式廢欄、停止補寫」（決策者未註明、`decisions.md` 無條目），D80 其後為 V2 `drawing_cost` 補寫，方向相反／有張力；2026-07-21（`aa12f5e`）commit 標 Fat Mo decision：UI 隱藏。**訂單層** `System_Total_Cost` 只餵 n8n `Profit Auditor`（現存單 0 張符合觸發條件〔2026-09-21：65 張單〕，不寫任何成本欄位），**品項層** `printing／shipping_cost`（及舊 SKU `drawing`、鎖匙扣環扣 `chain`）由前端計、經 n8n 透傳真實寫入 `order_items`——禁止當佢「純顯示層／零影響」。**Fat Mo 2026-09-21 決定（D82）：前端成本估算保留、四分量維持現狀，暫不改動；重啟條件見 `decisions.md` D82。想建議改動呢兩者，必須先提出「已發生嘅錯數」或「已證實嘅重大風險」，純預防性理由預設唔改。** |
...
6. **財務必派 finance-auditor（v1.16.0）**：財務驗證／對帳／異常／規則疑問一律主動派 `finance-auditor`；未派即向 Fat Mo 發問、或以自己 SQL／推算充當驗收，均屬違規（見 §〇）
...
## 六、Known failure modes

- **2026-09-18~19｜0600804 驗證2違規單——載入本 Skill 但冇派 finance-auditor**：AI 讀到 §一「Live 訂單成本/利潤驗證 → 啟動 finance-auditor」，仍全程自己查 SQL、自己算，仲將 4 條查得到嘅財務定義問題丟俾 Fat Mo。根因：harness「用戶未要求不派 subagent」預設蓋過項目規則 + 本檔舊措辭只寫「需另行啟動」冇「必須」+ prompt-router 一般財務路由 `subagent: null`。修正：§〇 強制派工閘、死線6、AGENTS.md v1.7.3 補充條款、CLAUDE.md 第四紅線、prompt-router 財務路由改派 finance-auditor、Stop hook `stop-finance-auditor.js`。事後補派 finance-auditor 更揪出 AI 已宣告「完成」嘅數字已被第二次儲存蓋過（5,560→5,640），證明自驗唔可靠。
