...
├──────────────────────────────────────────────────────────────┤
│  ┌─ A. 繪圖成本 (4) ────────────────────────────────────┐    │
│  │  嬰兒/大寶 (S)    [_____60_____]  HKD   ✓ 已儲存     │    │
│  │  嬰兒/大寶 (P)    [____110_____]  HKD                │    │
│  │  成人 (S)         [____110_____]  HKD                │    │
│  │  成人 (P)         [____240_____]  HKD                │    │
│  └──────────────────────────────────────────────────────┘    │
│  ┌─ B. 立體擺設物料 (2) ▼ ─────────────────────────────┐    │
│  │  ...                                                  │    │
│  └──────────────────────────────────────────────────────┘    │
...
| 元素 | 規範 |
|------|------|
| 分組面板 | 預設 GROUP A 展開，B–MISC 收合（accordion 樣式）；**Desktop 允許多組同時展開**（螢幕空間足夠）；單組限制僅適用 Mobile |
...
```css
/* 既有 V41 token 直接複用 */
--fhs-primary: V41 既有 primary color
--fhs-spacing-md: 16px
...
| 區塊 | 行號預估 | 改動 |
|------|---------|------|
| 財務設定 panel HTML | V41 既有 7-key 區段 | 改為 17-key 分組顯示；複用既有 input class |
