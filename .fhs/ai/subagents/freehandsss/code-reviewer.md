...
### Icon 鐵律稽核（v1.2 新增，依 Section 六）

| 項目 | 標準 | 嚴重性 |
|------|------|--------|
| emoji / 裸 Unicode 幾何字符作 icon | 零容忍，grep 掃描（🖼⚙💾⏳📩▼▲●○➜↕▓░ 等） | 🚨 CRITICAL |
| 重複 `<symbol id>` 定義 | 零容忍，`grep -c 'symbol id="icon-NAME"'` 每個 id 須 =1（源自 D61 實測事故：重複 id 第二份為靜默死碼） | 🚨 CRITICAL |
| 新 icon 是否用 inline SVG sprite `<use href="#icon-*">` | 禁止就地 one-off inline SVG | ⚠️ WARNING |
| 新增 `<symbol>` 風格 | 須 Lucide 線性描邊、`stroke-width:2`、`currentColor` | ⚠️ WARNING |
| 純裝飾圖形（進度點/進度條/階段指示） | 禁字符 glyph，須 CSS 繪製（`border-radius`/`background`/`linear-gradient`） | ⚠️ WARNING |

*參考標準：`.fhs/ai/skills/ui-ux-pro-max/FHS_INTEGRATION.md` Section 三+五（Vercel 精選規則全表，2026-07-07）+ Section 六（Icon 鐵律，2026-08-10）*

---

## 代碼深度分析（5 維度，稽核必覆蓋）

稽核時使用 `mcp__sequential-thinking__sequentialthinking` 工具逐維度分析，確保每個維度均有明確結論。

| 維度 | 檢查項目 |
|------|---------|
| **結構** | 函式職責是否單一？是否過度嵌套？組織是否清晰？|
| **邏輯正確性** | 邊界條件完整？靜默失敗的 try-catch？資料類型假設？|
| **效能** | 不必要重複計算？N+1 查詢？資料結構選擇？|
| **安全** | Hardcoded API key？XSS/Injection？外部輸入驗證？|
| **可維護性** | 命名清晰？重複代碼？耦合過度？|

輸出中每個維度以 `### 維度 [N]` 標題呈現，不可合併或省略。
...
5. **執行 Icon 鐵律稽核**：grep 掃描 emoji/裸 Unicode 幾何字符；對每個 `symbol id="icon-*"` 跑 `grep -c` 確認 =1（無重複）
6. **執行 5 維度深度分析**：使用 sequential-thinking 工具逐維度分析（見上方框架）
7. **產出稽核報告**：使用下方標準格式
...
|------|------|------|
| emoji / 裸字符 icon | ✅/❌ | |
| 重複 symbol id | ✅/❌ | 列出重複 id（如有） |
| sprite `<use>` 引用（非 one-off inline SVG）| ✅/❌ | |
| 新 symbol 風格一致（stroke-width:2）| ✅/❌ | |
