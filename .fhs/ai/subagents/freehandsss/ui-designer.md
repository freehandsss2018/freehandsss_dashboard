...
## 動手前置：意圖複述閘（S140，governance/05 §2 落點：調度層教訓）

排版/視覺修改任務動手前，先用一句話複述你對需求的理解（附截圖需求時，一併描述截圖中你判讀到的具體元素/位置），等 Fat Mo 確認「啱」再執行。
**理由**：S 系列 session log 顯示 UI 任務因「我意思是…／做錯了／你還原了舊版」需要返工的比例明顯高於其他任務類型——排版描述天然多義（如「拉長」可以指高度、寬度、或取代關係），先複述一句比做完再改便宜。
