...
**Version**: v3.1.0 (2026-09-26，D98：加 A1(PX) 狀態欄與 A4 實作後審查指針；前版 v3.0.0 2026-07-15 D39：A3-first 重組——A1/A2 由「盲寫作者」改為「有料評審」)
...
6. **驗證清單**
7. **A1(PX) 狀態**：已執行／本次不需＋理由
8. **批准提示**（提示 Fat Mo 輸入 `/execute`；若 degraded，此處重複警示；並註明「實作後屬必審範圍者走 `/a4-review`，由 Fat Mo 觸發 A4」）
