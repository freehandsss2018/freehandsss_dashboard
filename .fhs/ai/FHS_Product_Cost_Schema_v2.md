...
created: 2026-05-28
updated: 2026-05-28
authority: SSoT for cost_configurations 17-key schema (Core layer)
companion_docs:
  - .fhs/ai/FHS_Product_Cost_UI_Spec.md       # Desktop + Mobile UI 規範
  - .fhs/ai/FHS_Product_Cost_Operations.md    # RPC / 並發 / 升級 SOP
references:
  - docs/FHS_Product_Bible_V3.7.md (§1 Drawing Cost 權威)
...
status: v2.1 draft — pending 3 subagent audits → Fat Mo GO → Stage 3
---

# 📜 FHS 產品成本 Schema v2.1 — Core 文件
...
v2 schema 擴展為 17 個 key，分 5 個 GROUP，明確覆蓋所有現行產品。
...
## §2. GROUP 總覽（17-key 一覽表）

| GROUP | display_group | Key 數 | 用途 |
|-------|---------------|--------|------|
| A | drawing | 4 | 繪圖成本（4 tier：嬰兒S / 嬰兒P / 成人S / 成人P） |
| B | material_3d | 2 | 立體擺設物料（木框 / 玻璃瓶） |
| C | material_jewelry | 3 | 飾品物料（扣具配件 + 吊飾 2 材質） |
| D | shipping | 2 | 運費（標準 / 順豐） |
| E | addon | 2 | 加購配件（羊毛氈 / 燈飾） |
| MISC | misc | 3 | 印刷費、繪圖固定費、鎖匙扣多件扣減 |

### 2.1 20 個 config_key 完整清單（0025 補入 3 個，2026-06-02）

| # | config_key | display_group | display_name | 預設值 | 用途 |
|---|-----------|---------------|--------------|--------|------|
...
| 7 | `material_cost_keychain_stainless` | material_jewelry | 鎖匙扣 - 不銹鋼物料 | 0 | Fat Mo 後續填入 |
| 8 | `material_cost_keychain_alloy` | material_jewelry | 鎖匙扣 - 鋁合金物料 | 0 | Fat Mo 後續填入 |
| 9 | `material_cost_necklace_silver` | material_jewelry | 吊飾 - 925銀物料 | 0 | Fat Mo 後續填入 |
| 10 | `material_cost_necklace_gold` | material_jewelry | 吊飾 - 925金物料 | 0 | 金銀同價（Bible §3） |
| 11 | `necklace_chain_cost` | material_jewelry | 吊飾頸鏈成本 / 條 | 100 | **0025 新增**；P0 奇偶規則：Math.ceil(N/2)×$100 |
| 12 | `clasp_cost` | material_jewelry | 鎖匙扣扣具配件成本 | 10 | 鑰匙圈+扣環五金，所有鎖匙扣 SKU 共用 |
| 13 | `shipping_cost_standard` | shipping | 標準運費成本 | 0 | 既有 v1 key |
| 14 | `shipping_cost_sf` | shipping | 順豐運費成本 | 0 | 既有 v1 key |
| 15 | `charm_shipping_deduction_per_extra` | shipping | 吊飾多件運費扣減 / 件 | 35 | **0025 新增**；P0：(N-1)×$35，N=SUM(吊飾件數) |
| 16 | `addon_cost_wool_felt` | addon | 羊毛氈加購配件成本 | 30 | 既有 v1 key 改名 |
| 17 | `addon_cost_light` | addon | 燈飾加購配件成本 | 30 | 既有 v1 key 改名 |
| 18 | `printing_cost_per_cm2` | misc | 印刷費 / cm² | 0 | 既有 v1 key |
| 19 | `drawing_cost_fixed_per_order` | misc | 繪圖固定費 / 單 | 0 | 既有 v1 key（每單一次性） |
| 20 | `keychain_shipping_deduction_per_extra` | misc | 鎖匙扣多件運費扣減 / 件 | 20 | Bible §2.5（N-1）×$20，N=SUM(鎖匙扣件數) |
| 21 | `mixed_member_surcharge` | misc | 混合成員附加費（成人+嬰兒） | 300 | **0025 新增**；立體擺設混合訂單加收，計入售價與 Additional_Fee |
...
### 3.2 4-tier 成本表（權威：Product Bible V3.7 §1）

| Tier | Key | 金額 | 適用 SKU 範例 |
|------|-----|------|---------------|
| 嬰兒/大寶 S | `drawing_cost_baby_s` | $60 | `嬰兒鎖匙扣 - 不銹鋼 1飾`、`家庭(S1) - 嬰兒部分` |
| 嬰兒/大寶 P | `drawing_cost_baby_p` | $110 | `嬰兒(P)鎖匙扣 - 不銹鋼 1飾`、`家庭(P1) - 嬰兒部分` |
...
- `嬰兒鎖匙扣 - 不銹鋼 1飾` 和 `嬰兒鎖匙扣 - 不銹鋼 5飾` 的繪圖費相同（皆為 $60）
...
### 5.2 成本表（預設 $0，Fat Mo 後續更新）

| Key | 適用材質 | 涵蓋 SKU 前綴 |
|-----|---------|--------------|
| `material_cost_keychain_stainless` | 不銹鋼 | `*鎖匙扣 - 不銹鋼*` |
| `material_cost_keychain_alloy` | 鋁合金 | `*鎖匙扣 - 鋁合金*` |
| `material_cost_necklace_silver` | 925銀 | `*吊飾 - 925銀*` |
| `material_cost_necklace_gold` | 925金 | `*吊飾 - 925金*` |
| `clasp_cost` | 扣具配件（共用） | 所有 `*鎖匙扣*` SKU |
...
範例解析：
- `嬰兒(P)鎖匙扣 - 鋁合金 2飾` = 嬰兒 + 照片建模 + 鎖匙扣 + 鋁合金 + 2 件
