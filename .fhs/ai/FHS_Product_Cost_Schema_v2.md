...
|------|---------|----------|------------------|
| 嬰兒 | **初生，或客人的首個孩子**，立體倒模主角 | 無或 (S)/(P) | drawing_cost_baby_* |
| 大寶 | **客人的第二個孩子**（嬰兒的兄／姊），與嬰兒共享成本層 | 同嬰兒 | drawing_cost_baby_*（共用） |

> ⚠️ **2026-08-22 定義修正**：本表原記「嬰兒＝0–3歲幼兒、大寶＝4歲以上兒童」（**年齡**定義），與 Fat Mo 2026-08-22 口述的**出生次序**定義不符，已更正。兩者非同義——次子可能僅 2 歲（落在原「0–3歲」範圍）但按次序仍為大寶。此定義差異是 2026-08-22 一次定價實作錯誤的根因之一，權威定義見 `FHS_Pricing_Bible.md` §0。推論：**有大寶必然有嬰兒**，純大寶單定義上不成立。
...
| 玻璃瓶套裝 | 圓形玻璃容器，倒模放入 | `玻璃瓶套裝 (2肢)`、`玻璃瓶套裝 (4肢)`、`玻璃瓶套裝 (2肢+大寶)`、`玻璃瓶套裝 (4肢+大寶)`、`玻璃瓶套裝 (家庭)` |
| N肢 | 肢數（2 或 4），代表容器內倒模件數 | 2肢 = 雙手 OR 雙腳；4肢 = 全四肢 |

> **玻璃瓶三組變體的成本一律 $210 flat**（`total_base_cost`，同木框一致，2肢/4肢不分價，migration 0030）——變體只影響 `suggested_price`，不影響成本。定價分歧見 `FHS_Pricing_Bible.md` §2.1：純嬰兒 $1,380/$1,680；**嬰兒＋大寶** $1,680/$1,980（migration 0091，2026-08-22，即同 tier ＋$300）；含父母 `(家庭)` $2,580 flat（migration 0060）。
> ⚠️ 肢數 tier 自 2026-08-22 起**只數嬰兒肢體**（推翻 2026-07-21「大寶肢體同等計入」定案），大寶改為「有冇參與」的 ＋$300 修正項。
> ⚠️ `(家庭)` 自 2026-07-19 上線起、`(N肢+大寶)` 自 2026-08-22 起，**皆不在 n8n `Smart Cache Strategist` 的 `BASE_PREFIXES` 前綴表內**——此為設計上可接受：前綴表只是變體家族的查詢優化，未命中者自動 fallback 至 `sku.eq` 精確查詢 `products` 表（節點註解：「New products automatically picked up without code change」），故新增立體擺設 SKU **無須改 n8n**。
