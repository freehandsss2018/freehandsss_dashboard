...
| 日期 | 檔案 | 主題 | 關鍵字 | 摘要 |
|------|------|------|--------|------|
| 2026-08-16 | 2026-08-16_accessory-cost-doc-sync-and-worktree-path-leak.md | accessory_cost文件補漏+worktree路徑事故 | finance-gatekeeper, accessory_cost, subagent-verification, worktree, absolute-path | 補7份文件accessory_cost第四分類缺口；grep sweep清單本身有盲點；Session全程絕對路徑漏worktree前綴致改錯主倉，兩輪fresh-context覆核揭發並修復 |
...
| 2026-08-10 | 2026-08-10_supabase-secret-key-public-repo-leak-auto-revocation.md | Supabase Key公開Repo洩漏遭自動撤銷（含5輪追加：全掃描/平台層封鎖/latest無聲升級/防線落地/警報追到底） | supabase, secret-key, github, public-repo, n8n, credential, security, env-access, task-runner, health-check, pitfall | 健檢腳本PASS掩蓋落地失敗警告；n8n Code node寫死fallback secret隨workflow備份洩落public repo，GitHub↔Supabase自動撤銷觸發401；「無人改過」需查`git log -S`+repo可見度先落結論。**追加**：①揪出一個寫死secret要全workflow掃同一前綴(共3節點)，純寫死vs有env判斷嘅修復難度完全不同；②n8n Code node預設封鎖env存取(`$env`throw denied+沙盒無`process`)，此即當年硬編碼secret嘅真正成因，需`N8N_BLOCK_ENV_ACCESS_IN_NODE=false`；③`:latest`tag令重啟＝無聲升級，`require('axios')`進程級崩潰try/catch捉唔到；④寫低教訓≠落地防線(補run_all.py DEGRADED層/測試腳本短路bug/MCP備份繞過hook洩密);⑤新警報第一次觸發須逐個案追到底，恆定DEGRADED=恆定PASS，且session中途被推翻嘅結論要回頭檢查基於舊結論寫嘅代碼 |
| 2026-09-06 | 2026-09-06_cross-branch-production-overwrite-and-device-only-symptom-misdiagnosis.md | 跨分支生產版覆寫(第三次)+真機症狀誤診 | deployment, cross-branch, current-html, gate0, lineage, ios-safe-area, powershell-bom | 部署覆寫另一分支117個commit；D70已登記風險但散文層面零效果故加Gate 0血統閘；真機專屬症狀應先核實部署版本同用戶載入版本是否同一份，唔好推測裝置行為；.ps1本身要UTF-8 BOM |
