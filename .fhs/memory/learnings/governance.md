...
4. **功能低使用率檢視點，先排除「唔熟悉操作」先假設「入口/UX 太深」**：D58 IG看門狗詞句規則觀察期滿，提案數 7<10 觸發預定判準「入口太深」，AI 已備好三個改介面方案候選。但一問 Fat Mo，佢反應係「重新教我點用」而非揀邊個改法方向——顯示真正瓶頸可能係操作方式從未被講解清楚，非入口深度本身。修法：低使用率覆核先簡短重述操作流程俾決策者，等佢自己判斷「係唔識用」定「識用但唔想用」，唔好一見到數字未達標就直接跳入「重新設計 UX」呢層，會做多咗唔啱嘅工 — D58-follow 2026-09-05 `@governance` <!-- v:2026-09-05 -->
