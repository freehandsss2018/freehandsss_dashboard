...
7. **【高頻 ⚠️】n8n Code node 預設封鎖環境變數存取（`N8N_BLOCK_ENV_ACCESS_IN_NODE`）——見到「secret 硬編碼 + 防禦性 try/catch 包住 env 讀取」呢個組合，唔好當疏忽，要當「env 存取曾經失敗過」嘅訊號**：診斷 401 憑證事故時實測 `typeof $env=object` 但 `$env.X` throw `access to env vars denied`、`typeof process=undefined`（沙盒內冇 `process`）。呢個解釋咗點解舊代碼會出現 `process.env.X || '<寫死secret>'` 呢種寫法——原開發者試過讀env失敗，冇再深究就退回硬編碼。淨係喺 Docker 設環境變數唔夠，仲要喺 n8n 容器加 `N8N_BLOCK_ENV_ACCESS_IN_NODE=false` 先解封。**附帶教訓**：`require('axios')` 喺容器 `:latest` tag 重啟後由「可用」變「令 task runner 進程崩潰」（同 #2 D55 同源但唔同觸發時機），改用 `this.helpers.httpRequest()`（task runner 下經 RPC 代理主進程，唔崩）；`:latest` 令「重啟」等於「無聲升級」，任何用 `:latest` 嘅容器重啟後見新故障，第一個假設應該係版本升級而非設定改動 — D62/2026-08-10 [[project_n8n_supabase_401_credential_incident]] `@n8n +supabase +governance` <!-- v:2026-08-10 -->
8. **【高頻 ⚠️】節點輸出會被 n8n 存入 execution data（預設14日、成功失敗皆存全文），secret 唔可以經節點輸出傳遞；HTTP 節點失敗時 `error.context.request.headers` 只遮 `Authorization` 唔遮 `apikey`**：單改用 `{{ $env.X }}` 只堵成功路徑，失敗路徑仍洩漏，須 n8n credential + 刻意失敗單實測；查洩漏要跨 workflow 掃全 execution 並去重 — 源自 2026-09-19 D79 `@n8n +governance` <!-- v:2026-09-19 -->
9. **live 節點含 U+FFFD 亂碼字元時，唔好用 `update_node_code`（要整段重輸出 newCode）**：改 GET→精準替換（每處 count==1）→PUT 4 欄，並比對其餘節點同連線零改動 — 源自 2026-09-19 D80 `@n8n` <!-- v:2026-09-19 -->
10. **【高頻 ⚠️】節點設 `continueOnFail`+`alwaysOutputData` 會將 401 吞成「success」——健檢/稽核 workflow 唔可以睇 execution `status`，要讀該節點輸出有冇 `error` 欄位；揪出一個 workflow 嘅寫死 secret 後必須全伺服器掃所有 workflow（D62 只收斂咗 OrderProcessor，另 4 個 workflow 帶住已撤銷舊 key 靜默失敗 ≥5 週：Financial_Overview/GlobalReview 持續 401、IGWatchdog 兩個寫入節點被吞、ErrorMonitor `error_logs` 由始至終 0 行）；PUT 更新 workflow 時剔走 `availableInMCP`/`binaryMode` 會令 `availableInMCP` 被重設為 false，須連同 `availableInMCP:true`（剔走 `binaryMode`）再 PUT 還原 — 源自 2026-09-20 D79續 `@n8n +governance` <!-- v:2026-09-20 -->
11. **憑證外洩事故修復必須跨全部工作流程搜同一模式，唔可以只修「出事嗰條」**：D62（2026-08-10）修復咗 Core OrderProcessor 工作流程 3 個寫死 Supabase secret key 嘅節點，但同一條已死 key 原來喺**獨立第二條**工作流程（IG 睇門狗 `D4LK6VrQbiXlju0V`）嘅另外 3 個節點都有寫死，D62 修復範圍冇覆蓋，令呢 3 個節點靜默 401 近 4 星期都冇人發現（外層 `continueOnFail` 令整體執行仍回報 `status=success`，唔查節點級輸出睇唔出）。**發現契機純屬巧合**（例行檢視另一件事嘅 checkpoint 撞正資料斷流異常）。修法：憑證洩漏/撤銷事故修復時，應該用 `git log -S <key片段>` 或 n8n API 逐一 workflow 掃描相同 header 值，確認**所有**工作流程都冇殘留同一條 key，唔可以假設「呢個 workflow 出事就淨係呢個 workflow 有問題」 — D58-follow/2026-08-27 [[project_n8n_supabase_401_credential_incident]] `@n8n +governance` <!-- v:2026-08-27 -->
12. **live 手動修復 credential header 後，若對應 builder 腳本仍係 build-time 字面插值，下次重跑 builder 會靜默覆蓋返修復**：D58-follow 手動修咗 `D4LK6VrQbiXlju0V` live 上 3 個節點嘅 `apikey`/`Authorization` header（由寫死 key 改 `={{ $env.SUPABASE_SERVICE_KEY }}` runtime 表達式），但對應嘅 `scripts/ig-watchdog/build_n8n_workflow.cjs` 一直用 `const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_KEY` 喺 build time 焗死插值——若之後有人（人或 AI）跑呢個 builder 重新部署（例如加新節點順手重新 generate），會用當時機器 env 值蓋返 live 已經改好嘅 runtime 表達式，令同一故障原封不動重演。範圍亦唔限於已知壞嘅 3 個節點：builder 入面凡係用 `SUPABASE_SERVICE_KEY` 呢個 const 嘅節點（本例其實有 5 個）都有同一結構性缺陷，只係另外 2 個未被 D62/D58 兩次事故點名過。**同 [[feedback_migration_repo_db_drift]] 屬同一族根因**（live 快速修復 vs repo/generator 源頭未同步，下次從源頭重新產生就回歸）——差別喺呢次係 n8n workflow builder 而非 SQL migration，通用教訓：任何「live 手動 patch」都要問一句「呢個 live 物件係咪由某個 repo 內腳本產生／部署嘅？如果係，個腳本本身改咗未？」 — D58-follow-v2/2026-08-28 [[project_n8n_supabase_401_credential_incident]] `@n8n +governance` <!-- v:2026-08-28 -->
