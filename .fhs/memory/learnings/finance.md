...
7. **「待確認」＝日期、刻字內容等訂單細節仍待確認，同財務無關；訂金／全付一律已實收**：唔好因狀態質疑收入／成本；`confirmed_at` 只係入單日 — 源自 Fat Mo 2026-09-20 澄清（曾兩度誤解為「未確認收入」「生產階段名」） `@finance` <!-- v:2026-09-20 -->
...
2. **「已知 cosmetic」審計差值可能係真缺陷嘅唯一信號**：V2 品項 drawing_cost 恆 0 藏喺 convergence_note 差額 8 週；見到就先拆解差值成因，唔好淨問「影響唔影響入帳」 — 源自 2026-09-19 D80 `@finance` <!-- v:2026-09-19 -->
3. **建議改動任何財務組件前，必須先答「現況四問」（角色／建立原因／邊個依賴／唔改會點），預設唔改**：無「已發生嘅錯數」或「已證實重大風險」唔建議修改；立場翻轉要明寫原因；更正時唔好由一個強斷言擺盪去另一個 — 源自 Fat Mo 2026-09-21（同 session 建議三度翻轉、連環答錯，見 decisions.md「AI 過失記錄」） `@finance` <!-- v:2026-09-21 -->
