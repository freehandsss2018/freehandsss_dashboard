...
13. **【高頻 ⚠️】程式碼旁邊嘅中文註解聲稱「已排除某值」，唔代表判斷式真係排除咗——註解同實作可以悄悄漂移**：`mapOrder()` Fix 4D 肢數統計，`父母/大寶: 待定 = default empty → do NOT count` 呢句註解寫得明明白白，但實際判斷式 `if(v&&v!=='無')` 淨排除咗 `'無'`，漏咗 `'待定'`。表單無論揀邊個款式（木框/玻璃瓶），DOM 都會預埋父母/大寶嘅 `limb_sel_*` 欄位（預設值 `待定`），`captureFormState()` 照單全收存入 `raw_form_state`，令木框單（本身冇父母/大寶選項）都被誤判有父母2手/大寶4肢。`.claude/skills/fhs-p-product-display/SKILL.md`（本檔對應嘅診斷技能文件）其實早已記低正確演算法（`v!=='無' && v!=='待定'`），但技能文件記嘅係「應該點做」，唔會自動同步落實際代碼——兩者可以長期並存分歧而冇任何錯誤訊號。**判斷訊號**：修 bug 前若已有對應技能文件/skill 記低「正確邏輯」，第一步應該直接逐字比對技能文件同實際代碼嘅判斷式，唔好假設代碼一定跟緊文件 — 2026-09-02 `@frontend` <!-- v:2026-09-02 -->
