...
13. **【高頻 ⚠️】用「數值本身係咪0」判斷輸入框係「未填/仍可自動填」定「操作員刻意輸入嘅確定值」必然出錯，因為0本身就係一個合法嘅確定值**：修收款分帳箱「手動歸零豁免收費」被強制覆寫 bug（D69），揪出三個獨立函式（blur revert／deposit↔balance交叉同步／週期性自動半填）都犯呢個同一錯誤——`isStandard`／`val==='0'`呢類判斷式將0同「空白/未填」歸做同一類，令用戶刻意輸入嘅0冇機會落地。**連帶陷阱**：正確判斷理應靠一個獨立嘅「人手觸碰過」旗標（`dataset.isDefault`），但呢個旗標若靠 `element.dataset.x` 掛喺 DOM 元素上，一旦所屬容器會被 `innerHTML` 重繪（例如`calculatePricing()`每次都會重建整個 split box 容器），旗標就會靜默重置消失——單改判斷式唔夠，仲要額外做一層快照機制（渲染前存、渲染後喺新元素身上還原）先可以令旗標跨重繪存活。**判斷訊號**：見到 `value !== '0'`／`paid === 0`呢類將0特殊化嘅判斷式，或者見到一個追蹤「人手/自動」嘅 flag 掛喺會被 `innerHTML=''`重建嘅元素上，就要問「呢個flag重繪後仲喺唔喺度」 — D69/2026-09-03 `@frontend` <!-- v:2026-09-03 -->
