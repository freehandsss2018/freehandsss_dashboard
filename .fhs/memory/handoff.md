```handoff
【FHS 交接摘要 — 更新: 2026-07-22 / S187續XI（D43續完成：Financial Overview 3-layer 落差修復——真正 bug 喺前端 sbFetchFinancial() current/yearly tab_mode 重複，非最初分析嗰個 n8n 層；已修復+V42→current.html升格+NAS三關驗證PASS；順手新建 Supabase RPC get_financial_overview_full 修復 n8n fallback path；S187續X：財務結算報價明細行距收貼+粗體範圍補齊）】
...
> 上次更新：2026-07-22（[D43續完成] Financial Overview 3-layer 落差已修復並部署，詳見下表列；真正bug喺前端`sbFetchFinancial()`而非最初分析嗰個n8n層，見decisions.md「D43續完成」條目更正說明）

| 優先 | 項目 | 狀態 | 備註 |
|------|------|------|------|
| ✅ 完成 | **[D43續完成] Financial Overview 3-layer 落差修復** | ✅ 全交付，browser實測+NAS三關驗證PASS | **關鍵更正**：最初分析對象（n8n `Financial Aggregator`）係錯嘅——live browser實測發現Dashboard實際用另一套獨立實作`sbFetchFinancial()`（前端直呼Supabase RPC，繞過n8n，`localStorage.fhs_supabase_read`預設開），呢套嘅分類groups本身已正確，真正bug係佢嘅Current/Yearly兩個分頁都call `tab_mode:'yearly'`（顯示同一組數字）。**修復**：(1)`sbFetchFinancial()`4個「current tab」呼叫改`tab_mode:'current'`，current.html+V42同步，Fat Mo確認後升格部署；(2)順手新建Supabase RPC `get_financial_overview_full`（migration 0061，Fat Mo選定方案C）取代n8n `Financial Aggregator`自製JS聚合，修復埋fallback path嘅正確性。驗證：browser直接讀`window.FO_LIVE_DATA`同RPC直查交叉核對（Current=$31,030月迄今／Yearly=$154,860全年／手模擺設分類=$67,068.57，全部區分正確）。教訓：分析bug前必須先用browser確認邊套代碼真正輸出緊畫面（呢個dashboard有兩套並存實作）。全文見`decisions.md`「D43續完成」條目、`.fhs/reports/planning/2026-07-22_financial-overview-3layer-gap-analysis.md`（含更正註記） |
