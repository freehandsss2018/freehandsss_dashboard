...
> 上次更新：2026-07-10（S161續 訂單完成偵測擴大納鎖匙扣/吊飾已部署NAS三關PASS，過程修復2bug；S161續 桌面「退回進行中」按鈕已改V42待批准部署；S161 3D打印pipeline v0方案書產出，Phase1腳待sonnet執行；S160 手機底部導覽列滑動指示器完成；S159續 部署授權放寬D21+正式部署NAS三關PASS；S158 Blueprint 降級+接線完成；S155 YouTube+NFC 工作流計畫新增待批；S149 待執行；S150 Phase 4-6 待 S149 後接續）

| 優先 | 項目 | 狀態 | 備註 |
|------|------|------|------|
| 🟡 中 | **[S161續] 訂單總覽桌面表格新增「退回進行中」按鈕** | ⏳ V42已改+驗證PASS，待批准升格current.html | 桌面稽核表格原本無任何完成/取消完成入口（只手機版有），補一顆條件按鈕，僅已完成訂單顯示，呼叫既有`triggerArchiveOrder()`；只做退回單方向（Fat Mo裁決不需桌面正向完成按鈕）。全文見 [Changelog.md](../../Changelog.md) S161續II條目；決策見 [decisions.md](../notes/decisions.md) |
