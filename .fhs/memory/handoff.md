```handoff
【FHS 交接摘要 — 更新: 2026-07-22 / S187續X（D43：Airtable全面剝離停用，Supabase正式翻轉唯一SSoT；6個n8n workflow+Dashboard前端已改駁；順手修復5個獨立pre-existing bug；fresh-context opus驗收全PASS；S187續VIII：財務結算卡片報價排版斷行修復；S187續IX：報價明細按分類配色；S187續X：明細行距收貼+粗體範圍補齊）】
...
📋 待辦: 🟡[D43續]**Financial Overview 財務總覽數字未對齊 3-layer revenue fallback RPC，Fat Mo 定性「得重要」，指定新 session 處理**（完整技術交代見下方 MASTER 表首列，唔好淨睇呢句就當已充分理解） 🟡[D43]system_config.last_id暫存保守預設值06000，需Fat Mo設定面板核實/更正 🟢[舊D42]Airtable阻塞已移除，可隨時補做test9002式端對端測試確認V47.20（非緊急，收尾性質） 🟡[S182]iOS月曆重疊待實機覆核 🔴[S161/S166]3D打印Phase1待簽收 🔴[S155]YouTube+NFC計畫待批 🔴[S149]治理可攜化待批准
➡️ 下一步: 新 session 優先處理 Financial Overview 3-layer 對齊（Fat Mo 明確要求）；其次 Fat Mo 核實 system_config 下一個訂單序號正確值（06000係保守預設，非查證原值）
...
> 上次更新：2026-07-22（D43：Airtable全面剝離停用，Supabase正式翻轉唯一SSoT，詳見下表[D43]列；下方兩條舊 Airtable 額度🔴項因剝離而解決，改列✅；Fat Mo 明確指示新增[D43續]Financial Overview技術債為下個session優先處理項）

| 優先 | 項目 | 狀態 | 備註 |
|------|------|------|------|
| 🟡 重要（Fat Mo 指定新 session 處理） | **[D43續] Financial Overview 財務總覽數字未對齊 3-layer revenue fallback RPC** | 🟡 待處理，技術債，Fat Mo 2026-07-22 明確要求「這個得重要，需要清楚交代 handoff 我在新session處理」 | **現況**：n8n `FHS_Financial_Overview`（workflow id `uQKtGDupMBnSygr3`）嘅 `Financial Aggregator` Code node 用一套獨立 JS 邏輯，直接 `SUM(final_sale_price)`/`SUM(total_cost)`/`SUM(net_profit)` 做月/年聚合，**冇實作3-layer revenue fallback**，混合單（同一張訂單同時有 `handmodel_cost>0` 又有 `keychain_cost>0 OR necklace_cost>0`）嘅 category 分類收入會計錯（`all`類總額唔受影響，但`handmodel`/`metal`分類細項會偏差）。<br>**對照嘅正確邏輯**：Supabase 已有 `get_financial_kpis`/`get_financial_charts` 兩個 RPC（`FHS_System_Logic_Overview.md` §10，已實作完整3-layer fallback：Layer1精確=`order_items.item_sale_price`加總；Layer2比例估算=`final_sale_price×品類成本/total_cost`；Layer3平均分兜底=`final_sale_price/品項數`；純單直接用`final_sale_price`唔套3-layer）。呢兩個RPC先係財務計算層真正SSoT，Dashboard財務總覽tab現時冇用佢哋。<br>**點解D43冇順手做**：RPC輸出嘅原始shape（KPI卡片/圖表資料）同Dashboard `FO_LIVE_DATA`/`FO_MOCK_DATA`預期嘅前端結構（`groups.{all,handmodel,metal}`＋`lineChart`/`barChart`/`pieChart`＋`handmodel_qty`/`metal_qty`）唔對應，直接換call RPC需要重新設計一層shape轉換，屬於獨立工程量，D43只做「換資料源」（Airtable→Supabase）呢個較細範圍，冇擴大去「換聚合邏輯」。<br>**下個session建議路徑**：(1) 先實測現時 Financial Overview tab 顯示嘅 handmodel/metal 細項數字，同直接跑 `get_financial_kpis('yearly','handmodel',...)` / `get_financial_charts(...)` 嘅結果做交叉比對，量化實際偏差幅度（先搞清楚係咪真係嚴重，定只係理論風險）(2) 若偏差有實質影響，設計一個轉換層（可以係n8n入面加一個Code node call RPC再重塑shape，或者Supabase側加一個view/函式直接輸出前端要嘅shape）(3) 相關檔案：n8n workflow `uQKtGDupMBnSygr3`（`Financial Aggregator`/`Collect Main Orders`/`Merge Datasets`/`Fetch Orders (Supabase)`/`Fetch Items (Supabase)` 呢幾個node，D43剛加嘅）、`Freehandsss_dashboard_current.html`+`V42.html`嘅`FO_LIVE_DATA`/`FO_MOCK_DATA`/`foDrawLine`/`foDrawBar`/`foDrawPie`一帶、`FHS_System_Logic_Overview.md` §10全文。決策脈絡見 `decisions.md` D43「五、技術債」段。 |
