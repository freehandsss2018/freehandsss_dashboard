```handoff
【FHS 交接摘要 — 更新: 2026-07-04 / S138】
🎯 目標: FHS 業務 POS+財務系統日常維護；S137 Fable 5 立制度 session——新建 `.fhs/ai/governance/`（00-06七檔：診斷/模型調度/判斷力rubric/派工模板/維護協議/未來信）+ CLAUDE.md 重寫為路由層；S137 記憶系統審視意外發現 S51 Obsidian D1決策「.fhs不可見」已推翻（外掛hidden-folders-access實測可解），補建wikilink使Graph View從4孤立點→12節點關聯網；S138 另開 session 清理 S137 發現的 docs/CHANGELOG.md 重複檔案，已刪除；下一步=Telegram 深連結完整端到端驗收（待notify>0觸發）+ 6支subagent過時model ID更新待授權+handoff.md首次輪轉（已超800行門檻）
✅ 已定決策: (1)V42=production(S115)；(2)Supabase-First，Airtable 僅備援；(3)IG 看門狗訂號 regex `/(?<!\d)0\d{6,7}(?!\d)/` leading-0 7-8位(S116)；(4)handoff SSOT=頂部便攜塊，hook 讀動態段(S118)；(5)ig_watchdog_alerts anon只讀+SECURITY DEFINER resolve RPC+service_role寫入(S119 Q2/Q4)；(6)Phase 1b 等 v3 Cron 驗收通過後才上(S119 Q3)→已解鎖(S122)；(7)嬰兒鋁合金物料=$115（同不銹鋼，S120）；(8)n8n PUT body只能含{name,nodes,connections,settings}四欄(S121)；(9)前端遇成本未隨件數累加只`fhsAudit_qtyWarn`誠實警示，禁做`單件×數量`假乘法（DB存值與真值皆非乘積，S124）；(10)Task A四欄(drawing/printing/chain/shipping_cost)=正式廢欄（保留欄位不DROP，停止補寫投資，Audit Ledger已改用訂單層分類欄，S125）；(11)21裸列NULL-subtotal=defer（財務真理於訂單層完整，S125）；(12)V42 簡化付款按鈕=「⊞ 簡化/≡ 逐件」操作者語言（非三大類/細分），P=橙/K=藍/M=紫，IG訊息付款行三類小計格式（S126）；(13)n8n HTTP Request v4 POST JSON array=用contentType:"raw"，禁specifyBody:"string"+JSON.stringify組合（PGRST204，S127）；(14)V42 簡化模式=default；全部半訂/付清按鈕=動作語義（顯示下次將執行），_depositMode初始=null；新訂單auto-fill填格→同步_depositMode='half'（filledAny guard，S131修正）；舊訂單還原期=_fhsSplitRestoreSnapshot≠null→跳過mode更新(S107保護)；(15)Audit Ledger 品項明細=左右手腳精簡標籤(item_key後綴)，刻字engraving不顯示，次序P→K→M固定，色標頭對齊S126簡化付款(S128)；(16)n8n workflow JSON序列化用ensure_ascii=True，禁emoji直接嵌入字串（surrogate pair silent fail，S129）；(17)cost_override_locked=true時批次跳過該訂單（fhs_batch_recalc_execute+fhs_apply_financial_batch_update雙守衛），人工覆蓋優先於n8n批次（S130 Phase B）；(18)Desktop App=主介面（收斂非遷移/除役），Antigravity=永久共存備援無除役時間表，Cursor/n8n三腦=休眠藍圖（S134，AGENTS §1.2）；(19)/ag-flow /ag-stitch-sync /ag-ui-import 三指令DEPRECATED，改用/cl-flow與ui-designer subagent（S134）；(20)`.fhs/ai/governance/`模型調度制度層建立，CLAUDE.md改路由層（S137）；(21)Obsidian D1推翻——`.fhs/`對Obsidian「永遠不可見」限制已解除（hidden-folders-access外掛白名單），D2三層記憶職責邊界不變（S137）
🔬 驗證: 已證實=IG v3 Cron PASS Exec 4012（S122）；Phase 1b Write Alerts body bug修復 versionId=2353e4da（S127）；tg2 emoji fix versionId=bb683165（S129）；audit_logs 0044+RPC live（S124）；S128 Audit Ledger視覺優化 node smoke test PASS；0047 migration smoke test 8/8 PASS（S130 Phase B）；NAS 部署 SHA256=AC3C4C00E8691FC8C96CAD5F5E725719D1437F97E1DCA165ADC3803BC15C362E 915065bytes（S130合包）；S131 filledAny guard修正落盤；S134 Desktop App Code分頁P1-P5全通過（hook/commands/MCP/subagent/auto-memory）+P10三腦API連線實測全通過（無Cloudflare封鎖）；n8n團隊workflow cztGsFXZYtvBUDA6已部署未執行（Anthropic$0待加值）；S136 Fat Mo NAS 實機確認簡化付款按鈕切換行為 PASS（S131 filledAny guard 修正 + S132 概覽篩選 UI 一併確認）；S136 Telegram深連結URL修復 versionId 683ed8e5→05740bb4（curl驗證壞URL 401/正確URL 200，含query string實測200，9個credential節點完整）；S137 governance 7檔 fresh-context opus對抗審查PASS-with-fixes（findings已修）；S137 Obsidian pilot實機驗證（computer-use screenshot）：handoff.md 3918行/lessons 70檔皆無效能問題，Graph View path:.fhs篩選4孤立點→12節點關聯網；S137 wikilink git diff逐行核對零誤改原文；未驗=Telegram 深連結完整端到端驗收（真實觸發+人工點擊，待實際 notify>0）；S134 Cowork P9手機查單未測；S138 docs/CHANGELOG.md 刪除已驗證（git status 僅 1 檔 D，唯一活引用已改指向根目錄版本）
📋 待辦: ✅S134 Desktop App平台收斂Phase0-4 ✅S135 /upload-web 部署S131+S132+S133至NAS ✅S136 Phase B NAS實機確認（按鈕切換PASS） ✅S136 Telegram深連結URL修復（5006/web/→公開網址） ✅S136 learnings.md整理59→49條 ✅S137 governance治理層建立+CLAUDE.md路由化 ✅S137 Obsidian D1推翻+wikilink補建 ✅S138 docs/CHANGELOG.md重複檔案已刪除 ⚪Telegram深連結完整端到端驗收（待notify>0觸發）⚪Anthropic加值後測n8n團隊workflow⚪P9手機查單測試⚪6支subagent過時model ID更新待授權⚪handoff.md首次輪轉（超800行門檻，SOP見governance/05§4）
➡️ 下一步: Telegram 深連結完整端到端驗收（待實際 IG 看門狗 notify>0 觸發，URL bug 已修，只待真實事件）+ handoff.md 首次輪轉（governance/05_maintenance-protocol.md §4 SOP已備）+ Anthropic 加值後測 n8n 團隊 workflow
─── 便攜邊界（以下為外部貼用靜態地雷，hook 動態注入截至上行）───
⚠️ 易猜錯: (1)mapOrder o.id=FHS string非UUID，o._uuid=Supabase UUID (2)NAS n8n Code節點fetch/require/process靜默失敗→用HTTP Request節點 (3)final_sale_price=Deposit+Balance+Fee=確收真理，n8n嚴禁覆蓋；total_cost=估算快照 (4)captureFormState()/raw_form_state/HTML ID不可動（斷鏈） (5)IG watchdog v3 lib/order-match.mjs=單一真源，改邏輯必改lib再rebuild，diff-guard測試保護 (6)便攜塊=版本/狀態SSOT，不得另開第二份版本維護檔 (7)Obsidian dot-directory「不可配置」認定已推翻(S137)，`.fhs`可經外掛白名單顯示，但D2職責邊界不變（AI仍唯一寫入.fhs/memory）
🗺 下鑽: 完整明細見下方「MASTER 持續待辦」表 + 各 Session 條目（搜尋「Session 1XX 完結」）+ 新增制度層見 `.fhs/ai/governance/00_INDEX.md`
```
...
> 上次更新：2026-07-04（S138 — docs/CHANGELOG.md 重複檔案清理完結）

| 優先 | 項目 | 狀態 | 備註 |
|------|------|------|------|
| ✅ 完成 | **[S138] docs/CHANGELOG.md 重複檔案清理** | ✅ 全交付（S138） | 確認為S63建立之過時分岔複本（末條S130 Phase B後停更）；唯一活引用`FHS_Product_Cost_Operations.md`Stage 4已改指向根目錄；備份後`git rm -f`刪除；`docs/repo-map.md`（原缺此檔樹狀圖條目）補`[已刪除]`標記；learnings Pitfall #25 |
| ✅ 完成 | **[S137] Governance 治理層建立**（Fable 5 立制度） | ✅ 全交付（S137） | `.fhs/ai/governance/`00-06七檔（診斷/模型調度/判斷力rubric/派工模板×5/維護協議/未來信）；CLAUDE.md重寫為路由層；fresh-context opus對抗審查PASS-with-fixes；完成記錄`2026-07-04_governance-layer-and-memory-system-audit_completion_report.md` |
| ✅ 完成 | **[S137] Obsidian D1（S51）技術限制推翻 + wikilink 補建** | ✅ pilot實測PASS（S137） | 外掛`hidden-folders-access`白名單`.fhs`，實機驗證handoff.md 3918行/lessons 70檔皆無效能問題；補建wikilink（governance7檔互連+Knowledge Map+decisions.md+learnings↔lessons 5/49配對）；Graph View由4孤立點→12節點關聯網；D2三層職責邊界不變；decisions.md D4記錄 |
| ⚪ 待辦 | **[S137] handoff.md 首次輪轉** | ⏳ 待執行 | 已超`governance/01_diagnosis.md`定義的800行觸發門檻（現3900+行）；SOP見`05_maintenance-protocol.md`§4，屬可自行執行級，先備份再動手 |
| ⚪ 待授權 | **[S137] 6支subagent過時model ID更新** | ⏳ 待Fat Mo授權 | frontmatter釘`claude-sonnet-4-6`（舊世代ID），建議更新為`claude-sonnet-5`或改繼承；見`governance/02_model-dispatch.md`§0現況表 |
| ✅ 完成 | **[S136] learnings.md 超量整理** | ✅ 59→49 條（S136），S137再加1條=50條 | 退役3條（Smart Cache COST_MAP/單一配件filter/generate()else，均已被更高層機制取代）+ 合併4組同主題（ENUM cast+PostgREST括號、Migration套用時序、付款split清空/污染、n8n API送出限制四合一）+ 修正 Pitfalls 區塊編號亂序（曾重複#22/#23/#24）；退役/合併項均附📌可追溯附註 |
| ✅ 完成 | **[S136] IG 看門狗 Telegram 深連結 URL 修復** | ✅ 部署 PASS（S136） | `Classify & Report` 節點硬編碼 `yanhei.synology.me:5006/web/` 實測401，改為公開網址 `yanhei.synology.me/Freehandsss_dashboard_current.html` 實測200；GET→改→PUT 外科手術，versionId 683ed8e5→05740bb4；9個credential節點完整；`build_n8n_workflow.cjs` 單一真源同步；剩餘=完整端到端驗收待notify>0觸發 |
| ✅ 完成 | **[S136] Phase B NAS 實機確認（簡化付款按鈕切換行為）** | ✅ Fat Mo 實機驗收 PASS（S136） | S131 filledAny guard 修正（新訂單 auto-fill 後按鈕自動切「全部付清」）+ S132 概覽篩選 UI 於生產環境 NAS 實機操作確認符合預期 |
| ✅ 完成 | **[S134] Claude Desktop App 平台收斂計劃 Phase 0-4** | ✅ 全數完成（AGENTS.md v1.5.0） | Flow ID 2026-07-03-0014；Desktop App 主介面確立（P1-P5探針全過）；MCP駁接（remote-first）；Skills收斂22+4橋接；Cowork治理文件+三模式決策卡；n8n三腦實測建成後對照/cl-flow裁定休眠；ag-flow/ag-stitch-sync/ag-ui-import標DEPRECATED；AG/Cursor皆定為休眠/共存藍圖非除役 |
| ✅ 完成 | **[S135] /upload-web 部署 S131+S132+S133 至 NAS** | ✅ 三關PASS（S135）+ 實機驗收PASS（S136） | V42升格current；SHA256=DCF266F11C961F865F3DC6F16A91F46CD89480EF744870697BAE02E78D2812C0，919,443bytes |
| ✅ 完成 | **[S132] 概覽篩選 UI 四項優化** | ✅ 落盤（S132）+ 部署NAS（S135） | Task1:手模狀態篩選(hm_pending/booked/laser/done，client-side)；Task2:重新載入後自動縮收；Task3:全尺寸折疊+localStorage持久化；Task4:時限警示排序(Appointment_Date asc null-last) |
| ✅ 完成 | **[S131] 簡化付款 auto-fill 按鈕狀態修正** | ✅ 落盤（S131）+ 部署NAS（S135） | `_quickHalfFillAllSplits` 新增 `filledAny` flag；條件 `if(force)` → `if(force\|\|(!_fhsSplitRestoreSnapshot&&filledAny))`；新訂單 auto-fill 後按鈕自動切「全部付清」；S107 舊訂單保護保留 |
...
| ✅ 完成 | **[審計日誌 Phase B] 訂單層成本修改 + 變更歷史** | ✅ 全交付（S130 Phase B + NAS 已部署）| migration 0047：`cost_override_locked` + `fhs_adjust_order_cost` + `fhs_unlock_order_cost` + 雙批次守衛；設定中心「訂單層成本修改」UI；Audit Ledger ② badge + ⑤ 變更歷史 `<details>`；smoke test 8/8 PASS；NAS SHA256=AC3C4C00 915065bytes ✅ |
| ✅ 完成 | **[S130b] 訂單總覽日期優先次序修正** | ✅ 全交付（S130b + NAS 合包已部署）| V42 mapOrder() L13773：Date=appointment_at\|\|confirmed_at；sbFetchGlobalReview L13825：order=appointment_at.asc.nullslast,confirmed_at.asc；FHS_System_Logic_Overview §10.11；CHANGELOG S130 條目 |
