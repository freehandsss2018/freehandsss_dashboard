```handoff
【FHS 交接摘要 — 更新: 2026-07-22 / S187（D42吊飾頸鏈成本全鏈路修復：前端+Audit Ledger+n8n V47.20已部署+7張歷史單backfill）】
🎯 目標: D42 全部完成——n8n V47.20已正式部署+7張歷史單backfill驗證PASS（DebbieHo0600727亦已一併修正，S181舊待辦解決）；🔴🆕Airtable月度額度問題再次確認仍未解決（fresh probe HTTP 429），阻擋咗端對端測試單驗證（test9002卡running），累計已2張執行卡死（4902/test5001舊+4906/test9002新）
✅ 已定決策（(1)-(66)收錄於decisions.md/MASTER表/Logic_Overview.md）：D42 吊飾頸鏈成本全鏈路對齊鎖匙扣模式——前端雙計修復/Audit Ledger badge拆分/n8n V47.20品項對稱+共用折扣（已正式部署）/7張歷史單backfill（推翻早前「只新單」決定，數學證明order-level總數零變動）；S186測試腳本雙態輪詢；D41成本架構Phase2
🔬 驗證: D42 — dry-run通過+Node harness數值吻合+7張歷史單UPDATE前後necklace_cost/total_cost/net_profit/final_sale_price逐一核對零變動；端對端live webhook驗證受Airtable 429阻擋未能完成（非本次改動引起，新舊code同樣會卡）
📋 待辦: 🔴🆕[S186續]Airtable額度問題持續超過12小時未解，已卡2個n8n執行（4902/4906），現行**任何**新落單都可能受影響，急需Fat Mo處理帳單頁 🔴🆕[D42]Airtable恢復後補做test9002式端對端驗證確認V47.20正確運作 🟡[S182]iOS月曆重疊待實機覆核 🔴[S161/S166]3D打印Phase1待簽收 🔴[S155]YouTube+NFC計畫待批 🔴[S149]治理可攜化待批准
➡️ 下一步: Fat Mo 處理Airtable帳單額度（最優先，已阻擋落單流程超過12小時）；額度恢復後補做D42端對端驗證
...
> 上次更新：2026-07-22（D42：吊飾頸鏈成本全鏈路修復已完成部署+7張歷史單backfill，詳見下表[D42]列；🔴Airtable額度問題持續未解）

| 優先 | 項目 | 狀態 | 備註 |
|------|------|------|------|
| 🔴 緊急 | **[S186續] Airtable API月度額度持續耗盡超過12小時** | 🔴 待 Fat Mo 處理，已卡死2個n8n執行 | S186首次發現後12+小時仍未解決（fresh curl探測仍HTTP 429）；execution 4902（舊test5001）+4906（新test9002驗證單）皆卡`running`未落地Supabase；現行任何新落單都可能受影響，非測試單專屬問題 |
| ✅ 完成 | **[D42] n8n V47.20 + 7張歷史單backfill：吊飾頸鏈成本記帳格式對齊鎖匙扣環扣模式** | ✅ n8n已正式部署+7張歷史單UPDATE驗證PASS；⚠️端對端live測試受Airtable阻擋未完成，待額度恢復後補測 | 節點 `Calculate Profit & Pack Items`（V47.19→V47.20）：品項層對稱摺入$100/件頸鏈成本，訂單層共用折扣`floor(N/2)×100`取代舊式加項，數學等價總成本不變。7張歷史單（Dede/Kathleen/Akira/DebbieHo/Amen/Selina Lai/Lokyi_C）直接Supabase UPDATE backfill，逐單核對order-level財務欄位零變動。全文見 `FHS_System_Logic_Overview.md` §5.4.5、decisions.md D42、Changelog.md S187系列 |
