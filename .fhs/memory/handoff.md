```handoff
【FHS 交接摘要 — 更新: 2026-07-21 / S184（V42 介面底部按鈕溢位簡化「← 返回/🗑 刪除/✅ 審閱」+ 財務金額框 inputmode="decimal" 數字鍵盤與洗 0 行為修復）】
🎯 目標: V42 介面底部按鈕溢位簡化 + 財務金額框數字鍵盤與洗 0 行為修復；S183 玻璃瓶套裝含父母$2,580定價與S182系列iOS月曆/cache-bust機制繼續運作
✅ 已定決策（(1)-(66)收錄於decisions.md/MASTER表/Logic_Overview.md）：S184 V42底部按鈕簡化「← 返回/🗑 刪除/✅ 審閱」+ 財務金額框 inputmode="decimal" 數字鍵盤與洗 0 行為修復（未編D號，屬例行UI優化）；S183 玻璃瓶套裝含父母$2,580定價；D41 成本架構Phase2全品類drift一次收網；D40 吊飾成本雙數簿修復
🔬 驗證: S184 V42 介面按鈕與數字鍵盤 — HTML/CSS flex:1 寬度適應與 inputmode="decimal" + JS `_fhsSimpClickToEdit()` dataset.isEditing 洗 0 與鍵盤喚起邏輯驗證完成；D41/D40/S183 驗證項續生效
...
➡️ 下一步: Fat Mo 覆核 V42 生產版按鈕與數字鍵盤輸入，及手動 resync 0600107 / DebbieHo 兩張歷史單
