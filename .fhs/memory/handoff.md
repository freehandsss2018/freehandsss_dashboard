```handoff
【FHS 交接摘要 — 更新: 2026-07-13 / S171續】
🎯 目標: Write Alerts on_conflict缺陷修復執行完成並live部署（task_e3a60daa，S171續）；P2b內容比對層（金額比對）執行完成並live部署（S150§4.8剝離範圍，S171續）；P2a IG訊息入庫+PII明文剝離執行完成（S171）；S150 Phase4-6 live cron覆核PASS結案（S171）；mattpocock/skills選擇性吸收4支落盤+治理登記（S170）；S155 YouTube+NFC工作流計畫待批
✅ 已定決策（(1)-(51)為S115-S166歷史決策，收錄於decisions.md/MASTER表/governance/03/Logic_Overview.md/learnings.md，不重複列；D9五項高風險規則—PUT四欄/qty禁假乘法/contentType raw/ensure_ascii/cost_override_locked—仍可查）：近期活躍：(52)S150 Phase4-6=D25：verified_ok正向記錄+orders anon權限收斂(S168)；(53)mattpocock/skills=D27(S170)；(54)P2a=D31：ig_messages表落地+PII三函式，fresh-context抓4項發現即時修復3項(S171)；(55)P2b=D32：content_mismatch表落地+extractAmountsFromText()/compareToOrder()金額比對+n8n三向分流+首次觸及Dashboard HTML，fresh-context opus抓5項發現即時修復4項——最關鍵F1：曆年形狀數字（V42確認文本嘅取模日期如「2026」）落喺金額合理範圍會被誤認金額，幾乎每張低價訂單確認訊息都會誤判，已修復（曆年數字需鄰近貨幣標記先當真金額）；F2 deposit fallback系統性誤報已移除；(56)Write Alerts on_conflict修復=D33：migration 0056新增`order_id_key` generated column具現化`COALESCE(order_id,'')`，淘汰舊expression index改建純欄位unique index，令PostgREST `on_conflict`參數真正生效（此前形同虛設，`Prefer:ignore-duplicates`從未真正觸發，真實重複反而擲23505整批打回）；live環境真實重複POST測試確認dedup生效(S171續)
🔬 驗證: S171續 Write Alerts修復：node --test 55/55 PASS(含diff-guard)+live環境用service_role key連續POST兩次同一測試資料，第一次201建立列，第二次201但回傳空陣列(無23505，dedup真正生效)，測試列已清除+live PUT後GET核對node數24不變/URL正確/其餘節點與9個credential節點不受影響+fresh-context code-reviewer(haiku)獨立審查。S171續 P2b：order-match.test.mjs 35/35 PASS(含F1/F2/F3回歸測試)+diff-guard+mock-execution harness全過+瀏覽器注入合成content_mismatch列驗證UI渲染+兩次live PUT後GET核對+fresh-context opus獨立審查PASS-WITH-CONCERNS(4/5即時修復)。S171 P2a：order-match.test.mjs 27/27 PASS+fresh-context opus獨立審查PASS-WITH-CONCERNS(3/4即時修復)。S171 S150 Phase4-6 cron覆核：execution 4638成功執行、0筆created_full屬正常空結果。
📋 待辦: 🟡[S171續]P2c(意圖標註+回覆範本庫)依cl-final-plan §8排隊，待Fat Mo下次/execute 🟡[S170]拷問4週試用閘：約2026-08-09覆核用量（同批覆核llm-council-skill暫緩決定，D28） 🔴[S161/S166]3D打印Phase1待Fat Mo最終目測簽收後開Phase2 🔴[S155]YouTube+NFC計畫待批 🔴[S149]治理可攜化待批准 🔴[S159規劃/S170已拷問修訂]取模排程中心方案書待執行（見D29） ⚪n8n Mirror Prep改RPC(需opus+live) ⚪Airtable API本月額度用盡(429)，PRICE_AUDIT暫時失效
✅ 已裁決結案（非待辦，避免重複詢問）：桌面版表頭對比度=Fat Mo主動選擇維持黑底原狀；[S168]S150 Phase4-6全部四項變更fresh-context PASS；[S170]mattpocock/skills唔改名+中文召喚詞疊加層方案Fat Mo已批准；[S171]S150 Phase4-6 live cron端到端驗證已覆核PASS，決策見decisions.md D25追覆核段；[S171]P2a IG訊息入庫+PII剝離已執行完成並live部署，決策見decisions.md D31；[S171續]P2b內容比對層（金額比對）已執行完成並live部署，含V42 Dashboard HTML首次改動，決策見decisions.md D32；[S171續]既有Write Alerts節點on_conflict缺陷（task_e3a60daa）已執行修復並live部署驗證，決策見decisions.md D33
➡️ 下一步: [PR #1](https://github.com/freehandsss2018/freehandsss_dashboard/pull/1)（Write Alerts on_conflict修復，claude/brave-jennings-a47074→main）已開，狀態OPEN，待Fat Mo決定是否merge；或批P2c是否現在繼續執行，或批S149/S155/取模排程計畫（已拷問修訂版）
...
> 上次更新：2026-07-13（S171續 Write Alerts on_conflict缺陷修復（task_e3a60daa）執行完成並live部署，migration 0056新增generated column令PostgREST on_conflict真正生效，live環境真實重複POST端到端驗證PASS，fresh-context code-reviewer獨立審查，詳見下表[S171續]列；S171續 P2b內容比對層（金額比對）執行完成並live部署，含首次Dashboard HTML改動，fresh-context opus獨立審查PASS-WITH-CONCERNS(4/5發現即時修復，含關鍵F1曆年誤判)；S171 P2a IG訊息入庫+PII明文剝離執行完成並live部署，fresh-context opus獨立審查PASS-WITH-CONCERNS(3/4發現即時修復)；S171同session覆核S150 Phase4-6 live cron結果PASS結案；S168 S150 Phase4-6執行完成）

| 優先 | 項目 | 狀態 | 備註 |
|------|------|------|------|
| ✅ 完成 | **[S171續] Write Alerts on_conflict缺陷修復**（task_e3a60daa 執行，D31 F3/D32 F4收尾） | ✅ 全交付，fresh-context code-reviewer獨立審查，live部署完成 | 既有`Write Alerts`節點（S119建立）POST `ig_watchdog_alerts`時缺`on_conflict`參數，PostgREST缺參數時UPSERT仲裁鍵誤落PRIMARY KEY，令`Prefer:ignore-duplicates`形同虛設——真實重複會擲23505整批打回（`continueOnFail:true`+`return=minimal`令此靜默無感）。根因：PostgREST `on_conflict`不支援expression index（`COALESCE(order_id,'')`），需具現化成generated column。修法：migration `0056_igwatch_alerts_on_conflict_fix.sql`新增`order_id_key text GENERATED ALWAYS AS (COALESCE(order_id,'')) STORED`，淘汰舊expression index改建純欄位unique index`ix_igwatch_alerts_dedup_v2`；`build_n8n_workflow.cjs`URL補`on_conflict=alert_date,thread,order_id_key,kind`。驗收：55/55單元測試+live環境真實重複POST兩次端到端驗證（第二次201空陣列無23505）+live PUT/GET核對+fresh-context haiku獨立審查。**過程事故**：本session一度誤在主git checkout而非指派worktree操作，且與另一session併行commit P2b產生migration編號衝突（0054→改號0056），已自行發現並修正，教訓見`.fhs/memory/lessons/INDEX.md` 2026-07-13條目。決策見decisions.md D33；Logic_Overview見§11.9；全文見Changelog.md S171續條目 |
...
| ⚪ 低 | **[S170] `llm-council-skill`（GitHub tenfoldmarc）暫緩安裝** | ⏳ Fat Mo 裁決方案A：暫緩，併入2026-08-09拷問試用閘覆核一併決定 | 5顧問人格平行辯論+匿名互審+主席裁決的Karpathy Council移植版技能；查證repo原文後評估：安全零風險（純prompt無外部API）、成本高（每次11個subagent）、核心價值（fresh-context匿名互審修正錨定偏誤）與既有`/8d`互補非取代、與`/cl-flow`/`/px`重疊且遜色（5顧問全為Claude分身無跨廠模型多樣性）。暫緩理由：避免同期連裝兩套決策meta-skill攤薄D27試用閘量測。決策見 decisions.md D28 |
