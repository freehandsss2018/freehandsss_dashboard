```handoff
【FHS 交接摘要 — 更新: 2026-07-22 / S187續XIII（交貨期進度卡「已完成訂單仍顯示逾期」修復：`v_delivery_reminders` view 從未引用權威完成旗標`orders.is_archived`，靠嘅兩個過濾條件（order層process_status字面值/item層process_status字面值）皆因唔匹配生產真實值而失效，33筆入面16筆已完成單被誤判逾期；migration 0063加`is_archived`過濾+擴充item層字面值集，fresh-context 5項覆核全PASS；接續S187續XII：Financial Overview 兩條路徑統一資料來源）】
🎯 目標: D43 全部完成——Airtable月度額度問題（HTTP 429，曾阻塞落單流程逾12小時）已透過全面剝離解決（非等額度恢復），6個n8n workflow（Core_OrderProcessor/OrderHistory/ErrorMonitor/Financial_Overview/MetadataUpdate/GlobalReview）+ Dashboard前端全部改駁Supabase，Airtable node/credential保留供未來重連直至另行通知；D42遺留嘅「Airtable恢復後補測test9002」待辦因阻塞源已移除，改列非緊急
✅ 已定決策（(1)-(67)收錄於decisions.md/MASTER表/Logic_Overview.md）：D43 全面剝離Airtable，Supabase唯一SSoT（AGENTS.md v1.7.0→v1.7.1）；新建system_config表+RPC取代舊Airtable借位設定儲存；ID語意由Airtable rec ID改Supabase業務字串（GlobalReview全案最高風險步驟，已驗證）
🔬 驗證: D43 — 6個workflow全部live webhook觸發+Supabase直查交叉核對零誤差（44單/103品項/年度財務總額全吻合）；fresh-context opus獨立覆核5大項全PASS（唔信文字記錄，重新live測試+清理測試數據）
📋 待辦: 🟢[舊D42]Airtable阻塞已移除，可隨時補做test9002式端對端測試確認V47.20（非緊急，收尾性質） 🔴[S161/S166]3D打印Phase1待簽收 🔴[S149]治理可攜化待批准
➡️ 下一步: 其餘待辦見 MASTER 表；YouTube+NFC計畫（S155）Fat Mo已裁決暫停擱置，非待辦
...
> 上次更新：2026-07-22（[S187續XIII] 交貨期進度卡已完成訂單誤顯逾期修復，`v_delivery_reminders` view 加 `is_archived` 過濾，詳見下表列；決策脈絡見decisions.md最新條目）
