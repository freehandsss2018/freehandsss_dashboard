```handoff
【FHS 交接摘要 — 更新: 2026-07-22 / S187續XII（D43續二：Financial Overview 兩條路徑統一資料來源——`sbFetchFinancial()` 由~130行前端組裝簡化為5行單一call `get_financial_overview_full`，migration 0062補齊marginChange/aovChange/isNewBusiness/orders_inclusive，同n8n fallback共用同一SQL實作，零重複邏輯；fresh-context覆核PASS+已部署；接續S187續XI current/yearly tab_mode重複修復；S187續X：財務結算報價明細行距收貼+粗體範圍補齊）】
...
📋 待辦: 🟡[D43]system_config.last_id暫存保守預設值06000，需Fat Mo設定面板核實/更正 🟢[舊D42]Airtable阻塞已移除，可隨時補做test9002式端對端測試確認V47.20（非緊急，收尾性質） 🟡[S182]iOS月曆重疊待實機覆核 🔴[S161/S166]3D打印Phase1待簽收 🔴[S155]YouTube+NFC計畫待批 🔴[S149]治理可攜化待批准
➡️ 下一步: Fat Mo 核實 system_config 下一個訂單序號正確值（06000係保守預設，非查證原值）；其餘待辦見 MASTER 表
...
> 上次更新：2026-07-22（[D43續二] Financial Overview 兩條路徑已統一資料來源，`sbFetchFinancial()`簡化為單一RPC call，詳見下表列；決策脈絡見decisions.md「D43續完成」+「D43續二」兩則條目）
