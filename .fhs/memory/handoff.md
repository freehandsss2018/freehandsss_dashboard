```handoff
【FHS 交接摘要 — 更新: 2026-07-16 / S180】
🎯 目標: V42快捷列優化完成（月曆入列+查看檔期掣取消+快捷列自訂系統+約定日期簡化+Eugenia row bug修復，S180）；S155 YouTube+NFC工作流計畫待批；S149治理可攜化待批准
✅ 已定決策（(1)-(62)為S115-S176歷史決策，收錄於decisions.md/MASTER表/governance/03/Logic_Overview.md/learnings.md，不重複列；D9五項高風險規則—PUT四欄/qty禁假乘法/contentType raw/ensure_ascii/cost_override_locked—仍可查）：S177/S177續/S179/S179續/S180均屬例行執行或既定方向迭代（非新架構決策），不編新D號
🔬 驗證: S180快捷列優化——方案書落盤(`shortcut-bar-custom-plan_2026-07-16.md`)分四批派sonnet執行+playwright本地伺服器實測（非file://，避開localStorage阻塞script block陷阱）：①月曆入快捷列+自訂系統(長按600ms/右鍵編輯,localStorage持久化)②近期排期tab取消③約定日期單一日曆化(readonly+隱藏原生圖示,桌面錨定零重疊)④Eugenia row bug修復(bookingRowHtml()淨view模式綁click,form模式已補回,表單草稿不受影響)。S179續／S179／S177續／S177 驗證細節已依P0.7.1輪轉搬移至 [handoff-portable-block-verified-2026-07-16.md](archive/handoff-portable-block-verified-2026-07-16.md)。
📋 待辦: 🟡[S177]finance-auditor MANIFEST新增行model欄暫填claude-sonnet-4-6（frontmatter缺此key外推值），待Fat Mo確認 🟡[S180]`globalOrders`快取limit:200理論上生產環境訂單量夠大時可能漏單，令月曆撳row出現「揾唔到」靜默失效，非本次回報症狀，留待日後評估加fallback補抓 🟡[S176]`.fhsAudit_qtyWarn` CSS死代碼清理（非阻塞，留待/fhs-slim） 🟡[S175]拷問4週試用閘＋llm-council-skill(D28)判準覆核：2026-08-09 09:00已設scheduled task自動觸發 🟡[S173]P2c §7量測待ig_messages自然累積足量真實訊息後補測+reply_templates 5筆草稿文案上線前需Fat Mo覆核 🔴[S161/S166]3D打印Phase1待Fat Mo最終目測簽收後開Phase2 🔴[S155]YouTube+NFC計畫待批 🔴[S149]治理可攜化待批准 ⚪[S179]取模排程中心C/D/E另日再議，先解決品項層狀態聚合落差（見decisions.md D29附錄） ⚪n8n Mirror Prep改RPC(需opus+live) ⚪Airtable API本月額度用盡(429)——現已知同時影響PRICE_AUDIT與FHS_Query_GlobalReview兩條workflow，暫時失效 ⚪item_base_cost欄位語意不一致（n8n源頭）留待日後獨立處理，非緊急（不影響確收金額） ⚪learnings.md 52條超governance/05§4上限50條，留待/fhs-slim ⚪session-log.md S172-S178區間缺entry，留待補記或視為可接受gap
✅ 已裁決結案（非待辦，避免重複詢問）：桌面版表頭對比度=Fat Mo主動選擇維持黑底原狀；[S180]V42快捷列優化四項全交付（月曆入列/查看檔期掣取消/快捷列自訂系統/約定日期簡化/row bug修復），已同步部署V42+current.html；[S179續]取模排程中心B v2重新設計完成，C/D/E依Fat Mo裁決另日再議，決策見decisions.md D29附錄；[S177續]n8n 22條殭屍workflow已刪除（備份於n8n/archive/zombies-2026-07-16/），FHS_Query_GlobalReview異常查明為Airtable額度牆非新故障，非架構決策不編D號；[S177]4項subagent版本漂移已修復，非架構決策不編D號；[S176]Audit Ledger假警示完全移除+current.html同步部署，決策見decisions.md D37
➡️ 下一步: Fat Mo批S149/S155/取模排程C+D+E繼續執行（需先解決品項層狀態聚合問題）之一，或評估S180`globalOrders`limit:200 fallback是否要做，或確認finance-auditor MANIFEST model欄外推值，或等ig_messages自然累積後觸發P2c §7補測，或順手清`.fhsAudit_qtyWarn`死CSS+learnings.md超量+session-log.md S172-S178缺口
...
> 上次更新：2026-07-16（S180 V42快捷列優化：月曆入快捷列+查看檔期掣取消+快捷列自訂系統+約定日期簡化+Eugenia row bug修復，詳見下表[S180]列）

| 優先 | 項目 | 狀態 | 備註 |
|------|------|------|------|
| ✅ 完成 | **[S180] V42快捷列優化：月曆入列+查看檔期掣取消+快捷列自訂系統+約定日期簡化+row bug修復**（worktree `v42-shortcut-bar-optimize`） | ✅ 全交付，四批sonnet執行+playwright實測PASS，已同步V42+current.html | Fat Mo直接指示UI優化：①快捷鍵「修改」改「月曆」(開`openMoldCalendar({bindMode:'view'})`)，預設隱藏「修改」可經編輯模式加返②訂單總覽頁頂獨立「查看檔期」掣取消，功能併入快捷列③快捷列新增自訂系統：手機長按600ms/Desktop右鍵bar進入編輯，可增減顯示掣，`localStorage['fhsShortcutBarV1']`持久化④追加：取消月曆popup「近期排期」tab、約定日期欄位原生+自訂雙日曆重疊問題簡化為單一自訂日曆(readonly+隱藏原生圖示)⑤現場回報bug修復：表單模式(`bindMode:'form'`)月曆明細row撳唔到，根因`bookingRowHtml()`淨view模式綁click，已修復兩入口統一可撳row跳單且不影響表單草稿。全文見Changelog.md S180條目；方案書見 [shortcut-bar-custom-plan_2026-07-16.md](../reports/planning/shortcut-bar-custom-plan_2026-07-16.md)
| ✅ 完成 | **[S179續] 取模排程中心 B：迷你月曆 v2 重新設計**（D29 第一期部分執行+同日迭代，worktree `monthly-calendar-empty-slots`） | ✅ v2全交付，playwright八項實測PASS | S179 B初版部署後Fat Mo回饋「不夠用」，指出兩痛點：日格只有點冇得撳落去查、操作者需時常查近期排期評估重新安排。先出mockup示意圖，經3條AskUserQuestion拍板（表單入口改二段式「先睇明細再揀日」/PM6:00起算晚上/近期排期睇成個月）後重新設計：①日格改三時段（上午/下午/晚上，由raw_form_state.appTimeHour/AmPm拆算）；②撳日展開當日明細（時間·客名·單號·狀態chip+空檔行），查看檔期入口明細行可撳開單；③新增「近期排期」tab（成月list，連續全日空自動摺行，＋跳去下一個全日空檔掣）。已取消訂單於查詢層濾走唔佔時段。實測抓到並修復1個真bug：撳日展開明細後popup長高反遮appDate（初版只處理咗初始定位），改為方向感知top/bottom錨定修復，複測零重疊。全文見Changelog.md S179續條目；決策見decisions.md D29附錄
