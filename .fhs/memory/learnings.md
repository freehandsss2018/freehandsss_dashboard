...
24. **n8n HTTP Request v4 POST JSON array 必用 `contentType:"raw"`**：`specifyBody:"string"` + `contentType:"json"` + `JSON.stringify([...])` → n8n 誤序列化字串為 `{"[...]":""}` → PostgREST PGRST204。修法：`contentType:"raw"`（移除 specifyBody），body 保持 `JSON.stringify(...)` 原生字串直送，不做二次處理 — Session 127
...
30. **n8n Code 節點內嵌 dashboard 網址禁憑印象寫死**：Telegram 深連結硬編碼 `yanhei.synology.me:5006/web/`（NAS 內網路徑）實測 401，正確應為 decisions.md 記載之公開網址。修法：任何嵌入網址一律對照 decisions.md + curl 實測 200 才寫入，勿假設內網 port/路徑對外可達 — Session 136
