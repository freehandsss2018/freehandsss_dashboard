...
- **【Pitfall ✅ 2026-06-10 Session 84】新增 order_items 欄位必須同步 n8n 寫入鏈，否則永遠 NULL**：新單主寫入走 n8n `sync_order_to_mirror` RPC（非前端 sbSyncOrder，後者僅 webhook 失敗時 fallback）。新增欄位（如 engraving_text）若只建表/只改前端，未同步 (a) n8n `Supabase Mirror Prep` items.map + (b) RPC INSERT/VALUES/ON CONFLICT 三處 → 該欄永遠 NULL。清單：order_items 加欄 → 必改 Mirror Prep payload + sync_order_to_mirror RPC（範本 0017 `NULLIF` / 0034）。engraving_text 前例：缺欄致鎖匙扣/吊飾刻字全失，立體擺設僅靠 mapOrder raw_form_state.pEngraving fallback 倖存。
- **【Pattern ✅ 2026-06-10 Session 84】Supabase MCP 掉線時用 Management API 繞過跑 DDL**：MCP stdio 掉線（專案/PAT/配置正常時），用 `POST https://api.supabase.com/v1/projects/{ref}/database/query` + `Bearer {PAT}` 跑任意 SQL（含 DDL）。⚠️ 必用 curl（python-urllib UA 觸 Cloudflare 1010）；payload 用 python 寫 cwd 相對檔（/tmp 在 Win python vs Git Bash 不一致）。仍官方路徑非降級。
