...
26. **`stop-kgov.js` 的 [G] flag 以「讀取/編輯財務文件內容密度」觸發，非 execute.md 定義的「diff 物理特徵」**：同一 session 內連續 4 次觸發 [G]，其中 3 次是讀取/編輯 `.fhs/ai/FHS_*.md` 財務專檔（純文件層修正，如修正欄位拼字、SKU 格式描述），零觸及 `.sql`/n8n workflow JSON/Dashboard HTML，卻仍觸發——hook 實際判準疑似關鍵字/檔案類型密度，而非 execute.md [G] 明文的「以 diff 物理特徵為準」（migrations *.sql 含 CREATE OR REPLACE FUNCTION／n8n節點code／Dashboard calculatePricing／cost_configurations資料值 四類）。**驗證法**：`git diff --stat` 確認變更檔案清單是否只有 `.md`，零 `.sql`/`.html`/n8n JSON，即可判定誤觸並安全清除 flag；若清單中真的出現上述四類檔案（如本 session 第 4 次真實觸發：`mcp__supabase__apply_migration` 執行後），則需依規則做完整後效同步（更新 System_Logic_Overview.md 對應章節），不得清除了事 — Session 147
