...
- **維度矩陣化顯示 (Detailed Product Grid) [V29 強化]**：所有產品細節強制採用結構化「膠囊標籤 (Badge)」陣列顯示。
  ...
  - **五維度精準提取**:
    ...
    - **立體細節 (V29 PRO)**: 自動識別套裝款式 (🖼️ 木框、🧴 玻璃瓶)、主題色款 (粉紅、草原、仿古、淺木)、肢體構成 (4肢、2肢)。
    - **數量**: **[V29 強制]** 即使數量為 1 也會顯示 `x1` 標籤，確保視覺一致性。
- **刻字橫向佈佈局**：刻字欄位 (Engraving) 採用 V29 強化顯示，強制將【上排】與【下排】內容平行並列，字體加粗，並移除多餘的括號雜訊，實現極簡視覺。
- **行內快速編輯 (Inline Edit)**：
